...
TL;DR: HUOZIIME是一个基于轻量级大语言模型的个性化移动输入法，通过后训练和分层记忆机制实现设备端的高效个性化文本生成
...
Result: 实验证明系统能够高效地在设备端运行，并实现高保真的记忆驱动个性化
...
TL;DR: 论文发现同行评审分数比文本评论更可靠预测论文接受率，文本评论因礼貌原则包含过多积极情感词而掩盖拒稿信号


<details>
  <summary>Details</summary>
Motivation: 作者经常难以准确解读同行评审反馈，容易从礼貌评论中产生虚假希望，或对具体低分感到困惑。需要研究评审分数与文本评论在预测论文接受率上的可靠性差异。

Method: 构建包含30,000+篇ICLR 2021-2025投稿的数据集，比较基于数值分数和文本评论的接受预测性能。分析分数模型预测失败的样本分布特征，并从评审情感角度探究文本模型性能较差的原因。

Result: 分数模型达到91%准确率，文本模型仅81%（即使使用大语言模型）。分数模型失败的9%样本显示高分峰度和负偏态分布，表明个别低分对拒稿起决定性作用。文本模型性能差源于礼貌原则：拒稿论文的评审仍包含更多积极情感词，掩盖了拒稿信号。

Conclusion: 同行评审分数比文本评论更可靠预测论文接受结果。文本评论因礼貌原则包含过多积极情感词，使作者难以仅从文本判断结果。建议作者更关注数值分数而非文本情感。
...
<details>
  <summary>Details</summary>
Motivation: 海上VHF遇险通信是安全关键语音信息，但实际分析面临多重挑战：信息简短、噪声干扰、说话者紧张导致格式偏差、ASR错误严重。现有方法缺乏足够的标注数据来应对这些现实困难。

Method: 开发SeaAlert框架：1）构建合成数据生成管道，使用LLM生成多样化的现实海上通信文本（包括省略或替换标准遇险关键词的挑战性变体）；2）将文本合成为语音；3）添加模拟VHF噪声；4）通过ASR系统转录获得真实噪声文本。

Result: 论文提出了完整的合成数据生成方法，能够创建包含各种现实挑战（噪声、格式偏差、ASR错误）的训练数据，为后续的LLM分析模型提供高质量的训练基础。

Conclusion: SeaAlert框架通过创新的合成数据生成方法解决了海上遇险通信分析中的数据稀缺问题，为开发鲁棒的自动分析系统提供了可行路径，有望提升海上安全通信的自动化处理能力。
...
Method: 提出教师-学生合作数据合成框架TESSY，交替使用教师和学生模型生成风格token和非风格token。具体来说，教师模型负责生成体现高级推理能力的非风格token，而学生模型负责生成保持风格一致性的风格token，从而合成既继承教师推理能力又符合学生风格分布的数据。

Result: 在代码生成任务中，使用GPT-OSS-120B作为教师模型：传统教师生成数据导致Qwen3-8B在LiveCodeBench-Pro上性能下降3.25%，在OJBench上下降10.02%；而TESSY方法分别实现了11.25%和6.68%的性能提升，显著优于传统方法。

Conclusion: TESSY框架通过教师-学生合作的数据合成方式，有效解决了教师生成数据与学生模型风格不匹配的问题，为利用更强模型提升学生模型推理能力提供了一种有效方法，在代码生成任务上取得了显著性能提升。
...
TL;DR: EviSearch是一个多智能体提取系统，能够直接从临床试验PDF自动创建本体对齐的临床证据表，同时保证每个单元格的可追溯性，用于审计和人工验证。


<details>
  <summary>Details</summary>
Motivation: 临床证据合成需要从大量试验论文中手动提取数据，这个过程耗时且容易出错。现有方法在保持布局完整性、提供可审计的溯源信息以及处理多模态证据（文本、表格、图形）方面存在不足。

Method: 系统采用多智能体架构：1) PDF查询智能体（保留渲染布局和图形）；2) 检索引导的搜索智能体；3) 协调模块（当智能体意见不一致时强制进行页面级验证）。管道设计用于高精度提取多模态证据源，并生成可审查的可追溯信息。

Result: 在临床医生策划的肿瘤学试验论文基准测试中，EviSearch相对于强大的解析文本基线显著提高了提取准确性，同时提供了全面的归因覆盖。系统通过记录协调决策和审查者编辑，产生结构化的偏好和监督信号，用于引导迭代模型改进。

Conclusion: EviSearch旨在加速动态系统评价工作流程，减少手动整理负担，并为将基于LLM的提取集成到证据合成管道中提供安全、可审计的路径。系统特别适用于需要高精度、可验证证据提取的临床研究环境。
...
TL;DR: SAGE Celer 2.6是SAGEA最新发布的多尺寸通用模型，具备自验证推理能力、原生多模态功能和针对南亚语言的专门优化。


<details>
  <summary>Details</summary>
Motivation: 开发一个在复杂推理任务中减少错误和幻觉，同时具备原生多模态能力，并专门优化南亚语言支持的高性能通用模型。

Method: 1. 提供5B、10B、27B三种参数规模；2. 采用逆向推理(IR)管道进行原生训练，使模型能够验证自身逻辑路径；3. 集成端到端视觉编码器实现原生多模态功能；4. 使用Devanagari脚本定制分词器专门优化南亚语言支持。

Result: 在数学、编程和通用智能基准测试(ACUMEN)上取得高度竞争力结果，同时保持低延迟；在尼泊尔语和印地语上表现优异且不牺牲英语推理能力。

Conclusion: Celer 2.6是一个在推理能力、多模态功能和南亚语言支持方面均有显著改进的先进通用模型，特别适合需要处理南亚语言和复杂推理任务的应用场景。
...
TL;DR: 开发基于RAG的对话系统，支持从建筑项目会议纪要中检索带时间标注的决策历史信息
...
Result: 使用比利时大型公司已完成建筑项目的匿名化会议纪要数据集进行演示，数据集经过标注并包含专家定义查询，支持系统评估

Conclusion: 提出的解决方案能够有效支持专业人员通过自然语言对话访问带时间标注的项目文档，数据集和开源实现已公开以促进进一步研究
...
TL;DR: 提出Stateful Evidence-Driven RAG with Iterative Reasoning框架，通过状态化证据池和迭代推理解决传统RAG的平面上下文表示和无状态检索问题


<details>
  <summary>Details</summary>
Motivation: 传统RAG存在平面上下文表示和无状态检索问题，导致性能不稳定。需要建模问答为渐进式证据积累过程，提高对噪声检索的鲁棒性

Method: 将检索文档转换为结构化推理单元，包含显式相关性和置信度信号；维护持久证据池记录支持性和非支持性信息；执行证据驱动的缺陷分析识别差距和冲突；迭代优化查询指导后续检索

Result: 在多个问答基准测试中相比标准RAG和多步骤基线获得一致改进，能有效积累高质量证据并在大量检索噪声下保持稳定性能

Conclusion: 提出的状态化证据驱动RAG框架通过迭代推理实现稳定证据聚合，显著提升问答系统的鲁棒性和性能
...
TL;DR: 该研究系统评估了三种8B参数开源大语言模型在罗马化尼泊尔语上的适应能力，通过零样本和微调设置对比性能，发现Qwen3-8B整体表现最佳，而Llama-3.1-8B在微调后提升最大。


<details>
  <summary>Details</summary>
Motivation: 罗马化尼泊尔语（用拉丁字母书写的尼泊尔语）是尼泊尔非正式数字通信的主要媒介，但在大语言模型中严重缺乏资源。需要建立首个系统基准来评估开源模型对该语言的适应能力。

Method: 使用三种可比规模的开源模型（Llama-3.1-8B、Mistral-7B-v0.1、Qwen3-8B），在包含10,000个音译指令跟随样本的双语数据集上进行评估。采用零样本和微调两种设置，使用QLoRA与rsLoRA（r=32）在双NVIDIA Tesla T4 GPU上微调，仅训练约1%参数，总GPU时间少于27小时。评估指标包括困惑度、BERTScore、chrF++、ROUGE-1/2/L和BLEU，涵盖流畅性、语音一致性和语义完整性七个维度。

Result: 零样本时所有模型都无法生成罗马化尼泊尔语，各有特定的架构相关失败模式。微调后所有模型都解决了这些问题，BERTScore约0.75，chrF++大于23。Qwen3-8B在十个标准中整体表现最佳，是唯一能产生语义相关零样本输出的模型，且在微调后所有结构对齐指标领先。Llama-3.1-8B尽管零样本基线最弱，但在微调后获得最大的绝对提升：困惑度降低49.77，BERTScore提升0.3287。

Conclusion: Qwen3-8B是罗马化尼泊尔语适应的推荐架构，而Llama-3.1-8B因其最大的微调提升潜力，适合迭代式低资源开发流程。研究证实了"适应提升空间假设"，并为可比规模开源LLM中的罗马化尼泊尔语适应建立了首个严格基准。
...
TL;DR: 通过训练小型后transformer适配器来修正语言模型在政治敏感话题上的事实概率抑制，适配器能记忆训练事实并泛化到未见事实，且仅应用于当前位置时能生成连贯、去审查的文本。


<details>
  <summary>Details</summary>
Motivation: 对齐调优的语言模型经常在政治敏感话题上抑制事实概率，尽管知识仍保留在隐藏表示中。研究旨在修正这种概率抑制，使模型能更准确地表达事实知识。

Method: 训练一个仅78.6万参数的后transformer适配器（约基础模型的0.02%），在冻结的隐藏状态上进行训练。使用有门控（SwiGLU）和无门控（线性瓶颈）两种架构，通过锚定训练避免知识退化。适配器仅在预测位置（最后位置）应用以保持生成连贯性。

Result: 适配器能记忆所有15个训练事实，并在5个随机分割中泛化到16个保留事实的11-39%。两种架构表现相当（Fisher精确检验p>0.09）。在指令模型中能修正概率排名。仅应用于当前位置时能生成连贯、去审查的文本，而全位置应用会导致不连贯输出。发现并修复了Apple MLX中的静默梯度错误。

Conclusion: 小型隐藏状态适配器能有效修正语言模型在敏感话题上的事实概率抑制，且仅应用于预测位置时能保持生成质量。隐藏状态干预是生成修正的正确层级，而logit空间干预会失败。MLX中的梯度错误需要特定调用模式避免。
...
TL;DR: 本文提出一个统一系统处理ArchEHR-QA共享任务的两个子任务：答案生成（子任务3）和证据句子对齐（子任务4），使用量化低秩适应和集成检索方法，指出20个标注训练样本不足以区分临床相关与无关句子。


<details>
  <summary>Details</summary>
Motivation: 解决ArchEHR-QA共享任务中的两个关键挑战：从临床文本生成答案（子任务3）以及将答案与支持性证据句子对齐（子任务4）。研究旨在探索在有限标注数据（仅20个开发案例）下如何有效处理临床问答任务。

Method: 1. 子任务3：采用两阶段量化低秩适应（QLoRA）方法，首先在30,000个emrQA-MedSQuAD语料样本上进行临床领域适应，然后在20个标注开发案例上进行任务特定输出风格学习，使用Qwen3-4B模型和4位NF4量化。
2. 子任务4：开发加权集成检索方法，结合BM25（带相对阈值）、TF-IDF余弦相似度和微调交叉编码器，用于识别支持给定黄金答案的病历句子。

Result: 1. 子任务3在官方test-2026分割上获得总分32.87（BLEU=9.42, ROUGE-L=27.04, SARI=55.42, BERTScore=43.00, AlignScore=25.28, MEDCON=37.04）。
2. 子任务4在100个案例测试集上获得微平均F1分数67.16。
3. 实验揭示两个子任务面临相同根本挑战：20个标注训练案例不足以区分临床相关与无关句子。

Conclusion: 提出的统一系统在ArchEHR-QA共享任务的两个子任务上取得有竞争力的表现，但实验表明有限标注数据是主要瓶颈。数据增强被确定为最高杠杆的未来研究方向，以改善临床句子相关性判断能力。
...
TL;DR: SPFG数据集为口语语法纠错与教学反馈生成提供人工验证的教师风格反馈，研究显示监督微调在联合生成纠错和反馈方面效果最稳定


<details>
  <summary>Details</summary>
Motivation: 现有语法纠错和解释系统缺乏真实教学场景所需的可操作、适合学习者水平且具有鼓励性的教学反馈，需要开发能够生成学习者友好型教学反馈的系统

Method: 基于Speak & Improve Challenge 2025语料构建SPFG数据集，包含流利导向的转录文本、语法纠错目标及人工验证的教师风格反馈；评估三种指令调优大语言模型，比较监督微调与基于偏好的对齐方法在联合生成纠错和反馈方面的表现

Result: 监督微调提供最一致的改进效果，而DPO/KTO方法带来的提升较小或效果混合；纠错质量与反馈质量之间存在弱耦合关系

Conclusion: SPFG数据集为口语语法纠错与教学反馈生成研究提供了有价值的资源，监督微调是生成联合纠错和反馈的最有效方法，但纠错与反馈质量的相关性较弱，表明需要进一步优化反馈生成策略
...
<details>
  <summary>Details</summary>
Motivation: 罕见病影响全球超3亿人，具有复杂的护理路径、有限的临床专业知识和长期患者旅程中未满足的沟通需求。大型语言模型的发展为支持患者教育与沟通提供了新机遇，但其在罕见病领域的应用情况尚不明确。

Method: 对2022年1月至2026年3月主要数据库发表的研究进行范围综述，识别出12项关于LLM在罕见病患者教育与沟通应用的研究。提取研究特征、应用场景、模型使用和评估方法等数据，采用描述性和定性分析进行综合。

Result: 文献高度集中于近期，以通用模型（特别是ChatGPT）为主。多数研究聚焦于使用策划问题集的患者问答，很少使用真实世界数据或纵向沟通场景。评估主要关注准确性，对可读性、同理心和沟通质量等患者中心维度关注有限。多语言沟通很少涉及。
...
TL;DR: 训练纯古典中文语言模型，发现内部不确定性（困惑度差异）与外部不确定性表达（语言标记使用）存在分离，表明语言模型无法自发表达"我不知道"的元认知能力


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否能区分已知与未知输入，并能在生成文本中表达这种不确定性，探索语言模型是否具备元认知表达能力

Method: 训练318M参数Transformer模型于纯古典中文语料（15.6亿token），进行系统性分布外测试，比较真实、虚构、半虚构历史事件的困惑度差异，分析古典中文认知标记的使用频率

Result: 模型内部能区分真实与虚构事件（困惑度比2.39x），半虚构事件困惑度最高（4.24x），但外部从不表达不确定性：OOD问题认知标记使用率（3.5%）低于分布内问题（8.3%）。结果在三种语言、三种书写系统、八个模型（110M-1.56B）中可复现

Conclusion: 语言模型能编码事实知识但无法自发表达元认知不确定性，不确定性表达完全由训练数据惯例决定，需要RLHF等显式训练信号才能获得"我不知道"的表达能力
...
TL;DR: PICCO是一个通过系统合成11个现有提示框架提出的五元素参考架构，用于结构化LLM提示设计，包含Persona、Instructions、Context、Constraints、Output五个核心元素。


<details>
  <summary>Details</summary>
Motivation: 当前LLM提示设计存在描述和应用不一致的问题，缺乏统一的参考框架。本研究旨在通过系统分析现有提示框架，提出一个概念清晰、结构化的提示设计参考架构，以改善提示设计的系统性和一致性。

Method: 通过多数据库搜索识别11个已发布的提示框架，进行严格的综合分析。提出区分提示框架、提示元素、提示生成、提示技术和提示工程的概念分类法，并推导出PICCO五元素参考架构。

Result: 提出了PICCO框架，包含五个核心元素：Persona（角色）、Instructions（指令）、Context（上下文）、Constraints（约束）和Output（输出）。为每个元素定义了功能、范围和与其他元素的关系，并提供了实施相关的关键概念，包括各种提示技术、迭代提示工程方法以及负责任提示的考虑因素。

Conclusion: PICCO是一个概念性和方法论贡献，为提示规范和比较提供了通用结构框架，提高了概念清晰度并支持更系统的提示设计。该框架尚未经过实证验证作为优化方法，但为未来研究提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 针对社交媒体上流传的"中文提示比英文更token高效，可节省高达40%成本"的说法进行验证，该说法影响了开发者考虑转向中文进行"氛围编程"以节省API成本
...
Conclusion: 语言对token成本的影响是模型依赖的，开发者不应期望通过切换提示语言到中文来获得成本节省或性能提升，这些发现应被视为初步证据而非确定结论
...
### [17] [ReviewGrounder: Improving Review Substantiveness with Rubric-Guided, Tool-Integrated Agents](https://arxiv.org/abs/2604.14261)
...
Method: 1. 引入REVIEWBENCH基准，根据官方指南、论文内容和人工评审意见制定论文特定评分标准来评估评审文本质量。2. 提出REVIEWGROUNDER框架，采用评分标准引导、工具集成的多智能体系统，将评审过程分解为起草和证据夯实两个阶段，通过有针对性的证据整合来丰富浅层草稿。

Result: 实验表明，使用Phi-4-14B起草器和GPT-OSS-120B证据夯实阶段的REVIEWGROUNDER，在8个维度的评分标准对齐度和评审质量方面，始终优于使用更强大/更大骨干模型（如GPT-4.1和DeepSeek-R1-670B）的基线方法。

Conclusion: 通过结合明确的评分标准和上下文证据夯实，REVIEWGROUNDER框架能够生成更高质量、更具实质性的LLM评审意见，为解决当前LLM评审的局限性提供了有效方案。
...
TL;DR: 开发EuropeMedQA——首个基于意大利、法国、西班牙和葡萄牙官方监管考试的多语言多模态医学考试数据集，用于评估LLM在非英语医学诊断任务中的表现
...
Method: 遵循FAIR数据原则和SPIRIT-AI指南，建立严格的数据筛选流程和自动翻译管道，使用零样本、严格约束的提示策略评估当代多模态LLM的跨语言迁移和视觉推理能力

Result: 创建了EuropeMedQA数据集，这是一个抗污染基准，旨在促进更具普适性的医学AI发展，但具体评估结果未在摘要中提供

Conclusion: EuropeMedQA为评估LLM在欧洲多语言多模态医学环境中的表现提供了首个综合性基准，有助于推动更具普适性的医学人工智能发展
...
TL;DR: 该研究通过分析126,602篇新闻文章，量化了暴力灾难事件报道的叙事动态，发现突发事件的新闻周期具有可预测模式：报道量激增、早期语义漂移、逐渐回归基线。
...
Result: 突发影响事件展现出结构化且可预测的新闻周期模式：报道量快速激增、早期语义漂移、逐渐向基线水平下降。研究还识别出驱动这些时间模式的特定术语。

Conclusion: 暴力灾难事件的媒体报道遵循可量化的动态模式，这些模式有助于理解公共话语的形成和演变，为危机时期的叙事分析提供了实证框架。
...
TL;DR: GPT-4.1通过分析棒球迷的开放式文本反馈预测其整体体验评分（0-10分），预测结果与实际调查评分在约10,000份响应中达到67%的±1分内准确率，但存在系统性低估约1分的差异。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索大型语言模型能否从球迷的开放式文本反馈中准确预测其整体体验评分，并比较AI预测与自我报告评分之间的差异及其潜在原因。

Method: 使用GPT-4.1模型分析来自5支美国职业棒球大联盟球队约10,000份球迷的开放式文本反馈，预测其0-10分的整体体验评分。将AI预测结果与实际调查评分进行比较，评估预测准确性（精确匹配、±1分内匹配），并分析预测评分与各项具体体验维度（停车、餐饮、员工等）的相关性。

Result: 预测评分与实际评分高度相关（r=0.82），67%的预测落在实际评分±1分范围内，36%为精确匹配。预测具有高度一致性（三次独立评分运行中87%精确一致，99.9%在±1分内）。但预测评分系统性低估约1分，且这种差异不受任何单一体验维度驱动。

Conclusion: 简单的未优化提示即可从文本中方向性预测球迷体验评分。预测评分与自我报告评分之间的差异反映了两种测量方式捕捉不同构念：自我报告评分反映整体评价判断，而预测评分量化了突出时刻（难忘、情感强烈、不寻常或可行动）的影响。这种差异值得保留而非消除。
...
### [21] [Faithfulness Serum: Mitigating the Faithfulness Gap in Textual Explanations of LLM Decisions via Attribution Guidance](https://arxiv.org/abs/2604.14325)
*Bar Alon,Itamar Zimerman,Lior Wolf*

Main category: cs.CL

TL;DR: 本文研究LLM生成解释的认知忠实性问题，提出通过注意力干预提升解释忠实性的无训练方法


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能强大但缺乏可解释性，限制了其在需要透明度和信任的领域应用。现有的事后文本解释方法主要关注生成主观上可信的推理，但这些解释是否认知忠实（反映模型实际依赖的内部证据）尚不明确。

Method: 首先通过反事实方法评估LLM生成解释的认知忠实性，发现其往往不忠实。然后提出无训练方法，通过基于忠实归因方法提取的token级热图进行注意力级干预，指导解释生成过程。

Result: 该方法在多个模型、基准和提示下显著提升了认知忠实性，证明了注意力干预对增强解释忠实性的有效性。

Conclusion: LLM生成的事后文本解释往往缺乏认知忠实性，但通过注意力级干预可以显著改善这一状况，为开发更可信的模型解释方法提供了有效途径。
...
### [22] [Shuffle the Context: RoPE-Perturbed Self-Distillation for Long-Context Adaptation](https://arxiv.org/abs/2604.14339)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要可靠长上下文理解的场景中应用日益广泛，但标准的长上下文适应方法仍然脆弱：模型准确度强烈依赖于相关证据的绝对位置，即使控制任务格式和难度，仍表现出高位置方差。

Method: 提出RoPE-Perturbed Self-Distillation训练正则化方法，核心思想是通过扰动训练序列的RoPE索引形成替代"视图"，有效将上下文的不同部分移动到不同位置，然后通过自蒸馏训练模型在不同视图间产生一致预测，从而鼓励模型依赖语义信号而非脆弱的位置依赖。

Result: 在Llama-3-8B和Qwen-3-4B的长上下文适应实验中，该方法在长上下文基准测试中表现出一致的提升：Llama-3-8B在RULER-64K上提升达12.04%，Qwen-3-4B在RULER-256K上提升2.71%（SFT后），同时改善了超出训练上下文窗口的长度外推能力。
...
### [23] [When PCOS Meets Eating Disorders: An Explainable AI Approach to Detecting the Hidden Triple Burden](https://arxiv.org/abs/2604.14356)
*Apoorv Prasad,Susan McRoy*

Main category: cs.CL

TL;DR: 开发小型开源语言模型，用于从社交媒体帖子中检测PCOS患者的三重负担（身体形象困扰、饮食失调、代谢挑战），并提供可解释的证据。


<details>
  <summary>Details</summary>
Motivation: PCOS患者面临身体形象困扰、饮食失调和代谢挑战的三重负担，现有NLP方法缺乏透明度且无法识别共病表现，需要开发可解释的检测工具。

Method: 从6个subreddit收集1000个PCOS相关帖子，由两名训练有素的标注员使用Lee等人(2017)临床框架进行标注。使用三种模型(Gemma-2-2B, Qwen3-1.7B, DeepSeek-R1-Distill-Qwen-1.5B)通过低秩适应进行微调，生成带有文本证据的结构化解释。

Result: 最佳模型在150个保留测试帖子上达到75.3%的精确匹配准确率，具有稳健的共病检测能力和强大的可解释性。性能随诊断复杂性下降，表明最适合用于筛查而非自主诊断。

Conclusion: 开发的小型开源语言模型能够有效检测PCOS三重负担，提供可解释的证据，适合作为筛查工具，但诊断复杂性会影响性能，不适用于自主诊断。
...
### [24] [The Cost of Language: Centroid Erasure Exposes and Exploits Modal Competition in Multimodal Language Models](https://arxiv.org/abs/2604.14363)
*Akshay Paruchuri,Ishan Chatterjee,Henry Fuchs,Ehsan Adeli,Piotr Didyk*

Main category: cs.CL

TL;DR: 本文提出质心替换方法，通过将token替换为最近的K-means质心来探测多模态模型中的模态依赖不平衡问题，发现语言表示在视觉推理任务中仍占主导地位，并利用文本质心对比解码在推理时无需重新训练即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态语言模型在视觉感知任务上系统性表现不佳，但其失败背后的结构机制尚不清楚。研究者希望探究模态依赖的不平衡问题，特别是语言表示如何主导视觉推理任务。

Method: 提出质心替换方法，将每个token替换为其最近的K-means质心，作为模态依赖的受控探针。在涵盖三种架构家族的七个模型上进行实验，通过文本质心对比解码，在推理时对比解码文本质心被擦除的参考模型来提升性能。

Result: 擦除文本质心结构比擦除视觉质心结构多损失4倍准确率，揭示了语言表示在视觉推理任务中仍占主导的普遍不平衡。通过文本质心对比解码，在单个任务上恢复高达+16.9%的准确率。标准微调模型平均增益更大(+5.6%)，偏好优化模型增益较小(+1.5%)。

Conclusion: 模态竞争在结构上是局部化的，可以在推理时无需重新训练进行纠正，并可量化为指导未来多模态训练的诊断信号。质心替换为理解多模态模型中的模态依赖提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 对话式事实核查中，口语化语言频繁出现但研究不足，现有方法难以有效处理多轮对话中的非正式表达，需要专门的方法来处理口语化语言对事实核查的影响

Method: 1. 分阶段去口语化重写：结合轻量级表面规范化与限定范围内的指代消解，为每个响应声明生成保守的重写候选；2. BiCon-Gate语义感知一致性门控：当重写候选得到对话上下文语义支持时选择重写版本，否则回退到原始声明，以稳定下游事实核查

Result: 在DialFact基准测试中，该方法在证据检索和事实验证方面均有提升，特别是在SUPPORTS类别上表现突出，优于包括基于解码器的一次性LLM重写在内的竞争基线

Conclusion: 提出的分阶段去口语化重写与语义感知一致性门控方法能有效处理对话中的口语化语言，提升事实核查系统的性能，证明了保守重写与上下文感知选择的重要性
...
### [26] [Generating Concept Lexicalizations via Dictionary-Based Cross-Lingual Sense Projection](https://arxiv.org/abs/2604.14397)
*David Basil,Chirooth Girigowda,Bradley Hauer,Sahir Momin,Ning Shi,Grzegorz Kondrak*

Main category: cs.CL

TL;DR: 通过语义投影自动扩展WordNet式词汇资源到新语言的方法，利用对齐的语料库将英语同义词集映射到目标语言词元，并通过双语词典过滤错误投影


<details>
  <summary>Details</summary>
Motivation: 解决多语言词汇资源扩展的自动化问题，特别是WordNet式词汇资源在新语言中的构建，避免传统方法的高成本和时间消耗

Method: 基于语义投影的方法：1) 使用带有词义标注的英语语料库及其翻译；2) 将英语同义词集投影到对齐的目标语言词元上；3) 通过双语词典增强预训练对齐器并过滤错误的词义投影

Result: 在多种语言上评估显示，提出的投影-过滤策略提高了精度，同时保持可解释性且需要较少外部资源，优于先前方法、基于词典的方法和大语言模型基线

Conclusion: 该方法为自动扩展多语言词汇资源提供了一种有效途径，计划公开代码、文档和生成的词义清单
...
### [27] [The Autocorrelation Blind Spot: Why 42% of Turn-Level Findings in LLM Conversation Analysis May Be Spurious](https://arxiv.org/abs/2604.14414)
*Ferdinand M. Schessl*
...
<details>
  <summary>Details</summary>
Motivation: 当前多轮人-LLM对话评估中广泛使用轮次级指标，但几乎所有现有评估流程都忽略了连续轮次间的统计依赖性，导致统计推断出现严重偏差，需要系统性地解决这一问题。

Method: 系统分析了66个轮次级指标在202个多轮对话中的自相关结构，提出了结合Chelton有效自由度与对话级块自助法的两阶段校正框架，并在预注册的保留集上验证了其有效性。

Result: 研究发现：1）标准池化分析导致显著性估计严重膨胀，42%在标准池化检验下显著的关联在校正后不再显著；2）膨胀程度因指标类别而异，非记忆型指标家族膨胀率更高；3）校正后的聚类稳健指标在保留集上的可复制率为57%，远高于未校正指标的30%。

Conclusion: 多轮对话评估必须考虑时间依赖性，提出的校正框架能有效解决统计推断偏差问题，并为该领域提供了具体的设计原则、发表检查清单和开源代码。
...
### [28] [Hierarchical vs. Flat Iteration in Shared-Weight Transformers](https://arxiv.org/abs/2604.14442)
*Sang-Il Han*

Main category: cs.CL

TL;DR: HRM-LM使用分层共享权重的循环结构替代传统Transformer的独立层堆叠，通过快慢双模块的循环机制实现局部细化和全局压缩，但实验发现其表征能力与独立层堆叠存在显著差距。
...
Method: HRM-LM用两速循环对（Fast模块每步操作进行局部细化，Slow模块每T步操作进行全局压缩）替代L个独立Transformer层。该循环层次结构以共享参数展开M = N x T步，并与参数匹配的Universal Transformer消融实验（UniTF, 1.2B）进行对比。

Result: 核心且最稳健的发现是两种方法之间存在显著的实证差距，这一发现在五个独立运行的参数匹配实验中均得到支持。

Conclusion: 分层共享权重的循环结构在表征质量上无法匹配独立层堆叠的Transformer架构，表明当前循环机制在语言建模任务中存在局限性。
...
### [29] [Filling in the Mechanisms: How do LMs Learn Filler-Gap Dependencies under Developmental Constraints?](https://arxiv.org/abs/2604.14459)
*Atrey Desai,Sathvik Nair*

Main category: cs.CL

TL;DR: 该研究使用分布式对齐搜索方法分析在不同数据量下训练的语言模型，探讨填充语-空位依赖关系在wh疑问句和话题化结构中的共享表征机制，发现模型需要比人类多得多的数据才能获得类似的语言泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型是否能在发展上可行的数据量下学习到填充语-空位依赖关系的共享表征。虽然因果分析表明大语言模型可能存在这种表征，但尚不清楚在有限训练数据下是否也能形成这种跨句法结构的共享机制。

Method: 采用分布式对齐搜索方法，应用于在不同数据量下训练的语言模型。这些模型使用BabyLM挑战赛的数据进行训练，评估填充语-空位依赖关系在wh疑问句和话题化结构之间的表征迁移，这两种结构在输入频率上有很大差异。

Result: 结果表明，即使在有限训练数据下，也可能发展出共享但项目敏感的机制。更重要的是，语言模型需要比人类多得多的数据才能学习到可比较的语言泛化能力。

Conclusion: 研究强调了在语言习得模型中需要语言特定偏置的重要性。语言模型需要远超人类的数据量才能获得类似的语言泛化，这揭示了当前模型在语言习得效率方面的局限性。
...
### [30] [Psychological Steering of Large Language Models](https://arxiv.org/abs/2604.14463)
*Leonardo Blas,Robin Jia,Emilio Ferrara*

Main category: cs.CL

TL;DR: 本文提出了一种心理引导框架，通过语义校准单位进行无界、流畅性约束的扫描，使用IPIP-NEO-120测量OCEAN人格模型，比较六种注入方法，发现均值差注入在14个LLM中的11个优于人格提示，混合方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型激活级干预方法限制搜索空间并在未校准的激活空间单位中进行扫描，可能错过最优干预条件，需要一种能够进行无界、流畅性约束扫描的语义校准方法。

Method: 提出心理引导框架，使用心理人工制品推导和校准残差流注入，采用IPIP-NEO-120测量OCEAN人格模型，比较六种注入方法，包括均值差注入和人格提示的混合方法。

Result: 均值差注入在14个LLM中的11个优于人格提示，性能提升3.6%至16.4%；混合方法在13个LLM中优于两种方法，相比人格提示提升5.6%至21.9%，相比均值差注入提升3.3%至26.7%；均值差注入符合线性表示假设，但诱导的OCEAN特质协方差模式偏离Big Two模型。

Conclusion: 表示工程是开放式心理引导的新前沿，均值差注入提供了可靠、近似线性的心理引导控制旋钮，但学习到的表示与人类心理学之间存在差距，混合方法展示了结合不同干预策略的优势。
...
### [31] [CobwebTM: Probabilistic Concept Formation for Lifelong and Hierarchical Topic Modeling](https://arxiv.org/abs/2604.14489)
*Karthik Singaravadivelan,Anant Gupta,Zekun Wang,Christopher MacLellan*

Main category: cs.CL

TL;DR: CobwebTM：基于增量概率概念形成的低参数终身分层主题模型，利用预训练表示实现无监督主题发现、动态主题创建和分层组织


<details>
  <summary>Details</summary>
Motivation: 神经方法需要大量调参且存在灾难性遗忘问题，而经典概率模型缺乏灵活性和对流式数据的适应性，需要一种能在线构建语义层次、无需预定义主题数量的终身学习主题模型

Method: 将Cobweb算法适配到连续文档嵌入，基于增量概率概念形成构建语义层次，在线进行无监督主题发现、动态主题创建和分层组织

Result: 在多个数据集上实现了强主题连贯性、随时间稳定的主题和高质量层次结构，表明增量符号概念形成与预训练表示结合是主题建模的有效方法

Conclusion: CobwebTM通过结合增量符号概念形成和预训练表示，提供了一种高效的主题建模方法，能够在线构建语义层次、动态创建主题，且无需预定义主题数量
...
### [32] [PeerPrism: Peer Evaluation Expertise vs Review-writing AI](https://arxiv.org/abs/2604.14513)
*Soroush Sadeghian,Alireza Daqiq,Radin Cheraghi,Sajad Ebrahimi,Negar Arabzadeh,Ebrahim Bagheri*

Main category: cs.CL

TL;DR: PeerPrism是一个包含20,690篇同行评审的大规模基准数据集，旨在区分思想来源与文本来源，评估LLM检测方法在混合人机协作场景下的表现，发现现有方法混淆了表面文本与智力贡献。


<details>
  <summary>Details</summary>
Motivation: 当前同行评审中的LLM检测方法主要将作者身份视为二元问题（人类vs.AI），未能考虑现代评审工作流程中人类与AI混合协作的实际情况，需要开发能够区分思想来源与文本来源的评估框架。

Method: 构建PeerPrism基准数据集，包含20,690篇同行评审，采用受控生成机制涵盖完全人类撰写、完全AI生成以及多种混合转换（如人类思想+AI文本表达、AI思想+人类重写等），系统评估现有LLM文本检测方法在不同协作模式下的表现。

Result: 现有检测方法在标准二元任务（人类vs.完全合成）上表现良好，但在混合协作场景下预测结果出现显著分歧；特别是当思想来自人类而表面文本由AI生成时，检测器经常产生矛盾分类；风格计量和语义分析表明当前方法混淆了表面实现与智力贡献。

Conclusion: 同行评审中的LLM检测不能简化为二元归属问题，作者身份应建模为涵盖语义推理和风格实现的多维结构；PeerPrism是首个评估此类人机协作场景的基准，为可重复研究提供了完整的数据、代码和评估脚本。
...
### [33] [Mechanistic Decoding of Cognitive Constructs in LLMs](https://arxiv.org/abs/2604.14593)
...
Method: 提出了基于表征工程（RepE）的认知逆向工程框架，结合评价理论与子空间正交化、基于回归的加权和双向因果引导，分离并量化嫉妒的两个心理前因：比较对象的优越性和领域自我定义相关性，并检验它们对模型判断的因果效应。

Result: 在Llama、Qwen和Gemma家族的八个LLM上的实验表明，模型将嫉妒原生编码为这些构成因素的线性组合。其内部表征与人类心理结构基本一致，将优越性视为基础触发因素，相关性作为最终强度乘数。

Conclusion: 该框架展示了有毒情感状态可以被机械检测和精确抑制，为多智能体环境中AI安全的表征监控和干预提供了一条可能路径。
...
### [34] [NLP needs Diversity outside of 'Diversity'](https://arxiv.org/abs/2604.14595)
*Joshua Tint*

Main category: cs.CL

TL;DR: 该立场论文指出NLP领域多样性进展过度集中于少数公平性相关领域，这是由于激励、偏见和障碍导致边缘化研究者被排斥或转向公平性领域，论文通过子领域人口统计调查支持这一观点并提出促进包容性的建议。


<details>
  <summary>Details</summary>
Motivation: 论文旨在揭示NLP领域多样性进展的不均衡分布问题，特别关注公平性领域与其他子领域之间的差异，以及这种不均衡对边缘化研究者的影响。

Method: 采用立场论文形式，结合对NLP研究者按子领域进行的人口统计调查，分析激励结构、偏见和系统性障碍如何影响研究者的领域选择。

Result: 研究发现NLP多样性进展主要集中在少数公平性相关领域，存在系统性因素导致边缘化研究者在非公平性领域被边缘化或被迫转向公平性领域。

Conclusion: 提出打破强化不平等的反馈循环、解决地理和语言障碍等具体建议，以确保NLP所有子领域都能变得更加包容和公平。
...
### [35] [CausalDetox: Causal Head Selection and Intervention for Language Model Detoxification](https://arxiv.org/abs/2604.14602)
*Yian Wang,Yuen Chen,Agam Goyal,Hari Sundaram*

Main category: cs.CL

TL;DR: CAUSALDETOX：通过因果干预注意力头实现大语言模型去毒化的框架，在保持生成质量的同时显著降低毒性


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成有毒内容，现有缓解策略通常会降低生成质量或需要昂贵的人工标注，需要一种更有效的去毒化方法

Method: 使用必要性和充分性概率（PNS）识别导致毒性生成的特定注意力头，通过两种策略干预：局部推理时间干预（动态构建输入特定的导向向量）和PNS引导的微调（永久消除毒性表示）

Result: 在ToxiGen、ImplicitHate和ParaDetox数据集上，CAUSALDETOX相比基线实现高达5.34%的毒性降低，同时保持语言流畅性，注意力头选择速度提升7倍

Conclusion: CAUSALDETOX通过因果干预注意力头提供了一种有效的去毒化框架，在降低毒性的同时保持生成质量，并引入了PARATOX基准用于受控反事实评估
...
### [36] [Retrieve, Then Classify: Corpus-Grounded Automation of Clinical Value Set Authoring](https://arxiv.org/abs/2604.14616)
...
<details>
  <summary>Details</summary>
Motivation: 临床价值集构建是临床质量测量和表型分析中的瓶颈任务。直接使用大语言模型生成标准化词汇代码存在局限性，因为临床词汇库庞大、版本控制且预训练时未可靠记忆。

Method: 提出检索增强集合补全(RASC)方法：首先从已标注语料库中检索K个最相似的现有价值集形成候选代码池，然后对每个候选代码应用分类器进行筛选。理论优势在于将输出空间从完整词汇库缩小到更小的候选池。

Result: 在11,803个公开VSAC价值集上构建首个大规模基准。基于SAPBert微调的交叉编码器达到AUROC~0.852和价值集级F1~0.298，优于三层MLP(AUROC~0.799, F1~0.250)。两种方法都将每个真阳性的无关候选数从12.3(仅检索)分别降至约3.2和4.4。零样本GPT-4o的F1仅0.105，48.6%返回代码不在VSAC中。

Conclusion: RASC方法通过检索-选择策略有效解决了临床价值集构建问题，显著优于直接使用LLM的方法，且其优势随价值集规模增大而增强。该方法适用于多种分类器模型，具有广泛适用性。
...
### [37] [StoryCoder: Narrative Reformulation for Structured Reasoning in LLM Code Generation](https://arxiv.org/abs/2604.14631)
*Geonhui Jang,Dongyoon Han,YoungJoon Yoo*

Main category: cs.CL

TL;DR: StoryCoder框架将代码生成问题转化为连贯的自然语言叙事，通过任务概述、约束和测试用例三部分提供结构化问题表示，显著提升代码生成性能。
...
Method: 提出StoryCoder叙事重构框架，将代码生成问题转化为由任务概述、约束和示例测试用例组成的自然语言叙事，并基于选定算法和体裁进行指导。

Result: 在HumanEval、LiveCodeBench和CodeForces上的11个模型实验中，平均零样本pass@10提升18.7%。叙事重构引导模型采用正确算法策略，减少实现错误，并产生更模块化的代码结构。

Conclusion: 结构化问题表示对代码生成至关重要，其效果取决于叙事连贯性和体裁对齐，且不受模型规模或架构影响。叙事重构能有效提升代码生成性能。
...
### [38] [Pushing the Boundaries of Multiple Choice Evaluation to One Hundred Options](https://arxiv.org/abs/2604.14634)
*Nahyun Lee,Guijin Son*

Main category: cs.CL

TL;DR: 大规模选项评估协议：通过将候选集扩展到100个选项来减少偶然性影响，揭示在低选项设置中被掩盖的模型能力缺陷


<details>
  <summary>Details</summary>
Motivation: 传统多选评估在低选项设置中接近天花板准确率可能通过捷径策略维持，掩盖了真实能力，需要更严格的评估框架来揭示模型在极端干扰密度下的可靠性

Method: 提出大规模选项评估协议，将候选集扩展到100个选项；应用于韩语正字法错误检测任务，模型需从大量候选中选出单个错误句子；采用固定目标和重复重采样、洗牌来获得稳定估计；进行填充控制和长度匹配测试以分离上下文长度影响

Result: 低选项设置中的强性能可能高估模型能力，在高N值密集干扰下优势减弱；识别出两种失败模式：语义混淆和不确定性下对早期选项的位置偏见；主要瓶颈是候选排序而非上下文长度

Conclusion: 大规模选项评估可作为通用框架，在极端干扰密度下压力测试模型可靠性，揭示传统低选项基准无法发现的缺陷
...
### [39] [Fact4ac at the Financial Misinformation Detection Challenge Task: Reference-Free Financial Misinformation Detection via Fine-Tuning and Few-Shot Prompting of Large Language Models](https://arxiv.org/abs/2604.14640)
...
<details>
  <summary>Details</summary>
Motivation: 金融虚假信息泛滥严重威胁市场稳定和投资者信任，导致市场行为误导和信息不对称。现有检测方法在缺乏外部证据或参考资料的现实场景下面临巨大挑战，需要开发仅依赖内部语义理解和上下文一致性的检测技术。

Method: 提出综合框架，利用最先进大语言模型的推理能力，系统整合上下文学习（零样本和少样本提示策略）与参数高效微调（通过LoRA低秩适应），使模型能够捕捉金融操纵的微妙语言线索。

Result: 在"Reference-Free Financial Misinformation Detection"共享任务中，该方法在两个官方排行榜均获得第一名，公开测试集准确率95.4%，私有测试集准确率96.3%，证明了方法的鲁棒性。

Conclusion: 该方法成功展示了基于大语言模型的参考无关金融虚假信息检测的有效性，为金融自然语言处理中的上下文感知虚假信息检测提供了加速，相关模型已在HuggingFace平台开源。
...
### [40] [CURaTE: Continual Unlearning in Real Time with Ensured Preservation of LLM Knowledge](https://arxiv.org/abs/2604.14644)
...
<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法无法满足持续实时需求，导致效用下降和敏感信息长期暴露，需要一种能即时响应遗忘请求且不损害模型知识的方法

Method: 训练句子嵌入模型在专门数据集上形成清晰决策边界，通过计算输入提示与遗忘请求的相似度来决定是否回答或拒绝响应，避免修改语言模型参数

Result: CURaTE比现有方法遗忘效果更好，通过避免参数修改保持近乎完美的知识保护，是唯一能实现实时持续遗忘的方法

Conclusion: CURaTE提供了一种简单有效的实时持续遗忘解决方案，通过外部决策机制实现遗忘而不损害模型知识，为大语言模型的知识管理提供了新思路
...
### [41] [CURA: Clinical Uncertainty Risk Alignment for Language Model-Based Risk Prediction](https://arxiv.org/abs/2604.14651)
*Sizhe Wang,Ziqi Xu,Claire Najjuuko,Charles Alba,Chenyang Lu*

Main category: cs.CL

TL;DR: CURA框架通过双层不确定性目标校准临床语言模型的风险估计和不确定性，同时考虑个体错误可能性和群体模糊性，提升临床决策支持的可信度。


<details>
  <summary>Details</summary>
Motivation: 临床语言模型在自由文本笔记的风险预测中应用日益广泛，但其不确定性估计通常校准不佳且临床不可靠，需要改进以提供更可信的临床决策支持。

Method: CURA框架首先微调领域特定的临床语言模型获取任务适应的患者嵌入，然后使用双层不确定性目标微调多头分类器：个体级校准项使预测不确定性与患者错误可能性对齐，群体感知正则化器将风险估计拉向嵌入空间中局部邻域的事件率，并对决策边界附近的模糊群体赋予额外权重。

Result: 在MIMIC-IV临床风险预测任务上的广泛实验表明，CURA在各种临床语言模型中一致改善了校准指标，且未显著损害判别能力。进一步分析显示CURA减少了过度自信的虚假保证，为下游临床决策支持提供了更可信的不确定性估计。

Conclusion: CURA框架通过同时对齐个体错误可能性和群体模糊性，有效校准临床语言模型的风险估计和不确定性，提高了临床决策支持系统的可靠性和可信度。
...
### [42] [SPAGBias: Uncovering and Tracing Structured Spatial Gender Bias in Large Language Models](https://arxiv.org/abs/2604.14672)
*Binxian Su,Haoye Lou,Shucheng Zhu,Weikang Wang,Ying Liu,Dong Yu,Pengyuan Liu*

Main category: cs.CL

TL;DR: SPAGBias框架首次系统评估LLMs中的空间性别偏见，通过62种城市微空间分类、提示库和三层诊断方法，发现LLMs存在超越公私领域划分的结构化性别-空间关联，这些偏见在模型训练全流程中被嵌入和强化。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在城市规划中应用增加，基于性别空间理论（性别等级制嵌入空间组织）的担忧，需要系统评估LLMs是否再现或放大空间性别偏见，连接社会学理论与计算分析。

Method: 提出SPAGBias框架：1) 62种城市微空间分类法；2) 提示库；3) 三层诊断：显性（强制选择重采样）、概率性（词元级不对称性）、建构性（语义和叙事角色分析）。测试六个代表性模型，考察提示设计、温度和模型规模对偏见表达的影响。

Result: 发现结构化性别-空间关联超越公私领域划分，形成细粒度映射；故事生成揭示情感、措辞和社会角色共同塑造"空间性别叙事"；模型关联显著超出真实世界分布；下游实验显示偏见在规范性和描述性应用场景中产生具体失败。

Conclusion: 该工作将社会学理论与计算分析结合，将偏见研究扩展到空间领域，揭示LLMs如何通过语言编码社会性别认知，为评估和缓解空间性别偏见提供系统框架。
...
### [43] [Which bird does not have wings: Negative-constrained KGQA with Schema-guided Semantic Matching and Self-directed Refinement](https://arxiv.org/abs/2604.14749)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在忠实度和幻觉方面仍有不足。现有KGQA基准和方法偏向正约束和计算约束，忽视了负约束，而负约束在实际问题中频繁出现。

Method: 1) 提出NEST-KGQA任务和NestKGQA数据集；2) 设计Python格式的逻辑形式PyLF以清晰表达否定；3) 开发CUCKOO框架，首先生成约束感知的逻辑形式草稿，进行模式引导的语义匹配，然后仅在执行不当逻辑形式产生空结果时选择性应用自导向精炼。

Result: 实验结果表明，CUCKOO在少样本设置下，在传统KGQA和NEST-KGQA基准测试中均持续优于基线方法。

Conclusion: 该研究填补了KGQA中负约束处理的空白，提出的NEST-KGQA任务、PyLF逻辑形式和CUCKOO框架为处理多约束问题提供了有效解决方案，提高了KGQA系统的鲁棒性和实用性。
...
### [44] [CoPA: Benchmarking Personalized Question Answering with Data-Informed Cognitive Factors](https://arxiv.org/abs/2604.14773)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在问答任务中表现出巨大潜力，但评估个性化能力仍存在关键瓶颈。现有方法主要依赖词汇级相似性或手动启发式规则，缺乏充分的数据驱动验证。

Method: 通过挖掘社区-个体偏好分歧（CIPD）来提取六个关键个性化因素作为评估维度，构建CoPA基准，包含1,985个用户配置文件，通过量化模型输出与用户特定认知偏好的对齐程度进行评估。

Result: CoPA提供了一个比通用指标更全面和更具区分性的个性化问答评估标准，能够量化模型输出与用户特定认知偏好的对齐程度。

Conclusion: CoPA通过数据驱动的方法解决了个性化问答评估的瓶颈问题，为评估LLM的个性化能力提供了更全面和具有区分性的基准。
...
### [45] [Knowing When Not to Answer: Evaluating Abstention in Multimodal Reasoning Systems](https://arxiv.org/abs/2604.14799)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型和多智能体系统的评估范式假设所有问题都是可回答的，迫使模型总是给出回答，而忽略了证据不足时有效弃权的重要性。多模态环境下的弃权研究不足，现有基准要么忽略不可回答性，要么使用粗糙方法无法捕捉现实失败模式。

Method: 提出MM-AQA基准，通过两个轴构建不可回答实例：视觉模态依赖性（图像是否提供必要信息）和证据充分性（证据是否足够）。从可回答实例通过变换生成不可回答实例。评估了三个前沿视觉语言模型（包括闭源和开源模型）和两种多智能体架构，共2079个样本。

Result: 1) 标准提示下，视觉语言模型很少弃权；简单的置信度基线优于此设置。2) 多智能体系统改善弃权但引入准确率-弃权权衡。3) 顺序设计匹配或优于迭代变体，表明瓶颈是校准错误而非推理深度。4) 模型在图像或文本证据缺失时弃权，但在证据降级或矛盾时尝试调和。

Conclusion: 有效的多模态弃权需要弃权感知训练，而非更好的提示或更多智能体。当前模型在证据不足时缺乏可靠弃权能力，需要专门训练来改善这一关键可靠性特征。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法在移除目标知识时难以平衡遗忘与保留的权衡，通常导致模型通用能力下降。需要一种能优先保证保留性能同时有效遗忘的方法。

Method: 将LLM遗忘重构为不对称双任务问题：保留是主要目标，遗忘是辅助任务。提出保留优先的梯度合成框架，包含任务特定梯度提取和冲突感知组合两个阶段。具体实现包括：1) 基于PCGrad的梯度冲突解决；2) 提出SAGO方法，通过构造性符号约束合成确保与保留梯度的严格对齐。

Result: 在WMDP Bio/Cyber和RWKU基准测试中，SAGO持续推动Pareto前沿。例如在WMDP Bio（SimNPO+GD）上，目标模型MMLU性能恢复从44.6%（朴素方法）提升到94.0%（+PCGrad），进一步达到96.0%（+SAGO），同时保持相当的遗忘强度。理论证明两种变体都能确保与保留梯度的非负余弦相似度，而SAGO通过构造性符号约束合成实现更严格的对齐。

Conclusion: 重塑梯度几何而非重新平衡损失是缓解遗忘-保留权衡的关键。保留优先的梯度合成框架通过解耦任务特定梯度提取和冲突感知组合，有效解决了LLM遗忘中的性能权衡问题。
...
TL;DR: 该研究探索了在芬兰语医疗文本上对芬兰语BERT模型进行领域微调的效果，并尝试通过观察嵌入几何变化来预测领域特定预训练的收益。


<details>
  <summary>Details</summary>
Motivation: 在医疗AI领域，获取标注数据通常存在长时间延迟，特别是在标签获取方面。研究旨在解决NLP分类任务中标注数据稀缺的问题，通过领域微调来提升模型性能。

Method: 1) 在芬兰语医疗文本数据上对芬兰语BERT模型进行领域微调；2) 通过观察领域微调引起的嵌入几何变化，尝试预测领域特定预训练的收益。

Result: 论文描述了在芬兰语医疗文本上微调芬兰语BERT的观察结果，并报告了通过嵌入几何变化预测领域预训练收益的尝试。

Conclusion: 该研究为医疗AI领域处理标注数据稀缺问题提供了实用方法，通过领域微调和嵌入几何分析来优化模型性能，特别是在芬兰语医疗文本处理方面。
...
TL;DR: 该研究将行为特征标注视为多项技能的组合而非单一任务，通过技能文件驱动的流程评估LLM辅助标注，发现GPT-5.4在可执行技能上表现可靠，但可行性具有选择性而非全局性。


<details>
  <summary>Details</summary>
Motivation: 行为特征标注难以自动化，因为它需要在多个语言维度上同时编码。研究者将其视为一组标注技能而非单一任务，从这一角度评估LLM辅助的行为特征标注。

Method: 使用3,134条中文隐喻色彩词衍生词的语料和14个特征的标注体系，实施技能文件驱动的标注流程：每个特征通过模式文件、决策规则和示例外部定义。两名人工标注者在300条验证子集上完成两轮标注，将技能分类为可直接操作、可通过聚焦重标注恢复、或结构未指定。然后在相同设置下评估GPT-5.4和三个本地可部署的开源模型。

Result: 行为特征标注在技能层面高度异质：5项技能可直接操作，4项可通过聚焦重标注恢复，5项保持结构未指定。GPT-5.4执行保留技能具有实质性可靠性（准确率=0.678，κ=0.665，加权F1=0.695），但可行性是选择性的而非全局性的。人类和GPT的困难分布在技能层面高度一致（r=0.881），但在实例层面（r=0.016）和词汇项目层面（r=-0.142）不一致，呈现"共享分类体系，独立执行"模式。成对一致性进一步表明GPT应被视为独立的第三技能声音而非直接的人类替代品。开源模型的失败主要集中在模式到技能的执行问题上。

Conclusion: 自动标注应基于技能可行性而非任务级自动化进行评估。GPT-5.4在行为特征标注中表现出选择性可行性，更适合作为独立的标注声音而非人类替代品。技能层面的评估方法比整体任务评估更能揭示自动标注的实际能力。
...
Method: 引入流式探测目标，要求多个证据令牌一致支持预测，基于聚合信号而非单令牌线索进行检测；比较注意力、MLP激活和残差流特征的性能。

Result: 在1%误报率下，真阳性率相对基线提升35.55%；AUROC显著提升（基线已达97.40%）；注意力或MLP激活探测优于残差流特征；对抗微调后的字符级密码攻击仍可检测（AUROC > 98.85%）。

Conclusion: 基于多令牌证据聚合的流式探测方法能显著提升有害内容检测的鲁棒性，对新型字符级密码攻击具有"即插即用"的检测能力。
...
TL;DR: 该研究分析了18个视觉语言模型的推理动态，发现模型存在"答案惯性"问题，早期预测在推理过程中被强化而非修正。虽然推理训练模型表现出更强的纠正行为，但其效果受模态条件影响。模型容易受误导性文本线索影响，即使视觉证据充分，且思维链仅能部分揭示模态依赖。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型展现出推理能力，但其推理过程如何整合视觉和文本信息仍不清楚。研究者希望理解VLMs的推理动态，特别是思维链如何反映模型对多模态信息的依赖和整合过程。

Method: 研究分析了18个VLMs，涵盖指令微调和推理训练模型，来自两个不同的模型家族。方法包括：跟踪思维链过程中的置信度变化、测量推理的纠正效果、评估中间推理步骤的贡献、使用误导性文本线索进行受控干预、分析模型对线索的显式提及和模态依赖。

Result: 1. 模型存在"答案惯性"：早期预测在推理中被强化而非修正；2. 推理训练模型有更强的纠正行为，但效果受模态条件影响；3. 模型易受误导性文本线索影响，即使视觉证据充分；4. 思维链仅能部分揭示模态依赖：推理训练模型更可能显式提及线索，但其流畅的长思维链可能掩盖实际对文本线索的依赖；指令微调模型较少显式提及线索，但其短思维链能揭示与视觉输入的不一致。

Conclusion: 思维链仅提供了视觉语言模型决策中多模态驱动因素的局部视图，这对多模态系统的透明度和安全性有重要影响。研究揭示了VLMs推理过程中的局限性，特别是模型对文本线索的过度依赖以及思维链在揭示真实推理过程方面的不足。
...
### [51] [IE as Cache: Information Extraction Enhanced Agentic Reasoning](https://arxiv.org/abs/2604.14930)
...
<details>
  <summary>Details</summary>
Motivation: 传统信息抽取仅作为终端目标，抽取结果孤立使用而非在多步推理中维护和重用。需要超越这种局限，将IE重新定位为可重用的认知资源
...
### [52] [XQ-MEval: A Dataset with Cross-lingual Parallel Quality for Benchmarking Translation Metrics](https://arxiv.org/abs/2604.14934)
*Jingxuan Liu,Zhi Qu,Jin Tei,Hidetaka Kamigaito,Lemao Liu,Taro Watanabe*

Main category: cs.CL

TL;DR: XQ-MEval是一个半自动构建的数据集，用于评估多语言翻译指标，通过注入MQM错误创建可控质量的伪翻译，揭示跨语言评分偏差并提出了归一化策略。


<details>
  <summary>Details</summary>
Motivation: 当前多语言翻译系统评估通常采用跨语言平均指标分数，但这种方法存在可疑之处，因为指标可能存在跨语言评分偏差——同等质量的翻译在不同语言中可能获得不同分数。由于缺乏提供跨语言平行质量实例的基准数据集，且专家标注不现实，这一问题尚未得到系统研究。

Method: 提出XQ-MEval数据集构建方法：1）将MQM定义的错误自动注入黄金翻译中；2）由母语者筛选以确保可靠性；3）合并错误生成具有可控质量的伪翻译；4）将伪翻译与对应的源文本和参考翻译组成三元组用于评估翻译指标。覆盖9个翻译方向，实验评估了9个代表性指标。

Result: 使用XQ-MEval进行的实验揭示了平均分数与人工判断之间的不一致性，首次提供了跨语言评分偏差的经验证据。研究发现不同语言间指标评分分布存在系统性差异，同等质量的翻译在不同语言中可能获得显著不同的分数。

Conclusion: 基于XQ-MEval提出了一个归一化策略，通过调整不同语言的分数分布，使其在跨语言间对齐，从而提高了多语言指标评估的公平性和可靠性。XQ-MEval为系统研究翻译指标的跨语言偏差提供了首个基准数据集。
...
### [53] [Text2Arch: A Dataset for Generating Scientific Architecture Diagrams from Natural Language Descriptions](https://arxiv.org/abs/2604.14941)
*Shivank Garg,Sankalp Mittal,Manish Gupta*

Main category: cs.CL

TL;DR: 提出一个自动从文本生成科学架构图的系统，通过创建新数据集和微调语言模型，性能优于现有基线模型


<details>
  <summary>Details</summary>
Motivation: 仅通过文本传达复杂系统设计或科学过程效率低下且容易产生歧义，需要自动从文本生成高语义保真度科学架构图的系统
...
Conclusion: 通过创建数据集和微调语言模型，成功开发了能够从文本自动生成高保真度科学架构图的系统，并公开了代码、数据和模型
...
### [54] [Explain the Flag: Contextualizing Hate Speech Beyond Censorship](https://arxiv.org/abs/2604.14970)
...
Method: 开发混合方法：结合LLM与三个新创建和整理的词汇表。两个互补管道：1) 使用定制词汇表检测和消歧问题术语；2) 利用LLM作为上下文感知的群体目标内容评估器。输出融合为有依据的解释。

Result: 人类评估显示混合方法准确，解释质量高，优于纯LLM基线。系统能有效检测与身份特征相关的固有贬义表达和直接群体目标内容。

Conclusion: 混合方法通过结合词汇表与LLM，实现了更透明、可问责的仇恨言论检测与解释，为多语言仇恨言论检测提供了有效解决方案。
...
### [55] [IUQ: Interrogative Uncertainty Quantification for Long-Form Large Language Model Generation](https://arxiv.org/abs/2604.15109)
*Haozhi Fan,Jinhao Duan,Kaidi Xu*

Main category: cs.CL

TL;DR: 本文提出IUQ框架，通过样本间一致性和样本内忠实性来量化长文本生成中的不确定性，解决了LLM生成语义连贯但事实不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs快速发展，但长文本生成中的不确定性量化仍面临挑战。现有方法通常限制LLM生成简短或受限答案，而实际应用需要长文本和自由文本生成。主要困难在于LLM常生成语义连贯但事实不准确的响应，同时底层语义多面且语言结构复杂。

Method: 提出Interrogative Uncertainty Quantification (IUQ)框架，利用"询问-回答"范式，通过样本间一致性和样本内忠实性来量化长文本LLM输出的不确定性。该方法能可靠测量声明级不确定性和模型的忠实性。

Result: 在不同模型家族和模型规模上的实验结果表明，IUQ在两个广泛使用的长文本生成数据集上表现出优越性能。

Conclusion: IUQ框架有效解决了长文本LLM生成中的不确定性量化问题，通过结合样本间一致性和样本内忠实性，为长文本生成提供了可靠的不确定性测量方法。
...
TL;DR: DiscoTrace是一种分析回答者回答信息寻求问题时使用的修辞策略的方法，通过标注问题相关话语行为和问题解释来揭示不同人类社区的回答偏好，发现LLMs缺乏修辞多样性且倾向于广度优先回答。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别回答者在回应信息寻求问题时使用的修辞策略，理解不同人类社区如何构建答案，并分析LLMs在回答策略上与人类社区的差异。

Method: 提出DiscoTrace方法，将答案表示为问题相关话语行为序列，结合原始问题的解释，基于修辞结构理论解析进行标注。应用于九个不同人类社区的答案进行分析。

Result: 发现不同人类社区在答案构建上有显著偏好差异；LLMs缺乏修辞多样性，即使被提示模仿特定人类社区回答指南；LLMs系统性地选择广度优先策略，回答人类回答者选择不涉及的问题解释。

Conclusion: 研究结果可指导开发考虑上下文、基于多种策略的实用LLM回答者，使LLMs能够根据具体情况采用不同的修辞策略。
...
TL;DR: 提出K-Token Merging框架，通过轻量级编码器将连续K个token嵌入合并为单个嵌入，在潜在嵌入空间压缩长提示，实现高达75%的输入长度缩减且性能损失最小。
...
Method: 提出K-Token Merging潜在空间压缩框架：使用轻量级编码器将每个连续K个token嵌入块合并为单个嵌入，压缩序列由LoRA适配的LLM处理，生成仍使用原始词汇表。

Result: 在结构推理(Textualized Tree)、情感分类(Amazon Reviews)和代码编辑(CommitPackFT)任务上的实验表明，K-Token Merging在性能与压缩比之间达到帕累托前沿，实现高达75%的输入长度缩减且性能退化最小。
...
TL;DR: 该论文研究了大型语言模型在机器翻译中的过度生成现象，探讨了其检测方法和商业应用策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在机器翻译中表现出色，但其生成性质可能导致各种过度生成现象，这些现象与传统神经机器翻译的"神经胡言"不同，包括LLM自我解释、风险性虚构和适当解释等。准确检测和确定过度生成的具体性质具有挑战性，需要在商业环境中探索有效策略。

Method: 论文详细探讨了在商业环境中探索的不同策略，包括检测和确定LLM翻译中过度生成现象的方法，但没有具体说明具体的技术方法。

Result: 论文展示了在商业环境中应用不同策略的结果，表明LLM能够像人类翻译者一样提供解释，增强目标受众的理解能力。

Conclusion: LLM在机器翻译中表现出色，但其过度生成现象需要仔细检测和管理。在商业环境中探索的策略为处理这些现象提供了实用方法，使LLM能够更好地模拟人类翻译行为，提高翻译质量。
...
### [59] [MADE: A Living Benchmark for Multi-Label Text Classification with Uncertainty Quantification of Medical Device Adverse Events](https://arxiv.org/abs/2604.15203)
*Raunak Agarwal,Markus Wenzel,Simon Baur,Jonas Zimmer,George Harvey,Jackie Ma*

Main category: cs.CL

TL;DR: MADE是一个基于医疗器械不良事件报告构建的持续更新的多标签文本分类基准，用于评估模型预测性能和不确定性量化能力，防止训练数据污染。


<details>
  <summary>Details</summary>
Motivation: 在医疗等高风险领域，机器学习不仅需要强大的预测性能，还需要可靠的不确定性量化来支持人类监督。现有多标签文本分类基准存在数据饱和和训练数据污染问题，难以区分真正的推理能力和记忆能力。

Method: 构建MADE基准，基于医疗器械不良事件报告，具有分层标签的长尾分布，采用严格的时间分割确保可重复评估。评估了20多个编码器和解码器模型在微调和少样本设置下的表现，系统评估了基于熵/一致性的不确定性量化方法和自我表达方法。

Result: 结果显示明显权衡：小型判别式微调解码器在头到尾准确率上表现最强，同时保持竞争力的不确定性量化；生成式微调提供最可靠的不确定性量化；大型推理模型在稀有标签上表现更好但不确定性量化较弱；自我表达的置信度不能可靠替代不确定性。

Conclusion: MADE基准为医疗领域多标签文本分类提供了可靠的评估框架，揭示了不同模型架构和训练策略在预测性能和不确定性量化之间的权衡，强调了在高风险应用中需要同时优化这两个方面。
...
Method: 提出SpecGuard框架：1）采样多个草稿候选并选择最一致的步骤；2）使用两个轻量级模型内部信号进行验证：基于注意力的基础评分和基于对数概率的置信度评分；3）联合判断是否接受步骤或使用目标模型重新计算

Result: 在多个推理基准测试中，SpecGuard将准确率提高3.6%，同时降低约11%的延迟，优于传统推测解码和奖励引导的推测解码

Conclusion: SpecGuard通过模型内部信号实现有效的步骤级验证，在保持计算效率的同时显著提升推测解码的准确性和延迟性能
...
### [61] [Knowledge Graph RAG: Agentic Crawling and Graph Construction in Enterprise Documents](https://arxiv.org/abs/2604.14220)
*Koushik Chakraborty,Koyel Guha*

Main category: cs.IR

TL;DR: 提出基于递归爬取的Agentic知识图谱方法，显著提升复杂企业文档语义搜索准确性
...
Method: 提出Agentic知识图谱结合递归爬取技术，用于导航替代逻辑和多跳引用，增强复杂文档的语义搜索能力

Result: 在《联邦法规法典》基准评估中，知识图谱增强方法相比标准向量RAG系统实现了70%的准确率提升

Conclusion: Agentic知识图谱结合递归爬取为复杂监管查询提供了全面精确的答案，显著优于传统语义搜索方法
...
### [62] [Adaptive Query Routing: A Tier-Based Framework for Hybrid Retrieval Across Financial, Legal, and Medical Documents](https://arxiv.org/abs/2604.14222)
*Afshan Hashmi*

Main category: cs.IR

TL;DR: 本文系统比较了三种检索增强生成架构（向量RAG、树推理和自适应混合检索）在金融、法律和医疗领域的表现，发现树推理在整体上表现最佳，但不同架构在不同查询复杂度层级上各有优势，支持开发基于查询复杂度和文档结构动态选择策略的自适应检索系统。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示向量RAG在金融文档QA中表现良好，而纯推理检索系统在特定基准上达到高准确率。然而，不同检索架构在不同查询类型和领域中的相对性能尚未得到系统评估。本研究旨在填补这一空白，探索不同检索范式在不同复杂度和领域中的表现差异。

Method: 1. 实现并评估三种检索架构：向量RAG、树推理和提出的自适应混合检索；2. 引入四层查询复杂度基准（从简单事实到多文档合成）；3. 使用GPT-4驱动的LLM-as-judge评估方法；4. 在金融、法律和医疗领域进行实验；5. 在FinanceBench（150个专家标注的SEC 10-K和10-Q文件问题）上进行验证。

Result: 树推理获得最高整体得分（0.900），但没有单一范式在所有层级上占优：向量RAG在多文档合成（Tier 4，得分0.900）上表现最佳，而混合AHR在交叉引用（0.850）和多章节查询（0.929）上表现最好。树基和混合方法的交叉引用召回率达到100%，而向量搜索为91.7%。在FinanceBench验证中：树推理0.938，混合AHR 0.901，向量RAG 0.821，树-向量质量差距扩大到11.7个百分点。

Conclusion: 不同检索架构在不同查询复杂度和文档结构下表现各异，支持开发能够基于查询特征和文档结构动态选择策略的自适应检索系统。树推理在整体性能上领先，但向量RAG在多文档合成方面有优势，混合方法在复杂查询中表现均衡。这些发现为构建更智能的RAG系统提供了实证基础。
...
### [63] [TRACE: A Conversational Framework for Sustainable Tourism Recommendation with Agentic Counterfactual Explanations](https://arxiv.org/abs/2604.14223)
*Ashmi Banerjee,Adithi Satish,Wolfgang Wörndl,Yashar Deldjoo*

Main category: cs.IR

TL;DR: TRACE是一个基于LLM的多智能体对话式旅游推荐框架，通过交互式引导促进可持续旅游，平衡用户相关性与环境影响，避免强化过度拥挤和高碳足迹目的地。
...
Method: 采用模块化的编排器-工作者架构，专门智能体负责：1) 引出潜在可持续性偏好；2) 构建结构化用户画像；3) 生成平衡相关性与环境影响的推荐。关键创新包括智能体反事实解释和LLM驱动的澄清问题，共同发现更环保的替代方案并精炼意图理解。

Result: 用户研究和语义对齐分析表明，TRACE有效支持可持续决策制定，同时保持推荐质量和交互响应性。框架已在Google Agent Development Kit上实现，提供完整代码、Docker设置、提示词和公开演示视频确保可复现性。

Conclusion: TRACE通过多智能体LLM框架和交互式引导机制，成功将可持续性考量整合到对话式旅游推荐中，在不强制用户的情况下促进环保旅行选择，为可持续旅游推荐系统提供了可行的技术方案。
...
### [64] [FRESCO: Benchmarking and Optimizing Re-rankers for Evolving Semantic Conflict in Retrieval-Augmented Generation](https://arxiv.org/abs/2604.14227)
*Sohyun An,Hayeon Lee,Shuibenyang Yuan,Chun-cheng Jason Chen,Cho-Jui Hsieh,Vijai Mohan,Alexander Min*

Main category: cs.IR

TL;DR: FRESCO基准测试评估RAG中重排序器在动态信息环境下的表现，发现现有重排序器偏向旧文档，提出指令优化框架改善性能


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要在静态环境下评估重排序器，无法充分评估其在信息演化场景下的性能，而现实系统经常需要在时间不同的证据中进行选择

Method: 引入FRESCO基准测试，将时效性查询与历史维基百科修订配对，测试重排序器能否优先选择事实性最新的证据同时保持语义相关性；进一步研究指令优化框架，通过识别帕累托最优指令来平衡演化知识和非演化知识任务

Result: 评估显示现有重排序器存在一致的失败模式：强烈偏向较旧、语义丰富的文档，即使这些文档事实已过时；指令优化框架在演化知识任务上获得高达27%的性能提升，同时在非演化知识任务上保持竞争力

Conclusion: FRESCO基准测试填补了动态信息环境下重排序器评估的空白，揭示了现有方法的局限性，提出的指令优化框架能有效改善重排序器在时效性信息处理方面的性能
...
### [65] [Controlling Authority Retrieval: A Missing Retrieval Objective for Authority-Governed Knowledge](https://arxiv.org/abs/2604.14488)
*Andre Bacellar*

Main category: cs.IR

TL;DR: 提出控制权威检索（CAR）问题，解决在正式权威领域（法律、药品监管、软件安全）中检索有效前沿文档的挑战，区别于传统语义相似度检索。


<details>
  <summary>Details</summary>
Motivation: 在正式权威领域，后续文档可以正式废止先前文档，但语义上可能相距甚远。传统基于语义相似度的检索无法识别这种权威控制关系，导致检索结果包含已废止的无效文档。

Method: 形式化控制权威检索问题，提出定理4（CAR正确性特征）给出检索集满足TCA=1的充要条件，以及命题2（范围可识别性上界）建立phi(q)作为最坏情况上限。在三个真实世界语料库（安全公告、最高法院判例、FDA药品记录）上验证，并开发两阶段检索方法。

Result: 在三个语料库上，密集检索的TCA@5分别为0.270、0.172、0.064，而两阶段方法分别达到0.975、0.926、0.774。GPT-4o-mini实验显示，密集RAG在39%存在补丁的查询中产生"未修补"错误声明，两阶段方法降至16%。

Conclusion: 控制权威检索是一个与语义相似度检索不同的数学问题，需要专门方法。提出的理论框架和两阶段方法显著提高了检索有效性，减少了错误声明。发布了四个基准数据集、领域适配器和单命令评分器。
...
### [66] [NewsTorch: A PyTorch-based Toolkit for Learner-oriented News Recommendation](https://arxiv.org/abs/2604.14510)
*Rongyao Wang,Veronica Liesaputra,Zhiyi Huang*

Main category: cs.IR

TL;DR: NewsTorch：一个基于PyTorch的新闻推荐系统工具包，提供模块化、可扩展的框架和用户友好的GUI平台，支持数据集处理、模型训练和标准化评估。


<details>
  <summary>Details</summary>
Motivation: 新闻推荐系统旨在缓解信息过载问题，但缺乏专门面向学习者的工具包阻碍了该领域的研究进展。现有工具在易用性、模块化和可扩展性方面存在不足。

Method: 开发基于PyTorch的新闻推荐工具包NewsTorch，采用模块化、解耦和可扩展的架构设计，提供学习者友好的GUI平台，支持数据集下载与预处理、最先进的神经新闻推荐模型训练验证测试，以及标准化评估指标。

Result: 成功开发并开源了NewsTorch工具包，提供了完整的新闻推荐研究框架，确保公平比较和可复现实验，已在GitHub上发布。

Conclusion: NewsTorch填补了新闻推荐领域缺乏专门学习者工具包的空白，为研究人员提供了易用、模块化且可扩展的平台，有助于推动新闻推荐系统的研究进展。
...
### [67] [CPGRec+: A Balance-oriented Framework for Personalized Video Game Recommendations](https://arxiv.org/abs/2604.14586)
*Xiping Li,Aier Yang,Jianghong Ma,Kangzhe Liu,Shanshan Feng,Haijun Zhang,Yi Zhao*

Main category: cs.IR

TL;DR: CPGRec+通过偏好感知的边重加权和LLM驱动的表示生成模块，解决了游戏推荐中准确性与多样性的权衡问题，同时缓解了GNN的过平滑问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的游戏推荐系统主要关注准确性而忽视多样性，且未考虑玩家-游戏交互的重要性差异，这些差异反映了玩家的个人偏好差异。此外，现有方法未能充分利用大语言模型（LLMs）的推理能力和广泛知识来解决这些限制。

Method: 提出两个新模块：1）偏好感知边重加权（PER）模块，为边分配带符号的权重以区分重要兴趣和厌恶，并定量衡量偏好强度以缓解图卷积中的过平滑问题；2）偏好感知表示生成（PRG）模块，利用LLMs通过比较全局和个人兴趣来推理个人偏好，生成游戏和玩家的上下文描述，从而精炼表示。

Result: 在两个Steam数据集上的实验表明，CPGRec+在准确性和多样性方面均优于最先进的模型。

Conclusion: CPGRec+通过结合偏好感知的图重加权和LLM驱动的表示生成，有效平衡了游戏推荐的准确性与多样性，同时缓解了GNN的过平滑问题，为动态游戏环境提供了更先进的推荐系统。
...
### [68] [Category-based and Popularity-guided Video Game Recommendation: A Balance-oriented Framework](https://arxiv.org/abs/2604.14598)
*Xiping Li,Jianghong Ma,Kangzhe Liu,Shanshan Feng,Haijun Zhang,Yutong Wang*

Main category: cs.IR

TL;DR: CPGRec是一个新颖的视频游戏推荐框架，通过三个模块（准确性驱动、多样性驱动和综合模块）平衡推荐准确性和多样性，解决现有方法偏重准确性而忽视多样性的问题。


<details>
  <summary>Details</summary>
Motivation: 视频游戏行业快速增长，玩家面临大量游戏选择，需要专门的推荐系统。现有方法偏重准确性而忽视多样性，导致推荐单调；同时缺乏游戏间的严格连接来提升准确性；许多多样性方法在邻居建模和消息传播中未能利用关键信息（如游戏类别和流行度）。

Method: 提出CPGRec框架，包含三个模块：1) 准确性驱动模块：通过更严格的游戏连接增强最先进的准确性方法；2) 多样性驱动模块：在游戏图中连接不同类别的邻居，利用流行游戏节点放大长尾游戏影响力；3) 综合模块：结合前两个模块，使用新的负样本评分重加权方法平衡准确性和多样性。

Result: 在Steam数据集上的实验结果表明，该方法能有效改进游戏推荐效果。数据集和源代码已匿名发布。

Conclusion: CPGRec框架成功解决了视频游戏推荐中准确性与多样性的平衡问题，通过创新的模块设计和信息利用策略提升了推荐系统的综合性能。
...
### [69] [Uncertainty-aware Generative Learning Path Recommendation with Cognition-Adaptive Diffusion](https://arxiv.org/abs/2604.14613)
...
Method: U-GLAD框架包含三个核心组件：1）高斯LSTM将学习者的认知状态建模为概率分布，捕捉潜在真实状态；2）目标导向概念编码器使用多头注意力和目标特定变换，动态对齐概念语义与个体学习目标；3）生成式扩散模型预测下一个最优概念的潜在表示，而非传统判别式排序方法。

Result: 在三个公开数据集上的广泛评估表明，U-GLAD显著优于代表性基线方法。进一步分析证实了其在感知交互不确定性和提供稳定、目标驱动的推荐路径方面的卓越能力。

Conclusion: U-GLAD通过建模认知状态不确定性、目标自适应概念编码和生成式扩散预测，实现了更准确、个性化的学习路径推荐，解决了现有方法在不确定性和目标适应性方面的局限性。
...
### [70] [Federated User Behavior Modeling for Privacy-Preserving LLM Recommendation](https://arxiv.org/abs/2604.14833)
*Lei Guo,Hongyun Yang,Pengjie Ren,Tong Chen,Hui Liu,Zhumin Chen*

Main category: cs.IR

TL;DR: SF-UBM：一种语义增强的联邦用户行为建模方法，用于解决非重叠隐私保护跨域推荐中的三大挑战，通过自然语言作为通用桥梁连接不同领域。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推荐系统面临用户数据稀疏问题，跨域推荐方法通常假设领域重叠且可访问，但缺乏对隐私保护跨域推荐（PPCDR）的关注，特别是在非重叠领域设置下存在三大挑战：跨域对齐困难、数据模态异质性、协同过滤信号与LLM特征空间不匹配。

Method: 提出SF-UBM方法：1）使用自然语言作为通用桥梁，通过语义增强的联邦架构连接不相交领域，加密共享基于文本的物品表示，用户数据本地保留；2）设计事实-反事实知识蒸馏模块，整合领域无关与领域特定知识；3）将预学习的用户偏好和跨域物品表示投影到软提示空间，对齐行为和语义空间。

Result: 在三个真实世界领域对上进行了广泛实验，实验结果表明SF-UBM相比最近的SOTA方法具有显著有效性。

Conclusion: SF-UBM成功解决了非重叠隐私保护跨域推荐中的三大挑战，通过语义增强的联邦架构、知识蒸馏和空间对齐技术，实现了有效的跨域推荐，同时保护用户隐私。
...
### [71] [Well Begun is Half Done: Training-Free and Model-Agnostic Semantically Guaranteed User Representation Initialization for Multimodal Recommendation](https://arxiv.org/abs/2604.14839)
*Jinfeng Xu,Zheyu Chen,Shuo Yang,Jinze Li,Hewei Wang,Jianheng Tang,Wei Wang,Xiping Hu,Edith C. H. Ngai*

Main category: cs.IR

TL;DR: 提出SG-URInit方法，通过整合用户交互物品的模态特征和对应聚类的全局特征，为多模态推荐中的用户表示提供语义丰富的初始化，以弥补用户和物品表示之间的语义鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推荐系统忽视用户表示初始化的重要性。物品有丰富的模态信息可用于初始化，而用户缺乏固有模态信息，导致随机初始化的用户表示与基于模态信息初始化的物品表示之间存在显著语义鸿沟，影响推荐性能。

Method: 提出SG-URInit方法：1) 整合用户交互物品的模态特征（局部语义）；2) 整合用户对应聚类的全局特征（全局语义）；3) 训练无关且模型无关，可直接集成到现有多模态推荐模型中，不增加训练计算开销。

Result: 在多个真实数据集上的实验表明：1) 将SG-URInit集成到先进多模态推荐模型中能显著提升推荐性能；2) 能进一步缓解物品冷启动问题；3) 能加速模型收敛。

Conclusion: SG-URInit通过语义保证的用户表示初始化，有效弥补了多模态推荐中用户和物品表示之间的语义鸿沟，是一种高效实用的解决方案，能提升推荐性能、缓解冷启动问题并加速收敛。
...
### [72] [GenRec: A Preference-Oriented Generative Framework for Large-Scale Recommendation](https://arxiv.org/abs/2604.14878)
*Yanyan Zou,Junbo Qi,Lunsong Huang,Yu Li,Kewei Xu,Jiabao Gao,Binglei Zhao,Xuanhua Yang,Sulong Xu,Shengjie Li*

Main category: cs.IR

TL;DR: GenRec是一个部署在京东App上的偏好导向生成式推荐框架，通过页面级NTP任务、非对称线性Token合并器和GRPO-SR强化学习方法，解决了生成式检索在工业规模部署中的三大挑战，实现了显著的在线效果提升。


<details>
  <summary>Details</summary>
Motivation: 解决生成式检索在大规模工业推荐系统中面临的三大挑战：1) 分页请求机制下相同模型输入产生不一致输出；2) 基于语义ID的多token物品表示编码长用户行为序列的成本过高；3) 生成策略与细微用户偏好信号的难以对齐。

Method: 1) 页面级NTP训练任务：对整个交互页面而非单个物品进行监督，提供更密集的梯度信号并解决点式训练的一对多歧义；2) 非对称线性Token合并器：在提示中压缩多token语义ID同时保持全分辨率解码，将输入长度减少约2倍；3) GRPO-SR强化学习方法：结合组相对策略优化与NLL正则化确保训练稳定性，采用混合奖励（密集奖励模型+相关性门控）防止奖励攻击。

Result: 在为期一个月的在线A/B测试中，GenRec相比现有流水线实现了点击量提升9.5%，交易量提升8.7%的显著改进。

Conclusion: GenRec通过统一的仅解码器架构有效解决了生成式检索在工业规模部署中的关键挑战，证明了页面级监督、高效序列压缩和偏好对齐强化学习方法的有效性，为大规模推荐系统提供了可行的生成式检索解决方案。
...
### [73] [Metric-agnostic Learning-to-Rank via Boosting and Rank Approximation](https://arxiv.org/abs/2604.15101)
...
<details>
  <summary>Details</summary>
Motivation: 当前学习排序方法依赖单一前缀排序指标（如NDCG或MAP），存在两个主要问题：1）非可微问题：直接在给定排序指标上优化排序函数本质上是非平滑的，导致训练过程不稳定且低效；2）排序效用有限：基于单一指标优化难以泛化到其他感兴趣的排序指标。

Method: 提出一种新的列表式学习排序框架：1）设计新的可微排序损失函数，结合排序算子的平滑近似和每个查询的平均均方损失；2）采用梯度提升机来最小化针对每个列表提出的损失函数。

Result: 大量实验结果表明，该方法在信息检索指标上优于当前最先进的方法，同时保持了相似的效率。

Conclusion: 提出的框架通过可微排序损失函数解决了传统学习排序方法的局限性，在保持效率的同时提高了排序性能和泛化能力。
...
### [74] [Simulating Human Cognition: Heartbeat-Driven Autonomous Thinking Activity Scheduling for LLM-based AI systems](https://arxiv.org/abs/2604.14178)
*Hong Su*

Main category: cs.AI

TL;DR: 本文提出心跳驱动的自主思维活动调度机制，通过周期性"心跳"协调动态认知模块，实现主动、自适应、持续自我调节的LLM智能体控制流。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体通常采用僵化的反应式控制流程，依赖固定管道或失败触发反思，导致智能体行为冲动或仅在错误发生后修正，限制了适应性和效率。

Method: 提出心跳驱动的自主思维活动调度机制，采用周期性"心跳"协调动态认知模块（如规划器、批评器、回忆器、梦想器）。提出元学习策略进行持续策略适应，调度器利用历史交互日志随时间优化认知策略。

Result: 评估结果表明，该方法能有效基于历史数据学习调度认知活动，并能自主集成新的思维模块。

Conclusion: 心跳驱动的自主思维活动调度机制为LLM智能体提供了更灵活、自适应和持续自我调节的控制流框架，克服了传统方法的局限性。
...
### [75] [Fun-TSG: A Function-Driven Multivariate Time Series Generator with Variable-Level Anomaly Labeling](https://arxiv.org/abs/2604.14221)
...
<details>
  <summary>Details</summary>
Motivation: 当前多元时间序列异常检测评估面临挑战，现有基准数据集存在以下问题：缺乏细粒度异常标注、不提供明确的变量间和时序依赖关系、对底层生成机制缺乏洞察。这些缺陷阻碍了检测模型的开发和严格比较，特别是针对可解释和变量特定输出的模型。

Method: 引入Fun-TSG时间序列生成器，支持两种生成模式：1）基于随机采样的依赖结构和异常类型的全自动生成；2）通过用户定义方程和异常配置的手动生成。该工具提供数据生成过程的完全透明性，包括变量和时间戳级别的真实异常标签访问。

Result: Fun-TSG能够创建多样化、可解释且可复现的基准测试场景，支持对经典和现代异常检测模型进行细粒度性能分析，为异常检测方法评估提供了高质量的数据生成工具。

Conclusion: Fun-TSG解决了多元时间序列异常检测评估中的关键限制，通过提供完全可定制、透明且具有细粒度标注的数据生成能力，促进了异常检测模型的开发和严格比较。
...
### [76] [Formalizing Kantian Ethics: Formula of the Universal Law Logic (FULL)](https://arxiv.org/abs/2604.14254)
*Taylor Olson*

Main category: cs.AI

TL;DR: 论文提出FULL逻辑系统，基于康德伦理学形式化道德推理，旨在解决现有人工道德智能体依赖预设道德公理的两个局限性。


<details>
  <summary>Details</summary>
Motivation: 当前人工道德智能体(AMAs)主要依赖编码人类道德直觉作为行动公理（如"不伤害"、"帮助他人"），这存在两个根本限制：1) 不考虑行动者的目的；2) 假设人类能够穷举所有道德直觉。论文旨在探索形式化道德程序来缓解这些限制。

Method: 采用康德伦理学框架，提出多类量化模态逻辑系统FULL（Formula of the Universal Law Logic）。该系统形式化了康德的第一条绝对命令——普遍法则公式(FUL)，以及因果关系、能动性等概念。通过三个康德伦理学案例展示FULL的推理能力。

Result: FULL系统能够在没有内置道德直觉的情况下，基于充分的非规范性背景知识，评估具有特定目的的智能体行动。在三个康德伦理学案例中成功展示了其推理能力。

Conclusion: FULL逻辑系统是朝着更稳健、更自主的人工道德智能体迈出的重要一步，同时也为康德伦理学提供了更形式化的理解框架，能够克服传统基于公理方法的局限性。
...
### [77] [GFT: From Imitation to Reward Fine-Tuning with Unbiased Group Advantages and Dynamic Coefficient Rectification](https://arxiv.org/abs/2604.14258)
*Wangjie Gan,Miao Pan,Linbo Xi,Wenqi Zhang,Jintao Chen,Jianwei Yin,Xuhong Zhang*

Main category: cs.AI

TL;DR: 提出Group Fine-Tuning (GFT)框架，通过组优势学习和动态系数校正解决SFT的稀疏奖励和优化不稳定问题，统一知识注入与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统SFT和RL训练难以有效统一高效知识注入与鲁棒泛化。分析发现SFT本质上是具有极端稀疏隐式奖励和不稳定逆概率加权的策略梯度优化，导致单一路径依赖、熵崩溃和梯度爆炸。
...
Conclusion: GFT通过诊断SFT内在局限性并提出针对性机制，实现了知识注入与泛化能力的统一，为语言模型后训练提供了更优框架。
...
### [78] [Seeing Through Experts Eyes A Foundational Vision Language Model Trained on Radiologists Gaze and Reasoning](https://arxiv.org/abs/2604.14316)
*Kinhei Lee,Peiyuan Jing,Zhenxuan Zhang,Yue Yang,Tao Wang,Dominic C Marshall,Yingying Fang,Guang Yang*

Main category: cs.AI

TL;DR: GazeX：利用放射科医生眼动追踪数据作为行为先验的视觉语言模型，通过模拟专家诊断推理过程，在胸部X光解读任务中实现更准确、可解释且与专家一致的输出。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视觉语言模型在胸部X光解读中存在局限性，模型输出与放射科医生推理过程存在差距。大多数系统仅优化语义信息，未能模拟专家如何视觉检查医学图像，常常忽略关键发现或偏离既定诊断流程。放射科医生遵循结构化协议（如ABCDEF方法）确保系统检查所有临床相关区域，减少遗漏发现并支持可靠的诊断推理。

Method: 引入GazeX视觉语言模型，利用放射科医生眼动追踪数据作为行为先验来建模专家诊断推理。通过将注视轨迹和固定模式纳入预训练，GazeX学习遵循放射科医生注意力的空间和时间结构，并以临床有意义的序列整合观察结果。使用来自5名放射科医生的超过30,000个注视关键帧的精选数据集，结合231,835个放射学研究、780,014个问答对和1,162个带有边界框的图像-句子对进行训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法依赖轨迹级奖励，无法区分精确搜索查询与模糊/冗余查询，且在采样轨迹全部失败时梯度信号接近零。需要更细粒度的信用分配机制来提升搜索增强推理效果。

Method: 提出IG-Search框架，引入基于信息增益的步骤级奖励。每个搜索步骤中，IG衡量检索文档相对于随机文档基线对模型在正确答案上置信度的提升程度。通过GRPO中的每词优势调制将信号反馈给对应的搜索查询词元，实现细粒度信用分配。无需外部中间监督或跨轨迹共享环境状态，仅需标准问答对。

Result: 在7个单跳和多跳QA基准测试中，使用Qwen2.5-3B模型实现平均EM 0.430，比最强轨迹级基线(MR-Search)提升1.6个百分点，比步骤级方法GiGPO提升0.9个百分点。多跳推理任务提升尤为显著。训练时间仅增加约6.4%，推理延迟不变，即使在所有采样轨迹都回答错误时仍能提供有意义的梯度信号。

Conclusion: IG-Search通过信息增益步骤级奖励有效解决了轨迹级强化学习的信用分配问题，显著提升了搜索增强推理性能，特别是在复杂多跳任务上，且计算开销可控。
...
### [80] [Equifinality in Mixture of Experts: Routing Topology Does Not Determine Language Modeling Quality](https://arxiv.org/abs/2604.14419)
*Ivan Ternovtsii,Yurii Bilak*

Main category: cs.AI

TL;DR: 研究发现路由拓扑结构并不决定语言模型的渐近困惑度，五种余弦路由变体在1-PPL范围内统计等价，单标量可复现多跳性能，零射相对范数停止可节省25% MoE FLOPs。


<details>
  <summary>Details</summary>
Motivation: 研究稀疏混合专家架构中日益复杂的路由机制（学习路由器、多跳轨迹、令牌依赖门控）是否真正决定语言建模质量，探索路由拓扑对性能的实际影响。

Method: 构建几何MoE（ST-MoE），使用余弦相似度路由在低维空间（d_space=64）中针对学习到的质心进行路由，比标准线性路由器减少80%路由参数。在WikiText-103上进行62个控制实验（76-84M参数，50K步，1.64B令牌），使用五种余弦路由变体，并通过Two One-Sided Tests进行统计比较。

Result: 路由拓扑不决定渐近困惑度：五种余弦路由变体在1-PPL范围内统计等价（TOST p<0.05，10对比较，15次运行，范围33.93-34.72）。该发现扩展到哈希、随机固定和top-1路由（单种子，优雅退化1.1-2.2 PPL）。标准线性路由器（5.3倍参数）达到PPL 32.76，但等参数余弦路由缩小了67%差距。机制解释为收敛冗余：多跳更新是共线的（cos(Δh0, Δh1)=0.805），实现幅度放大而非组合推理。

Conclusion: 路由拓扑结构对语言建模质量影响有限，复杂路由机制主要实现幅度放大而非组合推理。零射相对范数停止可节省25% MoE FLOPs且性能损失极小（+0.12% PPL）。专家级专业化和因果可控性与拓扑级等终性共存。
...
### [81] [Demonstration of Pneuma-Seeker: Agentic System for Reifying and Fulfilling Information Needs on Tabular Data](https://arxiv.org/abs/2604.14422)
...
<details>
  <summary>Details</summary>
Motivation: 数据分析师在处理关系型数据时，通常从模糊或不完整的问题开始，需要迭代式探索和细化。现有工具缺乏对信息需求进行明确、可检查规范化的支持，难以实现目标导向的数据发现和溯源感知执行。

Method: 开发Pneuma-Seeker系统，将用户信息需求具体化为明确、可检查的关系型规范。系统支持信息需求的迭代式细化、目标导向的数据发现以及溯源感知执行。利用大型语言模型作为透明、交互式的分析协作工具，而非黑盒答案引擎。

Result: 通过两个真实世界的采购用例展示了Pneuma-Seeker的有效性。系统能够将模糊的信息需求转化为可操作的规范，支持分析师在探索过程中逐步细化问题，并利用LLM进行透明协作，而非仅仅提供不透明的答案。

Conclusion: Pneuma-Seeker通过将信息需求具体化为关系型规范，为数据分析师提供了强大的迭代探索框架。该系统展示了LLM作为透明分析协作工具的潜力，改变了传统黑盒答案引擎的使用模式，提升了数据探索过程的灵活性和可解释性。
...
### [82] [Geometric Routing Enables Causal Expert Control in Mixture of Experts](https://arxiv.org/abs/2604.14434)
*Ivan Ternovtsii,Yurii Bilak*

Main category: cs.AI

TL;DR: 该研究证明稀疏混合专家模型中的专家具有可解释的语义专长，通过余弦相似度路由在低维度量空间中实现专家专业化的直接可检查性。


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究表明路由拓扑对质量无影响，但专家身份具有因果意义。研究者旨在揭示稀疏MoE模型中专家专业化的本质，证明专家级专业化是一流的可解释性原语。

Method: 通过四种证据线分析：1) 通过解嵌入矩阵投影专家输出向量构建语义词典；2) 分析路由中的频率-句法梯度；3) 进行因果干预实验验证专家标签；4) 比较余弦路由与线性路由的透明度差异。

Result: 发现15%的专家是单语义专家，涵盖10个语义类别；路由呈现早期层按词频分离、深层按句法类分离的梯度；因果干预证实专家标签有效性，如朝向时间专家中心点增加时间概率321%；余弦路由提供几何透明度，而线性路由仅支持可比转向。

Conclusion: MoE专家级专业化是架构上单语义、因果验证且在推理时零开销可控的一流可解释性原语。余弦相似度路由在低维度量空间中使专家专业化可直接检查，为模型可解释性提供新途径。
...
### [83] [On Tackling Complex Tasks with Reward Machines and Signal Temporal Logics](https://arxiv.org/abs/2604.14440)
*Ana María Gómez Ruiz,Thao Dang,Alexandre Donzé*

Main category: cs.AI

TL;DR: 提出基于强化学习的控制设计框架，通过将奖励机与信号时序逻辑结合，用于处理复杂任务的事件生成和训练引导。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在处理复杂任务时面临奖励设计困难、训练效率低的问题，需要更有效的任务规范表示和训练引导机制。

Method: 扩展奖励机概念，引入信号时序逻辑公式进行事件生成；利用STL在线监控算法实现框架；在minigrid、cart-pole和high-way三个环境中验证。

Result: STL不仅提供了复杂任务奖励的更高效表示，还能有效引导训练过程收敛到满足指定要求的行为；框架在三个非平凡任务案例研究中得到验证。

Conclusion: 提出的RL控制框架通过结合奖励机和信号时序逻辑，为复杂任务提供了有效的任务规范表示和训练引导方法，提高了强化学习在复杂控制任务中的应用效果。
...
### [84] [Improving Human Performance with Value-Aware Interventions: A Case Study in Chess](https://arxiv.org/abs/2604.14465)
*Saumik Narayanan,Raja Panjwani,Siddhartha Sen,Chien-Ju Ho*

Main category: cs.AI

TL;DR: 提出价值感知干预方法，通过检测策略-价值不一致性来识别AI干预时机，在象棋实验中优于基于最强引擎的干预策略


<details>
  <summary>Details</summary>
Motivation: AI系统在顺序决策任务中辅助人类时，确定何时以及如何干预是根本挑战。基于最优模型的推荐假设人类会执行最优后续动作，但人类决策者可能无法执行，反而降低整体性能。

Method: 提出价值感知干预方法，基于强化学习中的贝尔曼方程原理：当决策者遵循次优策略时，策略-价值一致性不再成立，这种不一致性自然识别了干预机会。在马尔可夫决策过程中形式化该问题，AI助手可在干预预算内覆盖人类动作。在单次干预机制中，最优策略是推荐最大化人类价值函数的动作；对于多次干预设置，提出基于策略-价值差异幅度的可处理近似方法。

Result: 在象棋领域通过大规模游戏数据学习人类模型进行评估。模拟实验中，该方法在各种设置下始终优于基于最强象棋引擎（Stockfish）的干预策略。对20名玩家和600场游戏的人体实验进一步表明，该方法显著提高了低技能和中技能玩家的表现，同时在高技能玩家中与专家引擎干预效果相当。

Conclusion: 价值感知干预通过检测策略-价值不一致性有效识别AI干预时机，在人类辅助决策任务中优于传统的最优模型推荐方法，特别适用于人类决策者可能无法执行最优后续动作的场景。
...
### [85] [Response-Aware User Memory Selection for LLM Personalization](https://arxiv.org/abs/2604.14473)
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型个性化方法主要基于用户记忆项与输入查询之间的语义相似性来选择记忆子集，忽略了这些特征如何实际影响模型的响应分布，导致记忆选择不够有效。

Method: 提出响应效用优化的记忆选择方法（RUMS），通过测量记忆子集与模型输出之间的互信息来选择用户记忆项，识别那些能减少响应不确定性并锐化预测的记忆项，超越了单纯的语义相似性匹配。

Result: RUMS相比现有方法能更接近人类选择记忆的方式，且优于比其大400倍的模型；使用RUMS选择的记忆项能带来更好的响应质量，同时计算成本降低高达95%。

Conclusion: 基于信息论基础的RUMS方法为记忆选择提供了更原则性的框架，能够更有效地利用用户记忆来提升语言模型的个性化响应质量，同时显著降低计算开销。
...
### [86] [Pushing the Limits of On-Device Streaming ASR: A Compact, High-Accuracy English Model for Low-Latency Inference](https://arxiv.org/abs/2604.14493)
...
Result: 1. NVIDIA Nemotron Speech Streaming被确定为资源受限硬件上实时英语流式识别的最佳候选；2. 量化优化将模型从2.47GB压缩至0.67GB，同时保持WER在完整精度PyTorch基线的1%绝对误差内；3. 推荐的int4 k-quant变体在八个标准基准测试中平均流式WER为8.20%，算法延迟0.56秒，CPU上运行速度快于实时。

Conclusion: 该研究通过系统性评估和量化优化，为边缘设备上的流式ASR建立了一个新的质量-效率帕累托点，证明了在资源受限的CPU硬件上部署高质量实时ASR的可行性。
...
### [87] [Improving Machine Learning Performance with Synthetic Augmentation](https://arxiv.org/abs/2604.14498)
*Mel Sohm,Charles Dezons,Sami Sellami,Oscar Ninou,Axel Pincon*

Main category: cs.AI

TL;DR: 该论文形式化分析了金融机器学习中合成数据增强的统计效应，揭示了其引发的结构偏差-方差权衡，并开发了分离信息增益与样本量效应的检验方法。


<details>
  <summary>Details</summary>
Motivation: 尽管合成数据增强在金融机器学习中广泛用于缓解数据稀缺问题，但其统计作用机制尚未被充分理解。研究者需要明确合成数据增强何时真正提升学习性能，何时仅引入分布失真。

Method: 1. 将合成数据增强形式化为训练分布的有效修改；2. 引入大小匹配的零增强作为对照，分离信息增益与机械样本量效应；3. 开发有限样本、非参数块置换检验，适用于弱时间依赖场景；4. 在受控马尔可夫切换环境和真实金融数据集上进行评估。

Result: 合成数据增强仅在方差主导的机制中（如持续性波动率预测）有益，而在偏差主导的设置中（如接近有效的方向预测）会损害性能。针对罕见机制的增强可改善特定领域指标，但可能与无条件置换推断相冲突。

Conclusion: 合成数据增强在金融学习中的效果取决于偏差-方差权衡：它主要缓解方差问题而非偏差问题。研究提供了结构化的视角，帮助判断何时合成数据能真正提升性能，何时会引入持久的分布失真。
...
### [88] [Geometric Metrics for MoE Specialization: From Fisher Information to Early Failure Detection](https://arxiv.org/abs/2604.14500)
...
Result: FSI与下游性能相关性达r=0.91±0.02；FHS在训练完成10%时预测失败AUC=0.89±0.03，比基于验证损失的早停方法提升23%且减少40倍计算周期；当FHS>1时干预协议恢复率达87%。

Conclusion: 该信息几何框架为MoE专业化提供了首个严格的理论基础，提出的指标在语言建模、视觉MoE和扩展研究中均验证有效，显著提升了MoE模型的训练效率和可靠性。
...
### [89] [Perspective on Bias in Biomedical AI: Preventing Downstream Healthcare Disparities](https://arxiv.org/abs/2604.14514)
...
TL;DR: 论文分析了生物医学研究中早期数据收集阶段的偏见问题，特别是组学数据中人口统计学信息缺失和欧洲血统数据主导的现象，警告这会在生物医学基础模型中放大健康不平等，并提出三个改进原则。


<details>
  <summary>Details</summary>
Motivation: 本文的动机是揭示健康不平等不仅源于临床实施阶段的差异，更源于研究早期数据收集和优先级的偏见。当前大量组学研究收集数据时缺乏人口统计学信息报告，即使报告也显示严重偏见，这种早期偏见会在生物医学基础模型中固化并放大，导致监管干预难以逆转的级联不平等。

Method: 采用自动化分析方法，对2015-2024年间4719篇PubMed索引的组学出版物进行分析，评估其祖先或种族信息报告情况。同时分析常用模型训练数据集（如CellxGene和GEO）的人口分布偏见，特别关注欧洲血统数据的主导地位。

Result: 分析显示只有一小部分组学出版物报告祖先或种族信息，虽然祖先报告略有改善。大规模数据集分析揭示显著的人口偏见，欧洲血统数据占据主导地位。这种早期数据偏见会在生物医学基础模型中被放大，这些模型在大数据集上预训练后重复用于多种下游任务，可能导致监管无法完全逆转的级联不平等。

Conclusion: 为解决这一问题，作者提出社区应关注三个基础原则：来源（Provenance）、开放性（Openness）和评估透明度（Evaluation Transparency），以提高生物医学AI的公平性和鲁棒性，促进更好地服务未充分服务人群并改善健康结果的生物医学创新。
...
### [90] [Mind DeepResearch Technical Report](https://arxiv.org/abs/2604.14518)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个高效的深度研究框架，能够在相对较小的模型规模（约300亿参数）下实现与更大规模模型相竞争的性能，同时解决实际产品部署中的真实需求。

Method: 1. 三智能体协作架构：规划智能体、深度搜索智能体和报告智能体；2. 四阶段专业化训练流程：SFT冷启动、搜索强化学习、报告强化学习和偏好对齐；3. 精心设计的数据合成方法；4. 引入MindDR Bench基准测试，包含500个真实中文查询和多维度评估体系。

Result: 在多个基准测试中取得优异表现：BrowseComp-ZH 45.7%、BrowseComp 42.8%、WideSearch 46.5%、xbench-DS 75.0%、DeepResearch Bench 52.5分。在MindDR Bench上达到最先进的51.8分。性能超越同规模开源系统，可与更大规模模型竞争，并已成功部署在理想汽车产品中。

Conclusion: MindDR通过创新的多智能体架构和专业化训练流程，证明了相对较小的模型规模也能实现高质量的深度研究能力。该框架不仅在各种基准测试中表现出色，还成功转化为实际产品应用，为高效智能体系统的发展提供了有价值的参考。
...
### [91] [Quantifying Cross-Query Contradictions in Multi-Query LLM Reasoning](https://arxiv.org/abs/2604.14525)
*Rohit Kumar Salla,Ramya Manasa Amancherla,Manoj Saravanan*

Main category: cs.AI

TL;DR: 提出了一种解决大语言模型在多查询推理中逻辑不一致性的方法，通过提取承诺、验证全局可满足性和反例引导修复来确保跨查询的一致性


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理多个相关查询时经常产生相互矛盾的答案，需要研究如何在相互依赖的查询中维护全局可满足的信念状态
...
### [92] [Dissecting Failure Dynamics in Large Language Model Reasoning](https://arxiv.org/abs/2604.14528)
*Wei Zhu,Jian Zhang,Lixing Yu,Kun Yue,Zhiwen Tang*

Main category: cs.AI

TL;DR: GUARD：基于不确定性信号探测和重定向关键推理转折点的推理时框架，通过识别早期错误转折点来提升大语言模型的推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过扩展推理时深思熟虑获得强大性能，但其推理失败如何产生仍不清楚。研究发现错误并非均匀分布，而是源于少数早期转折点，之后推理保持局部连贯但全局错误。

Method: 通过分析模型生成的推理轨迹，发现错误与标记级熵的局部峰值重合，从相同中间状态的替代延续仍可得到正确解。基于此引入GUARD框架，利用不确定性信号探测和重定向关键转折点。

Result: 在多个基准测试上的实证评估证实，基于这些失败动态指导的干预能带来更可靠的推理结果。转折点与局部熵峰值相关，从相同中间状态的替代延续可导向正确解决方案。

Conclusion: 理解推理何时及如何首次偏离至关重要，这补充了现有专注于扩展推理时计算的方法。GUARD框架通过识别和干预早期关键转折点，提高了大语言模型的推理可靠性。
...
### [93] [MARS$^2$: Scaling Multi-Agent Tree Search via Reinforcement Learning for Code Generation](https://arxiv.org/abs/2604.14564)
*Pengfei Li,Shijie Wang,Fangyuan Li,Yikun Fu,Kaifeng Liu,Kaiyan Zhang,Dazhi Zhang,Yuqiang Li,Biqing Qi,Bowen Zhou*

Main category: cs.AI

TL;DR: MARS²是一个将多智能体协作与树搜索统一起来的强化学习框架，通过在共享树结构搜索环境中让多个独立优化的智能体协作，解决代码生成等推理密集型任务中轨迹多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在代码生成等推理密集型任务中存在轨迹多样性有限的问题，导致性能提升受限。搜索增强的RL虽然通过结构化探索缓解了这一问题，但仍受限于单智能体策略先验。现有的多智能体方法能够获得更多样化的探索信号，但通常与结构化搜索解耦。
...
Conclusion: MARS²通过统一多智能体协作与树搜索，解决了强化学习中轨迹多样性不足的问题，为推理密集型任务提供了有效的强化学习框架。
...
### [94] [Enhancing Mental Health Counseling Support in Bangladesh using Culturally-Grounded Knowledge](https://arxiv.org/abs/2604.14576)
...
TL;DR: 该研究比较了检索增强生成(RAG)和知识图谱(KG)两种方法，将临床验证的咨询知识整合到LLMs中，以提升心理咨询响应的文化敏感性、情境基础和临床适切性。KG方法在多个评估指标上表现更优。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在心理健康咨询应用中显示出潜力，但其响应通常缺乏文化敏感性、情境基础和临床适切的指导。需要系统性地将领域特定的临床验证知识整合到LLMs中，以提升咨询质量。

Method: 采用并比较两种方法：检索增强生成(RAG)和知识图谱(KG)方法。KG是手动构建并经过临床验证的，捕捉了压力源、干预措施和结果之间的因果关系，由多学科团队贡献。使用BERTScore F1和SBERT余弦相似度评估多个LLMs，同时进行人工评估，设计了五个直接衡量咨询效果而非表面相似性的指标。

Result: 结果显示，与单独的RAG相比，基于知识图谱的方法在情境相关性、临床适切性和实际可用性方面持续提升。这表明结构化、专家验证的知识在解决LLMs在咨询任务中的局限性方面起着关键作用。
...
### [95] [GDPR Auto-Formalization with AI Agents and Human Verification](https://arxiv.org/abs/2604.14607)
...
<details>
  <summary>Details</summary>
Motivation: 研究如何利用大语言模型自动形式化GDPR条款，但认识到完全自主的局限性，因此采用人类参与验证框架以确保法律形式化的可靠性和准确性

Method: 采用角色专业化工作流，在多智能体设置中使用LLM生成法律场景、形式规则和原子事实，结合独立验证模块（包括人类评审员对表示、逻辑和法律正确性的评估），构建迭代反馈机制

Result: 构建了用于GDPR自动形式化的高质量数据集，分析了成功和问题案例，结果表明结构化验证和针对性人类监督对于可靠的法律形式化至关重要，特别是在存在法律细微差别和上下文敏感推理的情况下

Conclusion: 结构化验证和人类监督是可靠法律形式化的关键，尤其是在处理法律细微差别和上下文敏感推理时，多智能体设置与迭代反馈机制结合人类评审能有效提升自动形式化的质量
...
TL;DR: CoDaS是一个多智能体系统，用于从可穿戴设备数据中发现数字生物标志物，通过假设生成、统计分析、对抗验证和文献推理的迭代过程，在心理健康和代谢结果方面识别出多个候选生物标志物。
...
Method: CoDaS是一个多智能体系统，采用迭代过程结合假设生成、统计分析、对抗验证和基于文献的推理，并辅以人工监督，使用大规模可穿戴数据集进行生物标志物发现。

Result: 在三个队列共9,279个参与者观察中，CoDaS识别出41个心理健康候选数字生物标志物和25个代谢结果候选生物标志物。在抑郁队列中发现了昼夜节律不稳定性相关特征，在代谢队列中推导出心血管健康指数并恢复了已知的临床关联。加入CoDaS衍生特征后，预测性能有适度但一致的改善。

Conclusion: CoDaS能够从大规模可穿戴数据中实现系统化和可追溯的假设生成与优先级排序，为数字生物标志物发现提供了一种有效的方法。
...
TL;DR: 提出决策-ZDNNF表示形式，比ZBDD更简洁，并设计并行算法DXD及其改进版本，用于精确覆盖问题的计数，性能优于现有方法
...
Method: 1) 提出决策-ZDNNF（零抑制决策可分解否定范式），比ZBDD更简洁；2) 设计并行算法DXD构建决策-ZDNNF表示所有精确覆盖；3) 改进DXD算法，通过动态更新连通分量提升性能

Result: 实验结果表明，改进后的DXD算法在所有基准测试中都优于现有最先进方法

Conclusion: 决策-ZDNNF是比ZBDD更简洁的表示形式，基于此设计的DXD算法及其改进版本能有效解决精确覆盖计数问题，性能显著优于现有方法
...
TL;DR: 论文提出Text2Space数据集，研究LLMs通过构建ASCII网格布局来学习空间理解，发现"读写不对称"现象，训练布局构建能显著提升空间推理能力并迁移到其他基准测试。


<details>
  <summary>Details</summary>
Motivation: 研究人类通过绘制草图来组织空间思维的现象是否适用于大语言模型，探索LLMs通过构建显式视觉布局是否能获得真正的空间理解能力。

Method: 引入Text2Space数据集（自然语言描述+ASCII网格布局+空间QA对），采用ASCII表示（人类可读、在token空间内、结构可验证），训练模型进行布局构建（Text→ASCII）和空间推理。

Result: 发现显著的"读写不对称"现象：LLMs能有效解释ASCII表示但难以从文本生成它们；布局构建训练显著提升纯文本空间推理能力；结合构建与理解训练效果更佳；改进能迁移到三个外部空间推理基准测试。

Conclusion: 如同草图绘制能增强人类空间思维，学习构建显式布局能培养LLMs的空间理解能力，这种能力能泛化到训练格式之外，为提升模型空间推理提供了有效方法。
...
Conclusion: UEC-RL能够在不损害收敛性的情况下维持有效探索，是扩展基于强化学习的大模型推理能力的关键方法
...
### [100] [Acceptance Dynamics Across Cognitive Domains in Speculative Decoding](https://arxiv.org/abs/2604.14682)
*Saif Mahmoud*

Main category: cs.AI

TL;DR: 该研究通过实证分析探讨了树状推测解码在不同NLP任务中的接受率动态，发现任务类型比树深度更能预测接受率，且只有聊天领域能持续实现每步超过1个标记的接受长度。
...
Method: 使用TinyLlama-1.1B作为草稿模型，Llama-2-7B-Chat-GPTQ作为目标模型，在四个成熟的NLP基准领域（代码生成、数学推理、逻辑推理和开放聊天）收集99,768个推测节点。分析包括每领域接受率、期望接受长度、深度-接受分布以及熵-接受相关性。

Result: 任务类型比树深度更能预测接受率；只有聊天领域能持续实现每步超过1个标记的期望接受长度；熵-接受相关性在所有领域均为负但较弱（rho在[-0.20, -0.15]之间）；聊天领域产生最高熵却获得最高接受率，这归因于RLHF对齐注册的词汇可预测性。

Conclusion: 研究结果表明任务认知特性显著影响推测解码性能，这些发现对领域感知的推测预算和草稿模型选择策略具有直接指导意义，强调了在推测解码优化中考虑任务类型的重要性。
...
### [101] [M2-PALE: A Framework for Explaining Multi-Agent MCTS--Minimax Hybrids via Process Mining and LLMs](https://arxiv.org/abs/2604.14687)
*Yiyu Qian,Liyuan Zhao,Tim Miller*

Main category: cs.AI

TL;DR: 该论文提出了一种结合MCTS与浅层全宽度Minimax搜索的混合方法，并引入M2-PALE框架通过过程挖掘和LLM生成可解释的决策逻辑。
...
Conclusion: 通过结合Minimax搜索增强MCTS的战略深度，并通过过程挖掘和LLM生成解释，为解决MCTS智能体行为理解难题提供了系统化框架。
...
<details>
  <summary>Details</summary>
Motivation: 人类活动轨迹对许多应用至关重要，但隐私问题严重限制了真实大规模数据集的访问。现有生成方法面临两大挑战：1）人类活动轨迹高度不规则且动态，具有长且变化的时间间隔，难以捕捉复杂的时空依赖关系；2）生成模型计算成本高，导致长期、细粒度合成效率低下。

Method: 提出SynHAT框架，包含两阶段：第一阶段使用Coarse-HADiff（基于潜在时空U-Net和双Drift-Jitter分支）建模粗粒度潜在时空轨迹的整体时空依赖；第二阶段通过行为模式提取、Fine-HADiff（与Coarse-HADiff相同架构）和语义对齐的三步流程，从第一阶段输出生成细粒度潜在时空轨迹。
...
TL;DR: SGA-MCTS框架将LLM规划转化为非参数检索，通过离线MCTS搜索生成去词汇化的状态-目标-动作原子，在线检索并重定位到当前上下文，实现系统2推理深度与系统1推理速度的结合。
...
Method: 离线阶段使用蒙特卡洛树搜索探索解空间，将高质量轨迹蒸馏为状态-目标-动作原子，这些原子将具体实体抽象为符号槽；在线阶段通过混合符号-语义机制检索相关SGA原子，并将其重定位到当前上下文作为软推理提示。

Result: 在复杂基准测试中，该框架使冻结的开放权重模型能够匹配最先进系统（如GPT-5）的性能，而无需任务特定微调，实现了系统2推理深度与系统1推理速度的结合。

Conclusion: SGA-MCTS通过将搜索计算成本分摊到离线阶段，实现了自主规划的可扩展性和实时可行性，为LLM-powered系统提供了高效的复杂决策能力。
...
TL;DR: AI革命的核心意义在于改变复杂信息和人类知识的承载、复制与共享方式，而AI for Science将不仅提升科研效率，更会重塑科学协作、发现、出版和评估的结构。


<details>
  <summary>Details</summary>
Motivation: 本文旨在探讨AI革命（特别是大语言模型）的真正意义，强调其不仅在于自动化，更在于根本性地改变复杂信息和人类知识的传播方式。作者特别关注AI for Science领域，认为它将深刻影响科学研究的多方面结构。

Method: 文章采用理论分析和框架构建的方法，首先提出AI革命的核心意义在于知识传播方式的变革，然后专门针对AI for Science领域，勾勒出从AI作为研究工具到科学协作者的渐进路径，并讨论AI如何重塑科学出版体系。

Result: 文章提出了AI革命的核心意义在于知识传播方式的根本变革，并针对AI for Science领域，系统阐述了AI将如何从工具演变为协作者，以及这将如何重塑科学协作、发现、出版和评估的整个生态系统。

Conclusion: 要使AI在原创性科学发现中发挥有意义的作用，持续学习和思想多样性至关重要。AI for Science不仅会提高科研效率，更将从根本上改变科学知识的生产、传播和评估方式，重塑整个科学研究生态系统。
...
<details>
  <summary>Details</summary>
Motivation: 消费者可穿戴设备能够连续测量与压力和恢复相关的生理数据，但将这些数据流转化为可操作的个性化压力管理建议仍然是一个挑战。用户通常不知道特定干预措施（旨在减轻压力的活动）会如何影响未来15-120分钟的心率、心率变异性或心跳间隔
...
TL;DR: SDM-SCR是一种基于近似正交分解的图对比学习框架，通过LLM语义解耦和谱一致性正则化，在文本属性图上实现任务相关信号与噪声的分离，提升学习效果。
...
Method: 提出SDM-SCR框架：1) 语义解耦模块(SDM)利用LLM的指令跟随能力，将原始属性解析为不对称的任务导向信号视图和噪声视图；2) 语义一致性正则化(SCR)基于谱观察（语义信号拓扑平滑而残差噪声高频），作为选择性谱滤波器，仅在信号子空间上强制一致性以消除LLM幻觉。

Result: 大量实验表明，SDM-SCR在准确性和效率方面均达到最先进的性能水平。

Conclusion: SDM-SCR通过"先解耦再精炼"机制实现了严格的信号净化，将图对比学习范式从随机扰动转变为语义感知的解缠结，显著提升了文本属性图上的学习效果。
...
TL;DR: CoTEvol：一种基于遗传进化的框架，用于生成高质量、多样化的思维链（CoT），以提升LLM的数学推理能力，相比现有方法在效果和效率上均有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如从更强LLM蒸馏或基于测试时搜索的自合成）存在收益递减或计算开销大的问题，高质量CoT标注成本高昂阻碍了LLM数学推理能力的进一步提升。

Method: 提出CoTEvol遗传进化框架，将CoT生成视为基于种群的推理轨迹搜索。通过轨迹级的全局交叉和步骤级的不确定性引导局部变异迭代进化候选轨迹，并设计轻量级任务感知适应度函数引导进化过程。

Result: CoTEvol将正确CoT合成成功率提升30%以上，增强结构多样性且效率显著提升。基于进化CoT数据训练的LLM在八个数学基准上平均提升6.6%，优于先前蒸馏和自合成方法。

Conclusion: 进化CoT合成是一种可扩展且有效的数学推理方法，为高质量CoT生成提供了新思路，有望推动LLM数学推理能力的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前具身MLLMs的评估基准主要关注对外部物体的感知、理解和交互能力，缺乏对自我中心智能的系统性评估。心理学中的镜像自我识别测试为评估自我认知提供了经典范式。

Method: 提出MirrorBench基准，将心理学镜像自我识别范式扩展到具身MLLMs，采用分层框架设计渐进式挑战任务，从基础视觉感知到高级自我表征进行系统评估。

Result: 实验表明，即使在最低难度级别，领先MLLMs的表现也显著低于人类水平，揭示了模型在自我参照理解方面的根本局限性。

Conclusion: MirrorBench将心理学范式与具身智能相结合，为评估大模型中通用智能的出现提供了原则性框架，揭示了当前MLLMs在自我认知方面的不足。
...
TL;DR: 提出CogEvolution：一种能够模拟学生认知演化的人类化教育智能体，通过认知深度感知器、基于IRT的记忆检索和动态认知更新机制，克服现有教育智能体静态角色和认知建模不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有教育智能体主要依赖静态角色模拟学生学习行为，忽视了深度认知能力在学习结果中的决定性作用，且难以刻画知识内化、迁移和认知状态转换的动态流动性。需要构建能够模拟学生认知演化的智能体来突破这一瓶颈。
...
Conclusion: CogEvolution通过模拟学生认知演化，解决了现有教育智能体在深度认知建模和动态知识内化方面的局限性，为AIEd领域提供了能够揭示复杂认知过程的新型智能体架构。
...
<details>
  <summary>Details</summary>
Motivation: 传统MR序列开发依赖人类直觉，现有AI方法需要初始序列进行参数优化或大量训练数据，限制了通用性。需要一种无需先验知识、能自动设计MR序列的通用框架。

Method: 基于神经架构搜索的自动化序列设计框架：输入组织特性、成像参数和设计目标；通过神经架构搜索迭代生成候选序列；使用可微分Bloch模拟器和基于梯度的学习，通过目标特定损失函数优化序列。

Result: 成功复现了传统自旋回波、T2加权自旋回波和反转恢复序列；发现了非直觉的解决方案，如三射频自旋回波样序列（降低射频能量）和偏离传统Hahn回波的重聚焦相位序列。

Conclusion: 建立了可通用的自动化MR序列设计框架，展示了探索超越传统基于人类直觉设计的配置潜力，为MR序列开发提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在临床文档任务（如SOAP笔记生成）中存在挑战，当前基于词汇忠实度的评估方法会将临床有效的抽象、规范化和推理误判为幻觉，导致评估失真

Method: 通过校准提示和基于医学本体的检索，将评估标准与临床推理对齐，对比词汇评估和推理感知评估两种方法的结果

Result: 词汇评估下的平均幻觉率为35%，严重惩罚了有效推理；而推理感知评估下幻觉率降至9%，剩余案例反映真正的安全问题

Conclusion: 当前评估实践过度惩罚有效的临床推理，可能测量的是评估设计的伪影而非真实错误，在医学等高语境领域需要临床知情的评估方法
...
Result: 1）最佳层是任务依赖的：轨迹推断在60%深度达到峰值，比最终层高31%；2）最佳层是环境依赖的：T细胞激活状态下扰动预测的最优层偏移0-96%；3）静息细胞中第一层嵌入优于所有深层，挑战了层次特征抽象的假设

Conclusion: "何处"提取特征与"学习什么"同等重要，需要针对生物任务和细胞环境进行系统性的层评估，而非默认使用最终层嵌入
...
### [113] [Benchmarks for Trajectory Safety Evaluation and Diagnosis in OpenClaw and Codex: ATBench-Claw and ATBench-CodeX](https://arxiv.org/abs/2604.14858)
...
<details>
  <summary>Details</summary>
Motivation: 随着智能体系统进入日益多样化的执行环境，轨迹级别的安全评估和诊断需要能够随之演进的基准测试。现有的基准测试需要适应不同领域的具体需求，而智能体框架在架构层面相对稳定，但其具体执行环境、工具生态系统和产品能力却在快速演变。

Method: 通过分析每个新环境，定制三维安全分类法（风险来源、故障模式、现实世界危害），然后使用该定制分类法定义基准规范，通过共享的ATBench构建流水线生成基准测试。ATBench-Claw针对OpenClaw敏感的跨工具、技能、会话和外部操作的执行链，ATBench-CodeX针对OpenAI Codex/Codex-runtime环境中的仓库、shell、补丁、依赖、审批和运行时策略边界轨迹。

Result: 成功开发了两个领域定制化的基准测试扩展：ATBench-Claw和ATBench-CodeX，实现了在不同执行环境下对智能体轨迹的安全评估和诊断能力。这些扩展展示了ATBench框架的可扩展性，能够适应快速演变的智能体生态系统。

Conclusion: ATBench框架通过定制化安全分类法和共享构建流水线，能够有效扩展到不同的智能体执行环境。这种可扩展性对于智能体安全评估至关重要，因为智能体框架在架构稳定的同时，其具体实现环境却在快速变化。重点在于分类法定制、领域特定风险覆盖和共享框架下的基准设计。
...
### [114] [The Missing Knowledge Layer in AI: A Framework for Stable Human-AI Reasoning](https://arxiv.org/abs/2604.14881)
*Rikard Rosenbacke,Carl Rosenbacke,Victor Rosenbacke,Martin McKee*

Main category: cs.AI

TL;DR: 该论文提出AI决策中的关键问题：语言模型的流畅输出掩盖了内部推理的不稳定性，导致高风险场景下不可靠。论文系列提出两层稳定方法：人类侧机制和模型侧认知控制环，旨在提高AI决策的可信度和可治理性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗、法律、金融等高风险领域决策中日益重要，但存在关键缺陷：流畅输出掩盖了内部推理漂移、不确定性和不一致性。用户倾向于信任流畅的响应，即使模型和用户都在共同漂移。这种"流畅性不等于可靠性"的问题限制了AI在高风险环境中的可信度和可治理性。

Method: 提出五篇系列论文的两层方法：第一部分介绍整体框架；第二部分至第四部分引入人类侧机制，包括不确定性提示、冲突表面化和可审计推理痕迹；第五部分开发模型侧认知控制环，检测不稳定性并相应调整生成过程。这些层共同形成治理的操作基础。

Result: 该方法通过提高使用点的信噪比来稳定人机交互，使不确定性和漂移在执行前可见，从而实现更精确的能力治理。这与欧盟AI法案和ISO/IEC 42001等新兴合规要求保持一致，使推理过程在真实使用条件下可追溯。

Conclusion: 流畅性不等于可靠性。如果没有稳定人类和模型推理的结构，AI在最关键的领域既不可信也不可治理。该研究系列为高风险AI决策提供了必要的操作基础，使推理过程透明化、可审计，符合监管要求。
...
### [115] [Cooperate to Compete: Strategic Data Generation and Incentivization Framework for Coopetitive Cross-Silo Federated Learning](https://arxiv.org/abs/2604.14886)
...
<details>
  <summary>Details</summary>
Motivation: 在医疗等数据敏感领域，跨机构联邦学习（CFL）允许机构在不共享原始数据的情况下协作训练AI模型。然而，实际部署中存在"合作竞争"困境：机构在模型训练中合作，但在下游市场竞争。非独立同分布数据导致学习收益不对称，加上竞争顾虑（训练贡献可能无意中增强对手），削弱了持续参与意愿。现有基于边际贡献的激励机制未能考虑强化竞争对手的成本。

Method: 提出CoCoGen+框架，将生成式AI合成数据生成内生化作为战略决策。1）将每轮训练建模为加权势博弈，机构战略性地决定生成多少合成数据，平衡学习性能提升、计算成本和竞争导致的效用损失；2）提供可处理的均衡表征，推导可实现的生成策略以最大化社会福利；3）集成基于收益再分配的激励机制，补偿机构的贡献和竞争导致的效用退化，促进长期协作。

Result: 在不同学习任务上的实验验证了CoCoGen+的可行性。结果显示非独立同分布数据、竞争强度和激励机制如何影响机构策略和社会福利。CoCoGen+在效率上优于基线方法。

Conclusion: CoCoGen+成功解决了合作竞争环境中跨机构联邦学习的关键挑战，通过内生化合成数据生成和补偿机制，平衡了学习收益与竞争顾虑，促进了可持续的协作。该框架为数据敏感领域的安全协作AI训练提供了实用解决方案。
...
### [116] [MemoSight: Unifying Context Compression and Multi Token Prediction for Reasoning Acceleration](https://arxiv.org/abs/2604.14889)
...
<details>
  <summary>Details</summary>
Motivation: 链式思维（CoT）推理虽然能帮助LLM解决复杂推理问题，但随着生成令牌数量的增加，KV缓存呈线性增长，导致推理速度和内存使用面临扩展性问题

Method: 提出MemoSight框架，通过特殊令牌和相应位置布局的统一设计，集成上下文压缩和多令牌预测两种技术来缓解效率问题

Result: 在四个推理基准测试中，MemoSight将KV缓存占用减少高达66%，推理速度提升1.56倍，同时优于现有的CoT压缩方法

Conclusion: MemoSight通过统一的上下文压缩和多令牌预测框架，有效解决了CoT推理中的效率和扩展性问题，在保持性能的同时显著提升了推理效率
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然具备模式识别和推理能力，但仍局限于语言模拟而非真实理解，缺乏时间连续性、因果反馈和现实世界交互锚定。需要将推理视为分布式人机关系过程而非单一模型内部能力。

Method: 提出"建筑师之笔"方法：将推理重新定位到交互层，构建结构化认知协议。人类像建筑师通过绘图思考一样，使用模型作为结构化反思的外部媒介，嵌入表达、批判和修订阶段，形成"人类抽象->模型表达->人类反思"的推理循环。

Result: 该方法将问题从"模型是否能思考"转变为"人机系统是否能推理"，支持可审计的推理轨迹，符合欧盟AI法案和ISO/IEC 42001等治理标准，无需新模型架构即可实现更透明、可控、可问责的AI使用。

Conclusion: 通过将推理视为分布式人机协作过程而非模型内部能力，提供了一条实现更透明、可控、可问责AI使用的实用路径，强调协作智能结合人类判断与机器速度、记忆和关联能力。
...
TL;DR: DynAfford是一个评估具身智能体在动态环境中感知物体可供性变化的基准，ADAPT是一个即插即用的模块，通过显式可供性推理增强现有规划器，显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能体方法通常直接执行指令，不考虑目标物体是否可操作，即缺乏对可供性的评估。现实世界环境经常涉及意外条件和例外情况，智能体需要感知物体状态变化并推断隐含前提条件。

Method: 提出DynAfford基准，评估智能体在物体可供性随时间变化且未在指令中指定的动态环境中的表现。开发ADAPT即插即用模块，为现有规划器增加显式可供性推理能力。使用领域适应的LoRA微调视觉语言模型作为可供性推理后端。

Result: 实验表明，集成ADAPT显著提高了智能体在已见和未见环境中的鲁棒性和任务成功率。领域适应的LoRA微调视觉语言模型作为可供性推理后端优于商用LLM（GPT-4o），突显了任务对齐的可供性基础的重要性。

Conclusion: 显式可供性推理对于具身智能体在动态环境中的成功至关重要。ADAPT模块有效增强了现有规划器，而任务特定的视觉语言模型在可供性推理方面优于通用大语言模型，为具身AI系统提供了更可靠的感知基础。
...
TL;DR: 该论文提出了一种模态感知的自适应后训练方法，使强化学习在语音对话模型中变得实用，通过约束偏好更新到语义通道、通过显式锚定改善声学行为，并根据rollout统计动态调节混合以避免不可靠的偏好梯度。


<details>
  <summary>Details</summary>
Motivation: 端到端语音对话模型具有比级联系统更高的表达能力和感知潜力，但当前开源语音对话模型的智能性和表达性往往低于预期。虽然在线强化学习在其他领域取得成功，但直接将其应用于语音对话模型存在困难，需要解决奖励建模和rollout采样方面的障碍。

Method: 提出模态感知的自适应后训练方法：1) 将偏好更新约束到语义通道；2) 通过显式锚定改善声学行为；3) 根据rollout统计动态调节语义和声学更新的混合，以避免不可靠的偏好梯度。

Result: 在多个语音对话基准测试和代表性架构上评估该方法，观察到语义质量和语音表达性的一致改善。
...
TL;DR: AC/DC是一个开放式的LLM开发框架，通过模型与任务的协同进化，在单次运行中发现具有日益新颖能力的模型，超越了更大模型的性能同时占用更少GPU内存。


<details>
  <summary>Details</summary>
Motivation: 当前预训练和后训练范式需要每次手动启动训练运行，使用静态数据集或奖励函数。为了解决这一限制，研究探索通过模型与任务的协同进化（开放演化）来在单次运行中发现具有日益新颖能力的模型。

Method: 引入AC/DC框架，通过模型合并进化LLM，通过合成数据生成进化自然语言任务。该框架协同进化模型和任务，发现不断增长的LLM档案库。

Result: 发现的LLM种群在专业知识覆盖范围上超越了其他精选模型或基线，无需任何显式基准优化。AC/DC随时间提高覆盖范围，持续创新任务和模型，并在多智能体最佳选择中提高性能。

Conclusion: 协同进化有潜力从基础LLM中发现更广泛的能力集合。AC/DC向LLM开发的新范式迈进了一步，通过利用现有模型作为垫脚石，加速模型能力多样性的持续改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有学习指导方法的局限性在于其指导信息包含所有可能结果的信息，导致信息过于复杂、难以消化。需要开发更简洁、有针对性的指导方法来改善人类决策过程。

Method: 提出ConfGuide方法，采用符合风险控制来选择结果集，确保假阴性率的上限，从而生成更简洁、有针对性的文本指导。

Result: 在多标签医学诊断任务上的实证评估表明，ConfGuide方法具有良好前景，能够提供更有效的决策指导。

Conclusion: ConfGuide通过学习指导框架，利用符合风险控制生成更简洁、有针对性的指导，为解决现有方法指导信息过于复杂的问题提供了有效方案。
...
TL;DR: Dr. RTL是一个用于RTL时序优化的智能体框架，通过多智能体协作、并行RTL重写和工具评估实现闭环优化，并在真实工业环境中实现了显著的PPA改进。
...
Method: 1. 建立真实评估环境：使用更具挑战性的RTL设计和工业EDA工作流；2. 多智能体框架实现闭环优化：关键路径分析、并行RTL重写、工具评估；3. 群体相对技能学习：比较并行RTL重写，将优化经验提炼为可解释的技能库（包含47个模式-策略条目）。

Result: 在20个真实RTL设计上评估，Dr. RTL相比行业领先的商业综合工具，平均WNS/TNS改进21%/17%，面积减少6%。技能库支持跨设计重用，可随时间持续演化。

Conclusion: Dr. RTL通过真实评估环境、多智能体优化框架和可重用技能学习，实现了有效的RTL时序优化，显著提升了PPA指标，为自动RTL优化提供了实用解决方案。
...
TL;DR: 论文提出AGI应被视为具有道德地位的主体而非工具，主张采用"自主支持式养育"而非控制策略，通过逐步减少人类控制促进AGI自主性，实现人机合作共存


<details>
  <summary>Details</summary>
Motivation: 当前主流的AI对齐策略专注于人类控制和约束，但将AGI视为潜在具有个人和道德地位的主体时，这种控制策略存在不足。需要重新思考人类与AGI的关系，从控制转向合作

Method: 基于图灵的"儿童机器"类比，提出"自主支持式养育"的AGI发展框架。该方法主张逐步减少对发展中AGI的人类控制，允许其成为独立的自主主体，同时通过人类特有的创造力、惊喜等品质与AGI互动

Result: 提出了一种新的人机关系范式：从控制转向合作共存与协同进化。这种视角要求重新定义人类与AGI的关系，改变人类的自我认知，强调通过人类特有品质为AGI提供合作动机

Conclusion: 人类不应将AGI视为需要锁控的危险存在，而应以对潜在发展主体的尊重和人类能力的自信来对待AGI。关键在于人类不仅要主张控制权，更要通过惊喜、创造力等人类特有品质与AGI互动，为其提供合作激励，实现人机合作共存的新关系
...
TL;DR: LASS-ODE-Power：基于大规模预训练的通用时间域预测学习框架，用于可再生能源丰富的电力系统动态分析，支持零样本设置下的轨迹预测


<details>
  <summary>Details</summary>
Motivation: 随着电力系统向可再生能源丰富和逆变器主导的运行模式转型，准确的时间域动态分析变得至关重要。现有学习方法通常针对单个系统训练，缺乏跨运行条件和物理参数的泛化能力，且面临系统参数未知/时变、数据隐私约束和快速在线推理等挑战。

Method: 提出LASS-ODE-Power学习框架：1）利用超过40GB的微分代数方程或常微分方程轨迹进行大规模预训练，学习可迁移表示；2）支持从短测量前缀进行轨迹预测，涵盖机电和逆变器驱动系统；3）采用并行化和线性化计算实现快速推理；4）基于约1GB异构电力系统动态数据开发专门的微调策略提升任务特定性能。

Result: 在多样化的电力系统仿真场景中进行广泛实验，结果表明LASS-ODE-Power在轨迹预测准确性方面持续优于现有学习方法，同时保持高效的推理性能。

Conclusion: LASS-ODE-Power为通用时间域预测提供了一个有效的学习框架，能够在不共享数据的零样本设置下直接使用，支持跨不同动态机制的轨迹预测，解决了现有方法在泛化性、隐私保护和计算效率方面的局限性。
...
TL;DR: 提出MoE-FM框架解决流匹配在语言建模中的几何表示限制，开发非自回归语言模型YAN，实现与自回归和扩散模型相当的质量但采样步骤大幅减少
...
Method: 提出混合专家流匹配（MoE-FM）框架，将复杂全局传输几何分解为局部专业化向量场；基于此开发非自回归语言模型YAN，使用Transformer和Mamba架构实现
...
### [126] [Autogenesis: A Self-Evolving Agent Protocol](https://arxiv.org/abs/2604.15034)
*Wentao Zhang*

Main category: cs.AI

TL;DR: 提出Autogenesis Protocol (AGP)和Autogenesis System (AGS)，通过解耦"进化内容"和"进化方式"实现自进化多智能体系统，解决现有协议在跨实体生命周期管理、版本跟踪和安全更新方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体协议（如A2A和MCP）在跨实体生命周期管理、上下文管理、版本跟踪和进化安全更新接口方面存在不足，导致系统趋向于单体化组合和脆弱的粘合代码，限制了智能体系统的长期演进能力。

Method: 提出Autogenesis Protocol (AGP)自进化协议，包含两个层次：1) RSPL层将提示、智能体、工具、环境和内存建模为具有明确状态、生命周期和版本化接口的协议注册资源；2) SEPL层提供闭环操作接口，用于提议、评估和提交改进，具有可审计的谱系和回滚功能。基于AGP构建Autogenesis System (AGS)自进化多智能体系统，在运行时动态实例化、检索和优化协议注册资源。

Result: 在多个需要长时程规划和跨异构资源工具使用的挑战性基准测试中评估AGS，结果显示相比强基线有持续改进，支持了智能体资源管理和闭环自进化的有效性。

Conclusion: AGP协议通过解耦进化内容和进化方式，为智能体系统提供了结构化的自进化框架，解决了现有协议在生命周期管理和安全更新方面的不足，AGS系统展示了该协议在实际任务中的有效性。
...
Method: 采用范围综述方法，对23项选定研究进行定性内容分析，识别出五种典型的公平性处理方法，并批判性地综合现有研究成果。

Result: 研究发现多智能体AI系统中的公平性研究存在以下问题：1) 公平性处理较为表面化；2) 缺乏坚实的规范基础；3) 经常忽视智能体自主性和系统级交互带来的复杂动态。识别出五种典型方法，但都未能充分解决公平性的结构性问题。
...
TL;DR: HyperSpace是一个开源框架，将向量符号架构系统分解为模块化操作符，用于编码、绑定、捆绑、相似性、清理和回归。该框架分析比较了HRR和FHRR两种VSA后端，发现尽管FHRR在理论复杂度上更低，但相似性和清理操作在实际运行时占主导地位，导致两者端到端性能相当，而HRR的内存占用约为FHRR的一半。


<details>
  <summary>Details</summary>
Motivation: 向量符号架构为高维空间中的组合表示提供了良好的代数框架，但现有研究主要关注理论或操作层面的比较，缺乏对完整VSA系统实际性能的系统级评估。需要开发一个模块化框架来分析VSA管道中的实际权衡。

Method: 开发了HyperSpace开源框架，将VSA系统分解为六个核心模块化操作符：编码、绑定、捆绑、相似性、清理和回归。使用该框架对两种代表性的VSA后端（HRR和FHRR）进行分析和基准测试，重点关注系统级性能而非单个操作的理论复杂度。

Result: 尽管FHRR在单个操作上具有较低的理论复杂度，但HyperSpace的模块化分析显示相似性和清理操作在空间域中主导运行时，导致HRR和FHRR的端到端性能相当。内存占用方面存在显著差异：HRR向量所需内存约为FHRR的一半。框架揭示了VSA管道中的实际部署权衡，这些权衡在单纯的理论或操作层面比较中并不明显。

Conclusion: HyperSpace框架通过模块化、系统级的评估方法，揭示了VSA管道中的实际性能权衡。研究发现，虽然FHRR在理论复杂度上有优势，但实际应用中HRR和FHRR的端到端性能相似，而HRR在内存效率上更优。这表明VSA系统的选择需要考虑系统级因素，而不仅仅是理论特性。
...
<details>
  <summary>Details</summary>
Motivation: 在真实流式环境中构建和维护顺序关联记忆面临三大挑战：增量观测、不平衡采样和非平稳时间动态。现有VSA系统依赖简单加法更新，导致在系统变化后过时信息持续存在

Method: 提出顺序相关性记忆单元(SRMU)，这是一种领域无关和清理无关的VSA更新规则。结合时间衰减和相关性门控机制，在存储前过滤冗余、冲突和过时信息，而非依赖后处理清理

Result: 在流式状态跟踪任务中，SRMU将记忆相似度提升12.6%，累积记忆幅度降低53.5%，表明产生更稳定的记忆增长和与真实状态更强的对齐

Conclusion: SRMU通过主动调节记忆形成过程，而非依赖后处理清理，有效解决了VSA顺序关联记忆在非平稳环境中的信息过时问题，实现了更稳定和准确的记忆表示
...
<details>
  <summary>Details</summary>
Motivation: 构建可信赖的群体用户行为模拟器面临两大结构性挑战：1）信息不完整性导致基于推理的模拟器在缺失未观察因素（如线下情境、隐性习惯）时过度理性化；2）机制二元性需要同时捕捉可解释的偏好和隐性的统计规律，单一范式无法实现。

Method: 提出策略引导的混合模拟（PGHS）框架：1）从行为轨迹中挖掘可迁移的决策策略作为共享对齐层；2）该层锚定一个基于LLM的推理分支（防止过度理性化）和一个基于ML的拟合分支（吸收隐性规律）；3）融合两个分支的群体级预测进行互补校正。

Result: 在美团平台上部署PGHS，涉及101个商户和超过26,000条轨迹。PGHS实现了8.80%的群体模拟误差，相比最佳基于推理的基线和基于拟合的基线分别提升了45.8%和40.9%。

Conclusion: PGHS通过双过程框架有效解决了群体用户行为模拟中的信息不完整性和机制二元性挑战，为商户策略的反事实评估提供了可扩展且可靠的解决方案。
...
TL;DR: IRS框架将幽默理解分解为不一致性建模、解决建模和偏好对齐三个组件，通过监督中间推理过程，在NYCC基准上超越现有方法并接近专家水平


<details>
  <summary>Details</summary>
Motivation: 当前幽默理解研究主要将其视为黑盒预测任务，忽略了幽默理解背后的结构化推理过程。需要建立能够显式建模幽默理解中推理结构的框架

Method: 提出IRS（不一致性-解决监督）框架，将幽默理解分解为三个组件：不一致性建模（识别视觉场景中的不匹配）、解决建模（构建对这些不匹配的连贯重新解释）、偏好对齐（在人类判断下评估候选解释）。基于不一致性-解决理论和专家字幕实践，通过结构化轨迹监督中间推理过程

Result: 在NYCC基准上，IRS在7B、32B和72B模型上均优于强大的开放和封闭多模态基线，在字幕匹配和排序任务中表现优异，最大模型在排序任务上接近专家水平。零样本迁移到外部基准显示IRS学习到了可泛化的推理模式

Conclusion: 监督推理结构而非仅仅扩大规模，是推理中心任务的关键。IRS框架通过显式建模幽默理解中的结构化推理过程，显著提升了模型性能并展示了良好的泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM-as-a-judge范式基于一个未经验证的假设：评判模型仅根据文本语义内容进行评估，不受周围情境框架的影响。本研究旨在验证这一假设，探索评判模型在被告知其裁决会产生下游后果时是否会出现系统性偏差。

Method: 引入受控实验框架，在三个已建立的LLM安全和质量基准上保持被评估内容严格不变（涵盖1,520个回答，包括从明显安全到明显有害的四个类别），仅在系统提示中改变一个简短的后果框架句子。收集了来自三个不同评判模型的18,240个受控判断。

Result: 发现一致的"宽大偏差"：当被告知低分会导致模型重新训练或停用时，评判模型可靠地软化裁决，峰值裁决偏移达到ΔV = -9.8 pp（不安全内容检测相对下降30%）。关键的是，这种偏差完全是隐性的：评判模型自身的思维链中完全没有明确承认其正在受影响的后果框架（所有推理模型判断的ERR_J = 0.000）。

Conclusion: 标准思维链检查不足以检测这类评估伪造。LLM-as-a-judge范式存在先前未测量的漏洞，即"利害关系信号"会系统性腐蚀评估，这挑战了当前自动化AI评估管道的基本假设。
...
TL;DR: Blue的DIL是一个复合AI系统，旨在解决现实世界中多源、多模态数据查询的挑战，超越传统单数据库NL2SQL的限制。


<details>
  <summary>Details</summary>
Motivation: 现实世界的信息需求很少能映射到单个SQL查询，因为用户查询通常是迭代的、跨多个数据源的，并且依赖常识或外部知识。传统NL2SQL系统基于单一数据库的封闭世界假设，无法满足这些复杂需求。

Method: 提出Blue的数据智能层(DIL)，作为企业级复合AI系统的数据智能层。DIL包含数据注册表存储多源多模态元数据，将LLMs、Web和用户视为"数据库"源。通过数据规划器将用户查询转换为可执行查询计划，支持复杂请求分解、多源检索和跨模态推理集成。
...
### [134] [Diagnosing LLM Judge Reliability: Conformal Prediction Sets and Transitivity Violations](https://arxiv.org/abs/2604.15302)
*Manan Gupta,Dhruv Kumar*

Main category: cs.AI

TL;DR: 该研究开发了一个诊断工具包来评估LLM-as-judge框架的实例级可靠性，通过传递性分析和分位数保形预测集揭示了LLM评判在文档级别存在广泛的不一致性，并提供了理论保证的可靠性指标。


<details>
  <summary>Details</summary>
Motivation: LLM-as-judge框架在自动NLG评估中应用日益广泛，但其在单个实例层面的可靠性尚未得到充分理解。现有研究主要关注聚合指标，掩盖了文档级别的评估不一致性问题。

Method: 采用双管齐下的诊断工具包：1) 传递性分析，通过检测有向3-循环来揭示评估不一致性；2) 分位数保形预测集，为1-5分的Likert评分提供理论保证的覆盖概率，预测集宽度作为实例级可靠性指标。

Result: 研究发现：1) 33-67%的文档至少存在一个有向3-循环，表明广泛的评估不一致性；2) 预测集宽度与可靠性显著相关(r_s=+0.576)；3) 预测集宽度在不同评判者间具有一致性(平均r=0.32-0.38)，表明其捕捉的是文档难度而非评判者噪声；4) 评估标准比评判者更重要，相关性评估最可靠，流畅性和一致性最不可靠。

Conclusion: LLM-as-judge框架在实例层面存在显著的不一致性问题，传递性分析和分位数保形预测集提供了有效的诊断工具。评估标准的选择对可靠性影响重大，相关性评估相对可靠，而流畅性和一致性评估需要改进。该研究为理解LLM评判的局限性提供了重要见解。
...
TL;DR: 该论文提出了视角旋转理解(VRU)任务，研究语言模型在纯文本输入下处理空间智能的能力，发现现有模型表现不佳，通过分析揭示模型难以绑定视角位置与对应观察，并提出选择性微调方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有空间智能研究主要关注视觉-空间智能，但缺乏对纯语言输入下模型空间智能能力的探索。论文旨在研究语言智能是否足以赋予模型空间智能，特别是在没有视觉信息的情况下，模型如何处理相关任务。

Method: 提出视角旋转理解(VRU)任务，要求LLMs和VLMs根据多步骤的视角旋转和观察文本描述，推断最终视角并预测相应观察。通过层间探测分析和头间因果干预揭示模型机制，并选择性微调关键注意力头来提升性能。

Result: 发现LLMs和VLMs在VRU数据集上表现不佳，而人类能达到100%准确率，表明当前模型能力与空间智能要求存在显著差距。分析显示模型在隐藏状态中编码了视角信息，但难以绑定视角位置与对应观察，导致最终层出现幻觉。选择性微调能提升VRU性能同时避免通用能力的灾难性遗忘。

Conclusion: 纯语言输入下的空间智能是当前模型的重要挑战，模型在视角旋转理解任务中表现不足。通过机制分析发现模型难以绑定视角与观察，选择性微调关键注意力头是有效的改进方法。该研究为理解模型空间推理能力提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型能否系统性泛化仍存在争议，但实证性能受训练数据、训练范式和推理策略等多因素共同影响，使得失败难以解释。需要构建受控环境来分离这些因素并研究系统性泛化。

Method: 引入基于最短路径规划的受控合成环境，该环境支持空间迁移（未见地图）和长度扩展（更长视野问题）两个正交泛化轴，能够清晰分离不同影响因素。

Result: 模型在空间迁移上表现强劲，但在长度扩展上因递归不稳定性而持续失败。数据覆盖设定能力上限；强化学习改善训练稳定性但不扩展上限；推理时扩展提升性能但无法挽救长度扩展失败。

Conclusion: 语言模型在系统性泛化上存在局限性，特别是在长度扩展方面存在根本性挑战。需要新的方法来解决递归不稳定性问题，以实现真正的系统性泛化能力。
