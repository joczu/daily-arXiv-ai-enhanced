...
TL;DR: 该论文开发了一个轻量级、成本效益高的框架，用于使用参数高效微调进行灾难推文分类，在资源受限的应急环境中部署大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 从社交媒体及时分类人道主义信息对于有效的灾难响应至关重要，但在资源受限的应急环境中部署大型语言模型面临挑战，需要开发轻量级、成本效益高的解决方案。

Method: 通过整合和标准化HumAID数据集（76,484条推文，涵盖19个灾难事件）构建统一实验语料库，形成双任务基准：人道主义信息分类和事件类型识别。系统评估提示策略、LoRA微调和检索增强生成（RAG）在Llama 3.1 8B上的表现。

Result: LoRA仅训练约2%的参数就实现了79.62%的人道主义分类准确率（比零样本提升37.79%）；QLoRA以50%的内存成本实现了LoRA性能的99.4%；与常见假设相反，RAG策略会因检索示例中的标签噪声而降低微调模型的性能。

Conclusion: 这些发现为在有限计算资源下构建可靠危机情报系统建立了一个实用、可复现的管道，证明了参数高效微调在资源受限应急环境中的有效性。
...
TL;DR: 提出检索增强生成框架，通过重述语言模型和自适应思维链推理模型，显著降低ASR中命名实体的字符错误率。


<details>
  <summary>Details</summary>
Motivation: 端到端ASR系统经常错误识别领域特定短语如命名实体，这可能导致下游任务的灾难性失败。现有基于LLM的命名实体纠错方法尚未充分利用LLM的复杂推理能力。
...
TL;DR: 本章介绍了多模态大语言模型(MLLMs)的基础知识、代表性模型、实用技术(预处理、提示工程、LangChain/LangGraph构建多模态管道)，并讨论了挑战与趋势。


<details>
  <summary>Details</summary>
Motivation: MLLMs结合了LLMs的自然语言理解生成能力与图像、音频等模态的感知技能，代表了当代AI的重要进展。本章旨在系统介绍MLLMs的基础知识、实用技术和未来发展方向。

Method: 1. 介绍MLLMs的基本原理和代表性模型；2. 探讨预处理、提示工程等实用技术；3. 使用LangChain和LangGraph构建多模态管道；4. 提供在线补充材料供进一步实践研究。

Result: 提供了MLLMs的全面介绍，包括理论基础、实用工具和实现方法。公开了GitHub仓库(https://github.com/neemiasbsilva/MLLMs-Teoria-e-Pratica)作为补充学习资源。

Conclusion: 本章系统阐述了MLLMs的核心概念、技术实现和实际应用，同时指出了当前面临的挑战和未来发展趋势，为研究者和开发者提供了有价值的参考。
...
TL;DR: RankLLM是一个新颖的LLM评估框架，通过双向分数传播量化问题难度和模型能力，实现细粒度评估，在30个模型和35,550个问题上验证了有效性。
...
Result: 在30个模型和35,550个跨领域问题上评估，与人类判断达到90%一致率，优于IRT等基线方法，表现出强稳定性、快速收敛和高计算效率。
...
TL;DR: 提出RBCorr方法，通过简单策略校正语言模型在固定回答问题中的选项偏好偏差，提升模型性能


<details>
  <summary>Details</summary>
Motivation: 语言模型存在响应偏差，表现为在固定回答问题中的选项偏好偏差，需要开发低成本有效的校正方法来提升模型性能并准确评估模型能力
...
TL;DR: 提出基于主题建模的语义框架，利用上下文句子嵌入和密度聚类发现问卷项目的潜在语义结构，实现无需预定义因子数的量表简化，平均减少60.5%项目数同时保持心理测量学特性。


<details>
  <summary>Details</summary>
Motivation: 传统量表优化方法（如因子分析、项目反应理论）需要大样本且受数据可用性和跨文化可比性限制。自然语言处理进展表明问卷项目的语义结构可能编码潜在构念组织，提供了一种无需被试反应的补充视角。

Method: 引入主题建模框架：1) 使用上下文句子嵌入编码项目；2) 通过密度聚类发现潜在语义因子（无需预定义数量）；3) 基于类别的术语加权生成可解释的主题表示；4) 在集成简化流程中使用成员标准选择代表性项目。

Result: 在DASS、IPIP和EPOCH量表上验证：方法恢复了与既有构念一致的连贯因子式分组；所选项目平均减少60.5%量表长度同时保持心理测量学充分性；简化量表与原量表因子结构高度一致且保持因子间相关性。

Conclusion: 语义潜在组织提供了测量结构的无反应近似，该框架将语义结构形式化为可检查的量表构建和简化前端。提供可视化支持工具，实现一键式语义分析和结构化简化。
...
TL;DR: HiFloat格式（HiF8和HiF4）是针对昇腾NPU优化的低比特浮点格式，在权重激活和KV缓存任务中表现出色，特别适合高方差数据，4位版本能避免整数格式的精度崩溃。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模扩大，需要低比特浮点格式（如MXFP和NVFP4）来平衡精度和效率。针对昇腾NPU设计专门的浮点格式，以解决现有格式在特定场景下的局限性。

Method: 评估HiFloat格式家族（HiF8和HiF4），通过权重激活和KV缓存任务的严格比较，分析不同格式在不同数据分布下的表现，并验证其与最先进后训练量化框架的兼容性。

Result: 三个关键发现：1）INT8适合窄范围数据，浮点格式适合高方差数据；2）在4位场景中，HiF4的分层缩放能避免整数格式的精度崩溃；3）HiFloat与最先进的后训练量化框架完全兼容。

Conclusion: HiFloat为NPU上的高效大语言模型推理提供了解决方案，在保持精度的同时实现了高效率，特别适合处理高方差数据分布的场景。
...
TL;DR: CLASE是一种混合评估方法，专注于中文法律文本的风格评估，结合了基于语言特征的评分和经验引导的LLM评分，无需参考文本即可评估法律文本的风格质量。


<details>
  <summary>Details</summary>
Motivation: 现有法律文本评估方法存在不足：专家手动制定标准不切实际，现有自动评估方法要么将语义准确性与风格保真度混淆，要么缺乏透明度和一致性。需要一种可靠的法律文本风格评估方法。

Method: CLASE采用混合评分机制：1)基于语言特征的评分，2)经验引导的LLM-as-a-judge评分。特征系数和LLM评分经验均从真实法律文档与其LLM恢复版本的对比对中学习。该方法以透明、无参考的方式捕捉表面特征和隐含风格规范。

Result: 在200份中文法律文档上的实验表明，CLASE与人类判断的一致性显著高于传统指标和纯LLM-as-a-judge方法。除了更好的一致性外，CLASE还提供可解释的分数细目和改进建议。

Conclusion: CLASE为法律文本生成中的专业风格评估提供了可扩展且实用的解决方案，能够透明、无参考地评估法律文本的风格质量，并提供可解释的反馈。
...
TL;DR: 提出PACED-RL框架，通过将GFlowNets中的配分函数重新解释为每个提示的期望奖励信号，利用这一未充分利用的信息来提高LLM分布匹配训练的样本效率。


<details>
  <summary>Details</summary>
Motivation: 传统奖励最大化RL方法虽然能提升LLM的推理性能，但会降低输出多样性。现有GFlowNet方法将配分函数仅视为归一化因子，忽略了其包含的每个提示期望奖励信息，导致样本效率不高。

Method: 提出PACED-RL框架：1) 建立配分函数与每个提示准确率估计的理论关系；2) 利用准确率估计优先选择信息量大的问题提示进行训练；3) 通过准确率估计误差优先回放进一步提高样本效率。两个组件都重用GFlowNet训练中已产生的信息。

Result: 在多个基准测试上的广泛实验表明，PACED-RL相比GRPO和先前GFlowNet方法有显著性能提升，证明了其作为更高效LLM分布匹配训练方向的潜力。

Conclusion: 通过将配分函数重新解释为期望奖励信号并利用其指导训练，PACED-RL有效提高了LLM分布匹配训练的样本效率，同时保持了输出多样性，为LLM后训练提供了有前景的新方向。
...
Conclusion: 概率奖励模型(PRM)为奖励建模提供了新范式，通过将奖励视为随机变量并学习完整概率分布，克服了现有方法的局限性，实现了更好的准确性和数据效率，同时提供了概率解释能力。
...
Result: X-KD在抽象摘要、机器翻译和算术推理任务上超越了广义KD和MiniLLM基线方法。同时，X-KD在性能-多样性权衡和数据效率方面优于基线KD方法。

Conclusion: 通过让学生模型在教师模型的原始学习环境中学习，X-KD提供了一种更有效的知识蒸馏框架，不仅提升了性能，还改善了性能-多样性权衡和数据效率，为大语言模型的知识蒸馏提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG融合方法（查询融合、参数融合、潜在融合）在检索规模较小时有效，但当检索候选数k增加时，性能无法优雅扩展。更大的k虽然提高了证据覆盖率，但不可避免地包含不相关或冗余内容，同时增加了推理成本。

Method: ReFilter包含三个关键组件：上下文编码器（编码上下文特征）、门控过滤器（对每个token加权）和token融合模块（将加权token特征集成到LLM隐藏状态中）。该框架执行token级过滤和融合。

Result: 在四个通用领域QA基准测试中，ReFilter在域内适应和域外迁移下均获得最佳平均性能。在五个生物医学QA基准测试中，无需领域微调的零样本迁移达到70.01%平均准确率（使用Qwen2.5-14B-Instruct）。

Conclusion: ReFilter通过token级过滤和融合有效解决了RAG在检索规模扩大时的扩展性问题，在通用和生物医学领域均表现出优越性能，特别是在零样本迁移场景下具有很好的泛化能力。
...
TL;DR: 提出Lamer-SSL框架，通过层感知混合LoRA专家模块和重放策略，解决自监督语音模型在新语言泛化差和持续训练中遗忘的问题，仅需训练2.14%参数。


<details>
  <summary>Details</summary>
Motivation: 自监督语音模型在新语言上泛化能力差，且在持续训练过程中容易遗忘先前学到的知识，需要一种参数高效的方法来扩展模型到新语言同时保持原有性能。

Method: 提出Lamer-SSL框架：1) Lamer模块（层感知混合LoRA专家），通过LoRA专家混合平衡共享和语言特定表示，层感知专家分配为语义信息更丰富的深层分配更多专家；2) 重放策略，使用少量数据保留先验知识，减轻持续训练中的遗忘。

Result: 在自动语音识别（ASR）和语言识别（LID）任务上的实验表明，Lamer-SSL能有效将自监督模型扩展到新语言，同时保持先前学习语言的强性能，仅需训练2.14%的参数。

Conclusion: Lamer-SSL是一种参数高效的框架，通过层感知LoRA专家混合和重放策略，解决了自监督语音模型在新语言泛化和持续训练遗忘的问题，实现了多语言扩展的平衡。
...
TL;DR: 提出一种基于错误分析的序列标注任务评估方法，通过手工构建小型语言学动机测试集来提供诊断性、可操作性和预测性结果


<details>
  <summary>Details</summary>
Motivation: 传统NLP评估仅表明系统A平均优于系统B，但缺乏改进指导，且无法预测在外部数据上的表现。需要一种能提供定量和定性信息、指导性能改进并预测不同分布表现的评估方法。

Method: 提出基于错误分析的评估方法：不依赖大量真实分布数据，而是手工构建小型语言学动机测试集，系统覆盖跨度属性（形状、长度、大小写、句子位置等），创建能全面评估系统能力的测试集。

Result: 在西班牙语英语借词识别基准测试中验证，该方法提供诊断性（识别系统性弱点）、可操作性（指导模型选择）和预测性结果：预测外部数据集性能的中位数相关性达0.85。

Conclusion: 该方法为序列标注任务提供了一种更有效的评估框架，能够指导性能改进、支持模型选择，并能准确预测模型在不同数据分布上的表现。
...
TL;DR: 本研究针对波斯语旅游评论提出了一种混合BERT模型，结合Top-K路由和辅助损失函数，在低资源语言环境下实现了高效的情感分析，F1分数达90.6%，同时降低39%的GPU能耗。


<details>
  <summary>Details</summary>
Motivation: 针对波斯语作为低资源语言在旅游领域情感分析研究的不足，解决现有方法在效率和准确性方面的问题，特别是路由崩溃和计算资源消耗大的挑战。

Method: 提出混合BERT模型，包含三个步骤：1) 使用BERT对9,558条标注评论进行整体情感分类；2) 对六个旅游相关方面进行多标签方面提取；3) 采用动态路由的集成ABSA方法。模型采用Top-K路由和辅助损失函数防止路由崩溃。

Result: 模型在ABSA任务上获得90.6%的加权F1分数，优于基线BERT（89.25%）和标准混合方法（85.7%）。GPU能耗降低39%，清洁度和设施是提及率最高的方面。构建了包含58,473条预处理评论的数据集。

Conclusion: 这是首个专注于波斯语旅游评论的ABSA研究，提出的混合BERT模型在准确性和效率方面均表现优异，支持可持续AI发展。发布的标注数据集将促进旅游领域多语言NLP研究。
...
TL;DR: RAT-Bench是一个基于重识别风险的文本匿名化工具基准测试，使用美国人口统计生成包含直接和间接标识符的合成文本，评估发现即使最佳工具也不完美，LLM基匿名化工具提供更好的隐私-效用权衡但计算成本更高。


<details>
  <summary>Details</summary>
Motivation: 现有文本匿名化工具（如Microsoft Presidio、Anthropic PII purifier）通常只评估特定标识符的移除能力，但其防止重识别的实际效果尚不清楚，需要基于重识别风险的全面基准测试。

Method: 引入RAT-Bench基准：1）使用美国人口统计数据生成包含各种直接和间接标识符的合成文本，涵盖不同领域、语言和难度级别；2）评估基于NER和LLM的文本匿名化工具；3）基于LLM攻击者从匿名化文本中正确推断属性的能力，计算在美国人口中的重识别风险，并考虑标识符的不均衡影响。

Result: 1）不同工具能力差异很大；2）即使最佳工具也不完美，特别是当直接标识符以非标准方式书写时，以及间接标识符能够实现重识别时；3）基于LLM的匿名化工具（包括新型迭代匿名化器）提供更好的隐私-效用权衡，但计算成本更高；4）LLM基工具跨语言表现良好。

Conclusion: 需要开发更有效的匿名化工具，特别是处理非标准标识符和间接标识符。建议未来工具改进方向，并发布RAT-Bench基准以鼓励社区扩展，特别是扩展到其他地理区域。
...
TL;DR: LLM训练过程中脑活动预测能力的左-右半球不对称性与模型的形式语言能力（语法知识）共同出现，而非算术、世界知识或推理能力


<details>
  <summary>Details</summary>
Motivation: 探究LLM训练过程中脑活动预测能力左-右半球不对称性出现的原因，理解这种不对称性与模型获得何种能力相关

Method: 使用OLMo-2 7B在不同训练检查点的激活与英语参与者的fMRI数据，比较脑预测分数左-右不对称性与多个基准测试性能的演变关系；在Pythia模型和法语数据上进行泛化验证

Result: 左-右半球不对称性与LLM的形式语言能力（语法可接受性判断和文本生成能力）共同出现，但与算术、Dyck语言任务、世界知识和推理任务性能无关

Conclusion: 脑活动预测的左-右不对称性反映了LLM形式语言能力（语言模式知识）的进展，支持语言处理偏侧化与形式语言结构相关的神经科学假设
...
TL;DR: BaziQA-Benchmark是一个用于评估大语言模型符号和时序组合推理能力的标准化基准，基于200个专业策划的全球算命师竞赛多选题，要求对固定符号图表和交互时序条件进行结构化推理。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估方法多为轶事性或提示驱动的，缺乏对符号和时序组合推理能力的客观、标准化评估。需要建立一个能够进行受控比较、客观评分的基准来系统评估模型在这类复杂推理任务上的表现。

Method: 从2021-2025年全球算命师竞赛中选取200个专业策划的多选题构建基准，每个问题都涉及固定符号图表和交互时序条件。采用多轮设置评估当代语言模型，并引入轻量级结构化推理协议来约束推理顺序而不添加领域知识。

Result: 模型表现持续优于随机猜测但远未达到饱和水平，对时序组合和推理顺序表现出显著敏感性。在精确时序定位和多条件符号判断方面存在系统性失败，性能在不同时序难度、推理领域和推理协议间存在显著差异。

Conclusion: BaziQA-Benchmark为评估大语言模型的符号和时序组合推理能力提供了标准化工具，揭示了模型在这类复杂推理任务上的局限性，特别是对时序组合和推理顺序的敏感性，以及精确时序定位和多条件符号判断方面的系统性缺陷。
...
TL;DR: 构建越南语医疗代码切换语音数据集ViMedCSS，评估多种ASR模型在识别越南语中英语医疗术语的性能，发现越南语优化模型在通用段表现更好，而多语言预训练有助于识别英语插入，两者结合效果最佳。


<details>
  <summary>Details</summary>
Motivation: 越南语医疗交流中常出现英语词汇（如药名、医疗程序）的代码切换现象，这对自动语音识别系统构成挑战，尤其是低资源语言如越南语。现有ASR系统难以正确识别越南语句中的英语医疗术语，且缺乏相关基准数据集。

Method: 构建了34小时的越南语医疗代码切换语音数据集ViMedCSS，包含16,576个话语，每个话语至少包含一个来自双语词典的英语医疗术语，涵盖五个医疗主题。使用该数据集评估多个最先进的ASR模型，并研究不同的微调策略以改进医疗术语识别。

Result: 实验结果表明：越南语优化的模型在通用段表现更好，多语言预训练有助于捕捉英语插入。两种方法结合在整体准确率和代码切换准确率之间达到最佳平衡。

Conclusion: 本研究提供了首个越南语医疗代码切换基准，为低资源多语言ASR系统的有效领域适应提供了见解。
...
TL;DR: 本文介绍了首个大规模孟加拉语成语数据集（10,361个成语），采用19字段综合标注方案，并评估了30个多语言LLM的成语理解能力，发现模型准确率不足50%，远低于人类表现（83.4%），揭示了LLM在低资源语言和文化推理方面的局限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在比喻语言理解方面存在显著挑战，特别是对于低资源语言。目前缺乏高质量、文化背景丰富的孟加拉语成语资源，以及评估LLM跨语言文化推理能力的基准。

Method: 1. 构建包含10,361个孟加拉语成语的大规模数据集；2. 通过专家共识过程建立并完善包含19个字段的综合标注方案，涵盖语义、句法、文化和宗教维度；3. 评估30个最先进的多语言和指令调优LLM在推断成语比喻意义任务上的表现；4. 对比人类表现（83.4%准确率）。

Result: 1. 创建了首个大规模、文化背景丰富的孟加拉语成语数据集；2. 所有评估的LLM表现均不理想，没有模型准确率超过50%；3. 模型表现与人类表现（83.4%）存在显著差距；4. 揭示了现有模型在跨语言和文化推理方面的严重局限性。

Conclusion: 本文通过发布新的孟加拉语成语数据集和基准测试，为推进低资源语言（特别是孟加拉语）的比喻语言理解和文化基础提供了基础设施。研究结果强调了LLM在跨语言文化推理方面的不足，为未来改进方向提供了重要参考。
...
TL;DR: 该论文构建了首个大规模多标签阿拉伯语方言识别数据集，并提出LAHJATBERT模型，通过GPT-4o自动标注和课程学习策略，在MLADI任务上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语方言识别长期被建模为单标签分类任务，但实际应为多标签分类。现有研究受限于单标签数据集，缺乏大规模多标签资源。单标签数据集用于多标签任务时面临负样本选择困难，因为许多被标记为负样本的句子可能在多种方言中都可接受

Method: 1) 使用GPT-4o和二元方言可接受性分类器生成自动多标签标注，以阿拉伯语方言程度(ALDi)为指导进行聚合；2) 训练基于BERT的多标签分类器，采用与方言复杂性和标签基数对齐的课程学习策略

Result: LAHJATBERT模型在MLADI排行榜上获得0.69的宏F1分数，相比之前最强系统的0.55有显著提升。代码和数据已开源

Conclusion: 该研究成功构建了首个大规模多标签阿拉伯语方言识别数据集，并通过创新的自动标注和课程学习方法有效解决了多标签方言识别问题，为阿拉伯语方言处理提供了重要资源和方法
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速演进，静态评估方法已落后，理解LLM如何及为何失败成为核心挑战。现有自动化探测方法通常只能发现孤立的失败案例，缺乏对探索的原则性控制，且对模型弱点底层结构提供有限洞察。

Method: 提出ProbeLLM框架：1) 将探测建模为分层蒙特卡洛树搜索，在全局探索新失败区域和局部细化重复错误模式之间分配有限探测预算；2) 限制探测到可验证测试用例，利用工具增强生成和验证，将失败发现建立在可靠证据基础上；3) 通过失败感知嵌入和边界感知归纳将发现的失败整合为可解释的失败模式。

Result: 在多样化基准和LLM上，ProbeLLM揭示了比静态基准和先前自动化方法更广泛、更清晰、更细粒度的失败图景，支持从案例中心评估向原则性弱点发现的转变。

Conclusion: ProbeLLM框架通过结构化、原则性的弱点发现方法，为大语言模型评估提供了更深入、更系统的分析工具，能够揭示模型失败的根本模式和边界条件。
...
TL;DR: SciAgentGym是一个包含1780个领域特定工具的科学推理环境，SciAgentBench是分层评估套件，研究发现当前模型在复杂科学工具使用上存在瓶颈，提出的SciForge数据合成方法和SciAgent-8B模型在科学工具使用能力上超越更大规模模型。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试忽视了智能体在科学推理中协调使用工具的能力，科学推理需要整合复杂的工具集来导航领域特定知识，因此需要建立能够评估和提升科学工具使用能力的系统。

Method: 1) 构建SciAgentGym：包含1780个领域特定工具的可扩展交互环境，覆盖四个自然科学学科；2) 开发SciAgentBench：分层评估套件，从基础动作到长视野工作流测试智能体能力；3) 提出SciForge：将工具动作空间建模为依赖图的数据合成方法，生成逻辑感知的训练轨迹；4) 基于这些轨迹微调得到SciAgent-8B模型。

Result: 1) 最先进模型在复杂科学工具使用上存在瓶颈：GPT-5的成功率从60.6%下降到30.9%（随着交互视野延长）；2) SciAgent-8B在科学工具使用能力上超越了显著更大的Qwen3-VL-235B-Instruct；3) 模型展现出跨领域科学工具使用能力的正向迁移。

Conclusion: 研究揭示了当前模型在科学工具使用上的局限性，提出的SciForge方法和SciAgent-8B模型为解决这一瓶颈提供了有效方案，展示了下一代自主科学智能体的潜力。
...
TL;DR: 该论文研究了关键词预测模型的同质性评估，发现关键词抽取方法在预测同质性方面与生成模型竞争激烈，且生成缺席关键词的能力可能对同质性产生负面影响。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试未覆盖关键词预测模型的同质性评估。虽然关键词生成模型能够预测文档中未出现的"缺席关键词"，理论上应使处理相同主题的文档获得更一致的关键词预测，但这种同质性优势尚未得到验证。

Method: 引入了一种评估关键词预测模型同质性的方法，通过比较关键词抽取模型和生成模型在处理相同主题文档时的预测一致性，特别关注生成模型预测缺席关键词的能力对同质性的影响。

Result: 令人惊讶的是，关键词抽取方法在预测同质性方面与生成模型竞争激烈，生成缺席关键词的能力实际上可能对同质性产生负面影响，这与直觉预期相反。

Conclusion: 关键词生成模型预测缺席关键词的能力并不必然提高预测同质性，关键词抽取方法在保持预测一致性方面具有竞争力。研究强调了评估关键词预测模型同质性的重要性，并提供了相关数据和代码资源。
...
TL;DR: 提出了一种基于元认知框架的可靠知识增强方法，通过差异化干预和认知对齐来解决LLMs中的知识-置信度差距问题
...
Conclusion: 该元认知框架通过差异化干预和认知对齐，有效解决了知识-置信度差距问题，实现了更可靠的知识增强，使LLMs能够更准确地区分已知与未知
...
TL;DR: 研究发现现有AI模型在检测认知衰退时对英国少数民族多语使用者存在偏见，特别是南约克郡口音者更容易被误判为认知衰退更严重，这些工具尚不适用于这些人群的临床诊断。


<details>
  <summary>Details</summary>
Motivation: 英国少数民族人口占四分之一，痴呆症患病率在黑人及亚裔社区增长最快。研究旨在评估AI模型在检测多语健康英语使用者与认知受损人群时的可信度和偏见问题，确保这些工具具有临床实用性。

Method: 招募全国性单语参与者和谢菲尔德、布拉德福德四个社区中心的多语参与者。多语者除非母语英语口音外，还讲索马里语、汉语或南亚语言，并进一步分为西约克郡和南约克郡两种口音以全面测试AI工具效率。使用声学和语言特征进行分类和回归模型分析。

Result: ASR系统在各组间无显著偏见，但使用声学和语言特征的分类和回归模型对多语使用者存在偏见，尤其在记忆、流畅性和阅读任务中。使用公开DementiaBank数据集训练时偏见更明显。多语者更容易被误分类为认知衰退，南约克郡口音者更可能被误判为更严重的认知衰退。

Conclusion: 现有AI模型尽管整体性能良好，但对英国少数民族背景的多语个体存在偏见，尚不适用于这些人群的诊断用途。未来工作将开发更具普适性、偏见缓解的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有表格问答系统缺乏细粒度归因能力，即使答案正确也缺少可验证的支撑依据，这在高风险场景中限制了系统的可信度。需要一种能够提供单元格级归因的透明方法。

Method: TraceBack采用模块化多智能体框架：1) 剪枝表格至相关行和列；2) 将问题分解为语义连贯的子问题；3) 将每个答案片段与其支撑单元格对齐，捕捉显式和隐式证据。同时提出CITEBench基准和FairScore无参考评估指标。

Result: TraceBack在多个数据集和粒度上显著优于强基线方法。FairScore指标与人工判断高度一致，并能保持相对方法排名，支持可解释和可扩展的表格问答评估。

Conclusion: TraceBack通过细粒度单元格级归因提高了表格问答的透明度和可信度，CITEBench基准和FairScore指标为系统评估提供了标准化工具，推动了可解释表格问答系统的发展。
...
TL;DR: 提出基于大语言模型的新能力建模框架，将传统专家驱动方法转化为透明、数据驱动、可评估的分析过程


<details>
  <summary>Details</summary>
Motivation: 传统专家驱动能力建模方法依赖人工分析大量访谈文本，成本高且存在随机性、模糊性和可重复性有限的问题，需要更系统、可验证的解决方案

Method: 将专家实践分解为结构化计算组件：1)利用LLM从原始文本提取行为和心理描述；2)通过嵌入相似性映射到预定义能力库；3)引入可学习参数自适应整合不同信息源；4)开发离线评估程序进行系统模型选择

Result: 在软件外包公司的实际应用中展示了强大的预测效度、跨库一致性和结构稳健性

Conclusion: 该框架将能力建模从定性、专家依赖的实践转变为透明、数据驱动、可评估的分析过程，解决了传统方法的局限性
...
TL;DR: 研究通过特征选择构建可解释的机器学习模型，用于爱沙尼亚语水平考试写作（A2-C1级）的分类，准确率达0.9，并将结果应用于语言学习环境


<details>
  <summary>Details</summary>
Motivation: 自然语言处理分析真实学习者语言有助于构建自动化评估和反馈工具，并提供二语产出发展的新见解，但缺乏将这两方面明确结合的研究

Method: 分析爱沙尼亚语水平考试写作的多种语言属性，识别与复杂度、正确性相关的熟练度预测因子，使用词汇、形态、表层和错误特征训练分类模型，并与包含其他特征的模型比较

Result: 预选特征获得相似测试准确率（约0.9）但减少不同文本类型的分类变异；对早期考试样本的评估显示写作复杂度在7-10年间增加，某些特征集准确率仍达0.8

Conclusion: 精心选择的特征可构建更可解释、可推广的语言测试机器学习模型，结果已应用于爱沙尼亚开源语言学习环境的写作评估模块
...
TL;DR: SCOPE框架为LLM成对评估提供选择性判断，通过BPE不确定性信号和保形优化确保有限样本统计保证，在目标风险水平下实现高覆盖率。


<details>
  <summary>Details</summary>
Motivation: LLM作为评估者替代人工标注时存在校准不足和系统性偏差问题，需要一种具有统计保证的选择性判断框架来提高评估的可靠性。

Method: 提出SCOPE框架，结合BPE（双向偏好熵）不确定性信号，通过保形优化校准接受阈值，确保非弃权判断的错误率不超过用户指定的α水平。

Result: 在MT-Bench、RewardBench和Chatbot Arena基准测试中，BPE显著提升不确定性质量，SCOPE在所有基准和评估者规模下均满足风险界限（α=0.10时经验风险≈0.097-0.099），同时保持高覆盖率（RewardBench上达0.89-0.98）。

Conclusion: SCOPE框架通过BPE不确定性信号和保形优化，为LLM成对评估提供了可靠的统计保证，在控制风险的同时实现高覆盖率，显著优于朴素基线方法。
...
### [31] [From sunblock to softblock: Analyzing the correlates of neology in published writing and on social media](https://arxiv.org/abs/2602.13123)
*Maria Ryskina,Matthew R. Gormley,Kyle Mahowald,David R. Mortensen,Taylor Berg-Kirkpatrick,Vivek Kulkarni*

Main category: cs.CL

TL;DR: 该研究将英语新词涌现分析方法从传统出版文本扩展到社交媒体（Twitter），发现相同因素在不同领域都起作用，但主题流行度增长因素在Twitter上的贡献较小，可能源于不同领域偏好不同的新词形成机制。


<details>
  <summary>Details</summary>
Motivation: 语言演化受到多种内外压力的影响，不同社交和会话语境（如报纸与社交媒体）存在不同的约束条件。先前基于历史出版文本的新词涌现研究识别出两个相关因素，本研究旨在验证这些发现在社交媒体领域是否同样成立，并探索不同领域的新词形成机制差异。

Method: 扩展先前方法，在静态词嵌入基础上加入上下文嵌入分析，应用于Twitter帖子新语料库，比较社交媒体与传统出版文本中新词涌现的异同。

Result: 相同因素在两个领域都起作用，但主题流行度增长因素在Twitter上的贡献小于出版文本，表明不同领域可能偏好不同的新词形成机制。

Conclusion: 语言演化机制在不同领域具有一致性，但具体因素权重存在差异，反映了不同社交语境对新词形成的不同约束和偏好。
...
### [32] [OpenLID-v3: Improving the Precision of Closely Related Language Identification -- An Experience Report](https://arxiv.org/abs/2602.13139)
*Mariia Fedorova,Nikolay Arefyev,Maja Buljan,Jindřich Helcl,Stephan Oepen,Egil Rønningstad,Yves Scherrer*

Main category: cs.CL

TL;DR: OpenLID-v3是对OpenLID语言识别工具的改进版本，通过增加训练数据、合并有问题的语言变体簇、引入噪声标记标签来提升对相近语言和噪声的识别能力，特别关注低资源语言。
...
Method: 扩展OpenLID分类器：1) 增加更多训练数据；2) 合并有问题的语言变体簇；3) 引入特殊标签标记噪声。开发过程中重点关注三组相近语言：波斯尼亚语、克罗地亚语和塞尔维亚语；意大利北部和法国南部的罗曼语变体；斯堪的纳维亚语言，并贡献新的评估数据集。

Result: OpenLID-v3在多个基准测试中与GlotLID进行了评估。研究发现集成方法虽然提高了精度，但也显著降低了低资源语言的覆盖率。OpenLID-v3已在Hugging Face平台发布。

Conclusion: OpenLID-v3通过改进训练数据和引入噪声标记，提升了语言识别能力，特别是在相近语言区分和噪声过滤方面，为构建高质量多语言数据集提供了更好的工具支持。
...
TL;DR: 该论文提出了一种统计模型来解释印刷英语的熵率（约1比特/字符），通过自相似语义分层的框架定量捕捉自然语言的多尺度结构。


<details>
  <summary>Details</summary>
Motivation: 印刷英语的熵率约为1比特/字符，相对于随机文本的5比特/字符有近80%的冗余度。现代大语言模型（LLMs）最近才接近这一基准，但缺乏从第一性原理解释这种冗余水平的理论模型。

Method: 提出一个统计模型，将文本自相似地分割为语义连贯的块，直至单词级别。文本的语义结构可以分层分解，允许进行解析处理。模型通过现代LLMs和开放数据集进行数值实验验证。

Result: 模型定量捕捉了真实文本在语义层次不同层级的结构。预测的熵率与印刷英语的估计熵率一致。理论进一步揭示自然语言的熵率不是固定的，而是随语料库语义复杂性系统性地增加。

Conclusion: 该模型为自然语言的多尺度结构提供了第一性原理解释，成功解释了印刷英语的熵率，并揭示了熵率与语义复杂性之间的系统关系，为理解语言的信息结构和LLMs的性能提供了理论框架。
...
TL;DR: GT-HarmBench是一个包含2009个高风险场景的多智能体安全基准，涵盖囚徒困境、猎鹿博弈等博弈论结构，用于评估前沿AI系统在多智能体环境中的对齐表现


<details>
  <summary>Details</summary>
Motivation: 现有AI安全基准主要评估单智能体，缺乏对多智能体风险（如协调失败、冲突）的系统性评估，而前沿AI系统越来越多地部署在高风险多智能体环境中

Method: 从MIT AI风险库中提取真实AI风险场景，构建包含2009个高风险场景的基准，涵盖囚徒困境、猎鹿博弈、胆小鬼博弈等博弈论结构，评估15个前沿模型在多智能体环境中的表现

Result: 前沿模型仅能在62%的情况下选择社会有益行动，经常导致有害结果；博弈论干预可将社会有益结果提升高达18%；模型对博弈论提示框架和顺序敏感

Conclusion: 研究揭示了前沿AI系统在多智能体环境中存在重大可靠性差距，GT-HarmBench为研究多智能体环境中的对齐问题提供了标准化测试平台
...
### [35] [Scaling Web Agent Training through Automatic Data Generation and Fine-grained Evaluation](https://arxiv.org/abs/2602.12544)
*Lajanugen Logeswaran,Jaekyeom Kim,Sungryull Sohn,Creighton Glasscock,Honglak Lee*

Main category: cs.AI

TL;DR: 提出用于Web智能体的可扩展训练数据生成管道，通过约束评估框架量化任务完成进度，利用部分成功轨迹扩展可用训练数据，在BookingArena基准上超越开源方法并匹配商业系统


<details>
  <summary>Details</summary>
Motivation: 解决Web智能体训练数据质量评估的挑战，特别是轨迹评估问题——如何量化任务完成进度，以及如何高效创建多样、真实的网页交互数据集

Method: 1) 提出约束评估框架，提供任务完成进度的细粒度评估；2) 利用部分成功轨迹扩展可用训练数据；3) 构建BookingArena基准，包含20个流行网站的复杂预订任务；4) 通过蒸馏学生模型实现高效训练

Result: 1) 蒸馏学生模型在BookingArena基准上超越开源方法；2) 匹配或超过商业系统性能；3) 模型规模显著更小；4) 显著扩展了可用训练数据量

Conclusion: 该工作解决了高效创建多样、真实网页交互数据集的挑战，为复杂结构化Web任务提供了系统化评估方法，通过约束评估框架和部分成功轨迹利用，实现了高质量训练数据的可扩展生成
...
TL;DR: 该论文研究了在多领域强化学习中验证奖励（RLVR）的两种训练范式：混合多任务训练与分别训练后模型合并，发现在不同领域间RLVR存在较少相互干扰，且推理密集型领域表现出相互协同效应。


<details>
  <summary>Details</summary>
Motivation: 当前在多领域专家级模型训练中，主要采用混合多任务RLVR和分别RLVR后模型合并两种范式，但缺乏对这些范式的详细比较分析。需要系统研究不同领域间RLVR的协作效果和内在机制。

Method: 选择数学、编程、科学和指令跟随等多个常用高级任务作为目标领域，使用开源数据集设计广泛的定性和定量实验，从权重空间几何、模型预测行为和信息约束等角度分析互惠增益的内在机制。

Result: 发现不同领域间的RLVR表现出较少的相互干扰，推理密集型领域（如数学和编程）显示出相互协同效应。通过权重空间几何等分析揭示了互惠增益的内在机制。

Conclusion: 该研究为多领域RLVR训练提供了实证分析，表明混合训练和分别训练后合并两种范式在不同领域间具有兼容性，推理密集型领域的协同效应为多领域专家模型训练提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型中的计划-填充解码在数学和代码推理任务中表现出潜力，但其性能对槽位填充顺序高度敏感，导致输出方差较大。现有方法缺乏系统性的顺序优化策略。

Method: 提出McDiffuSE框架，将槽位选择建模为决策问题，使用蒙特卡洛树搜索优化填充顺序。通过前瞻模拟评估部分完成情况，系统探索生成顺序的组合空间。

Result: 实验显示平均比自回归基线提升3.2%，比基线计划-填充方法提升8.0%。在MBPP上获得19.5%的显著提升，在MATH500上提升4.9%。分析表明需要结合非顺序生成来最大化性能。

Conclusion: 基于MCTS的规划是提升掩码扩散模型生成质量的有效方法。研究发现较大的探索常数比增加模拟次数更能克服模型置信度偏差，发现有效的生成顺序。
...
### [38] [GeoAgent: Learning to Geolocate Everywhere with Reinforced Geographic Characteristics](https://arxiv.org/abs/2602.12617)
*Modi Jin,Yiming Zhang,Boyuan Sun,Dingwen Zhang,MingMing Cheng,Qibin Hou*

Main category: cs.AI

TL;DR: GeoAgent是一个能够与人类紧密推理并得出细粒度地址结论的模型，通过专家标注的地理定位数据集和地理相似性奖励解决现有RL方法依赖AI生成思维链数据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的方法虽然在性能和可解释性方面取得了突破，但由于依赖AI生成的思维链数据和训练策略，与地理特性存在冲突，需要解决这些问题。

Method: 首先引入GeoSeek数据集（包含地理专家和专业玩家标注的思维链数据），然后深入探索地理任务的内在特性，提出地理相似性奖励和一致性奖励（通过一致性智能体评估），以辅助训练。

Result: 实验结果表明，GeoAgent在多个粒度上优于现有方法和一系列通用VLLMs，同时生成的推理过程与人类思维紧密契合。

Conclusion: GeoAgent通过专家标注数据和地理感知奖励机制，成功解决了现有方法在地理任务中的局限性，实现了更准确、更符合地理特性的地址推理。
...
TL;DR: 论文提出了一个用于评估LLM推理器的诊断性2-SAT基准，通过参数化公式家族分离表面难度与结构现象，揭示传统SAT基准无法检测的脆弱性模式。


<details>
  <summary>Details</summary>
Motivation: 传统SAT基准常将表面难度（长度、措辞、子句顺序）与决定可满足性的结构现象混为一谈，无法有效诊断LLM推理器的具体能力和失败模式。

Method: 构建基于参数化结构化2-CNF公式的诊断基准，通过五种生成器隔离不同能力：矛盾循环UNSAT核心、控制解多性的SAT实例、植入骨干变量、晚期桥接子句、对称/重复变体，并测试语义保持扰动下的鲁棒性。

Result: 评估显示LLM推理器在目标结构干预下表现出急剧的性能转变，即使表面统计特征保持不变，揭示了传统聚合SAT准确率无法检测的脆弱性区域。

Conclusion: 该诊断基准能够揭示LLM推理器在特定结构现象上的能力和局限，为更精细的推理能力评估提供了有效工具，超越了传统SAT基准的表面评估。
...
### [40] [SkillsBench: Benchmarking How Well Agent Skills Work Across Diverse Tasks](https://arxiv.org/abs/2602.12670)
...
TL;DR: SkillsBench基准测试评估Agent Skills对LLM智能体的影响，发现精心设计的Skills能平均提升16.2个百分点的通过率，但效果因领域差异很大；模型无法可靠地生成自己受益的Skills；小而专注的Skills优于全面文档；小模型搭配Skills可匹敌无Skills的大模型。


<details>
  <summary>Details</summary>
Motivation: 尽管Agent Skills作为增强LLM智能体的程序知识包被广泛采用，但缺乏标准方法来衡量它们是否真正有效。需要建立系统的评估基准来量化Skills对智能体性能的实际影响。

Method: 提出了SkillsBench基准，包含11个领域的86个任务，每个任务配有精心设计的Skills和确定性验证器。在三种条件下评估：无Skills、精心设计的Skills、自生成的Skills。测试了7种智能体-模型配置，共7,308条轨迹。

Result: 精心设计的Skills平均提升通过率16.2个百分点，但效果差异显著：软件工程领域仅提升4.5个百分点，医疗领域提升51.9个百分点；84个任务中有16个出现负增长。自生成Skills平均无益处。小而专注的Skills（2-3个模块）优于全面文档。小模型搭配Skills可匹敌无Skills的大模型。

Conclusion: Agent Skills能显著提升LLM智能体性能，但效果高度依赖于领域和任务特性。模型无法可靠地生成自己受益的Skills，表明程序知识的创作与消费是不同的能力。小而专注的Skills设计策略更有效，且Skills能缩小模型规模差距。
...
### [41] [X-SYS: A Reference Architecture for Interactive Explanation Systems](https://arxiv.org/abs/2602.12748)
*Tobias Labarta,Nhi Hoang,Maximilian Dreyer,Jim Berend,Oleg Hein,Jackie Ma,Wojciech Samek,Sebastian Lapuschkin*

Main category: cs.AI

TL;DR: 提出X-SYS参考架构，将可解释AI系统化，通过STAR质量属性和五组件分解连接交互界面与系统能力，并在SemanticLens中实现验证。
...
Method: 引入X-SYS参考架构，围绕STAR四个质量属性（可扩展性、可追溯性、响应性和适应性）组织，并指定五组件分解（XUI服务、解释服务、模型服务、数据服务、编排与治理）。该架构将交互模式映射到系统能力，以解耦用户界面演进与后端计算。通过SemanticLens系统实现X-SYS，该系统用于视觉语言模型的语义搜索和激活引导。

Result: X-SYS提供了一个可重用的蓝图，SemanticLens作为具体实例展示了基于契约的服务边界如何支持独立演进、离线/在线分离确保响应性、持久状态管理支持可追溯性。该工作为在操作约束下支持端到端设计的交互式解释系统提供了架构指导和实际实现。

Conclusion: 将可解释AI视为信息系统问题，通过X-SYS参考架构连接交互界面与系统能力，为交互式解释系统提供了系统化解决方案，支持在重复查询、模型演化和治理约束下的实际部署。
...
TL;DR: 最优策略包含关于环境的确切信息量：在n个状态、m个动作的受控马尔可夫过程中，最优策略提供n log m比特的环境信息
...
Method: 采用受控马尔可夫过程模型，假设转移动态空间上的均匀先验，分析确定性最优策略与环境的互信息

Result: 证明对于任何非常数奖励函数的最优策略，其与环境的互信息恰好为n log m比特，该界限适用于多种目标函数

Conclusion: 为最优性所需的"隐式世界模型"提供了精确的信息论下界，表明最优策略必然编码环境的结构信息
...
TL;DR: 该研究评估了9个前沿推理模型在对抗攻击下的鲁棒性，发现推理能力提供有意义但不完整的鲁棒性，所有模型都表现出不同的脆弱性模式，并识别出5种主要失效模式。


<details>
  <summary>Details</summary>
Motivation: 尽管具有推理能力的大型模型在复杂任务上取得了最先进的性能，但它们在多轮对抗压力下的鲁棒性尚未得到充分探索。研究旨在评估推理模型在对抗攻击下的表现，了解推理能力是否自动带来对抗鲁棒性。

Method: 研究评估了9个前沿推理模型在对抗攻击下的表现，包括误导性建议和社会压力等攻击方式。通过轨迹分析识别失效模式，并测试了置信度感知响应生成（CARG）防御方法在推理模型上的有效性。

Result: 研究发现：1）推理提供有意义但不完整的鲁棒性，大多数推理模型显著优于指令调优基线；2）所有模型都表现出不同的脆弱性模式，误导性建议普遍有效，社会压力具有模型特异性；3）识别出5种失效模式（自我怀疑、社会顺从、建议劫持、情感易感性、推理疲劳），前两种占50%的失败；4）CARG防御对推理模型失效，因为扩展推理轨迹导致过度自信；5）随机置信度嵌入优于针对性提取。

Conclusion: 推理能力不会自动带来对抗鲁棒性，基于置信度的防御方法需要对推理模型进行根本性重新设计。研究强调了理解推理模型脆弱性模式的重要性，并为开发更鲁棒的推理系统提供了方向。
...
TL;DR: Hi-SAM提出分层结构感知多模态推荐框架，通过解耦语义分词器和分层记忆锚点Transformer解决现有语义ID方法中的冗余和层次结构缺失问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推荐中语义ID方法存在两个关键问题：1）分词器（如RQ-VAE）无法解耦跨模态共享语义和模态特定细节，导致冗余或坍缩；2）标准Transformer将语义ID视为扁平序列，忽略了用户交互、物品和token的层次结构，多token扩展会引入噪声并偏向局部细节而非整体语义。

Method: Hi-SAM包含两个核心设计：1）解耦语义分词器（DST）：通过几何感知对齐统一多模态表示，采用粗到细策略量化，共享码本提取共识语义，模态特定码本从残差中恢复细节，通过互信息最小化强制解耦；2）分层记忆锚点Transformer（HMAT）：通过分层RoPE将位置编码分解为物品间和物品内子空间以恢复层次结构，插入锚点token将物品压缩为紧凑记忆，保留当前物品细节而仅通过压缩摘要访问历史。

Result: 在真实世界数据集上相比SOTA基线获得一致改进，尤其在冷启动场景表现优异。部署于服务数百万用户的大规模社交平台后，核心在线指标提升6.55%。

Conclusion: Hi-SAM通过解耦语义表示和分层建模有效解决了多模态推荐中的语义冗余和结构失配问题，在实际部署中验证了其有效性，特别适合冷启动场景。
...
### [45] [Constrained Assumption-Based Argumentation Frameworks](https://arxiv.org/abs/2602.13135)
*Emanuele De Angelis,Fabio Fioravanti,Maria Chiara Meo,Alberto Pettorossi,Maurizio Proietti,Francesca Toni*

Main category: cs.AI

TL;DR: 提出约束ABA框架，通过引入约束变量扩展传统ABA的表达能力，支持非地面论证和攻击，并保持对标准ABA语义的保守泛化。


<details>
  <summary>Details</summary>
Motivation: 传统基于假设的论证框架受限于地面（无变量）论证和基于命题原子的攻击，表达能力有限。需要扩展ABA框架以支持包含约束变量的非地面论证，从而增强其表示能力。

Method: 提出约束ABA框架，允许框架组件和论证包含约束变量，这些变量可以取无限域中的值。定义了基于各种非地面攻击概念的非地面语义。

Result: 新提出的约束ABA框架能够表示更复杂的论证结构，支持非地面论证和攻击。证明了新语义是标准ABA语义的保守泛化，即标准ABA是约束ABA的特例。

Conclusion: 约束ABA框架成功扩展了传统ABA的表达能力，通过引入约束变量支持非地面论证，同时保持了与标准ABA语义的兼容性，为更复杂的论证场景提供了理论基础。
...
### [46] [Optimal Take-off under Fuzzy Clearances](https://arxiv.org/abs/2602.13166)
*Hugo Henry,Arthur Tsai,Kelly Cohen*

Main category: cs.AI

TL;DR: 提出一种混合障碍物规避架构，将最优控制与模糊规则系统结合，通过模糊层调节约束参数，为无人机提供自适应约束处理能力。


<details>
  <summary>Details</summary>
Motivation: 传统最优控制在不确定性下的局限性，以及航空安全关键系统需要可解释决策的需求，促使开发能自适应处理约束的混合架构。

Method: 采用三层Takagi-Sugeno-Kang模糊系统，基于FAA和EASA的分离标准调节约束半径、紧急程度和激活决策，将模糊生成的间隙作为软约束纳入最优控制问题，使用FALCON工具箱和IPOPT求解器。

Result: 在简化飞机模型上实现概念验证，每次迭代计算时间2-3秒，显示近实时应用可行性。但发现FALCON和IPOPT最新版本存在软件不兼容问题，拉格朗日惩罚项恒为零，导致约束无法正确执行。

Conclusion: 混合架构展示了潜在可行性，但需解决软件兼容性问题。未来工作包括验证早期软件版本、优化模糊隶属函数、扩展到高保真飞机模型和随机障碍环境。
...
### [47] [An Industrial-Scale Sequential Recommender for LinkedIn Feed Ranking](https://arxiv.org/abs/2602.12354)
...
TL;DR: Feed-SR是一个基于Transformer的顺序推荐模型，用于LinkedIn Feed，替代了原有的DCNv2排序器，在满足严格生产约束的同时显著提升了用户参与度。


<details>
  <summary>Details</summary>
Motivation: LinkedIn Feed需要为全球专业人士提供相关内容发现、建立联系和知识分享。现有DCNv2排序器存在改进空间，需要开发更先进的顺序推荐模型来提升用户体验和参与度，同时满足大规模生产环境的效率要求。

Method: 采用Transformer架构的顺序排序模型（Feed-SR），详细设计了建模选择、训练技术和服务优化策略，使其能够在LinkedIn规模下部署。同时探索了其他顺序模型和基于LLM的排序架构作为对比。

Result: Feed-SR已成为LinkedIn Feed的主要成员体验，在线A/B测试显示相比原有生产模型，用户参与度显著提升（时间花费增加2.10%）。与其他顺序模型和LLM架构相比，Feed-SR在在线指标和生产效率方面取得了最佳平衡。

Conclusion: Feed-SR成功部署为LinkedIn Feed的主要排序模型，证明了Transformer顺序推荐模型在大规模生产环境中的有效性。该模型在提升用户参与度的同时满足了严格的生产约束，为工业级推荐系统提供了有价值的实践经验。
...
### [48] [Visual RAG Toolkit: Scaling Multi-Vector Visual Retrieval with Training-Free Pooling and Multi-Stage Search](https://arxiv.org/abs/2602.12510)
*Ara Yeroyan*

Main category: cs.IR

TL;DR: Visual RAG Toolkit：通过训练无关的模型感知池化和多阶段检索，显著降低视觉多向量检索的计算开销，实现二次方级向量比较减少，保持检索精度同时提升4倍吞吐量。


<details>
  <summary>Details</summary>
Motivation: 多向量视觉检索器（如ColPali风格的延迟交互模型）虽然精度高，但扩展性差，每页产生数千个向量导致索引和搜索成本高昂。需要一种实用的系统来扩展视觉多向量检索，同时保持效率。

Method: 提出Visual RAG Toolkit系统，采用训练无关的模型感知池化和多阶段检索。基于Matryoshka Embeddings思想，通过静态空间池化（包括轻量级滑动窗口平均变体）对补丁嵌入进行处理，生成紧凑的瓦片级和全局表示用于快速候选生成，然后使用完整多向量嵌入进行精确的MaxSim重排序。

Result: 系统将每页存储向量从数千个减少到数十个，实现向量到向量比较的二次方级减少。在ViDoRe v2基准测试中，两阶段检索通常能保持NDCG和Recall @ 5/10指标，仅有轻微下降，同时显著提升吞吐量（约4倍QPS）。主要敏感度出现在非常大的k值时。

Conclusion: Visual RAG Toolkit通过强调常见截断点（k ≤ 10）的效率，降低了硬件门槛，使最先进的视觉检索在实际应用中更加可访问。系统还提供强大的预处理功能和可复现的评估流程。
...
### [49] [DiffuRank: Effective Document Reranking with Diffusion Language Models](https://arxiv.org/abs/2602.12528)
*Qi Liu,Kun Ai,Jiaxin Mao,Yanzhao Zhang,Mingxin Li,Dingkun Long,Pengjun Xie,Fengbin Zhu,Ji-Rong Wen*

Main category: cs.IR

TL;DR: DiffuRank：基于扩散语言模型的文档重排框架，通过并行解码和灵活生成顺序解决自回归模型效率低、错误传播问题


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的重排方法依赖自回归生成，存在效率低（逐token解码延迟高）、灵活性差（固定从左到右顺序导致早期错误传播且难以修正）的问题

Method: 提出DiffuRank框架，基于扩散语言模型(dLLMs)实现三种重排策略：1)点式方法评估单个查询-文档对相关性；2)基于logit的列表方法联合评估多个文档相关性；3)基于排列的列表方法适配dLLMs标准解码过程到重排任务

Result: 在多个基准测试中，dLLMs在零样本和微调设置下达到与相似规模自回归LLMs相当甚至更好的性能，证明扩散模型作为自回归架构替代方案的潜力

Conclusion: 扩散语言模型为文档重排提供了有前景的替代方案，支持更灵活的生成过程和并行解码，在效率和可控性方面具有优势
...
### [50] [Reasoning to Rank: An End-to-End Solution for Exploiting Large Language Models for Recommendation](https://arxiv.org/abs/2602.12530)
*Kehan Zheng,Deyao Hong,Qian Li,Jun Zhang,Huan Yu,Jie Jiang,Hongning Wang*

Main category: cs.IR

TL;DR: 提出Reasoning to Rank框架，通过强化学习端到端训练LLM进行逐步推理，优化推荐效用


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统需要超越模式匹配的深度推理能力，但如何有效优化LLM以提升推荐效用仍待探索

Method: 提出Reasoning to Rank端到端训练框架，在用户-物品级别进行推理，采用强化学习端到端训练LLM，避免位置偏差并直接优化推理过程

Result: 在三个Amazon数据集和一个大规模工业数据集上实验，相比传统方法和现有LLM方案均取得一致提升

Conclusion: 框架关键组件验证有效，为基于LLM的推荐系统未来发展提供了方向
...
### [51] [CAPTS: Channel-Aware, Preference-Aligned Trigger Selection for Multi-Channel Item-to-Item Retrieval](https://arxiv.org/abs/2602.12564)
*Xiaoyou Zhou,Yuqi Liu,Zhao Liu,Xiao Lv,Bo Chen,Ruiming Tang,Guorui Zhou*

Main category: cs.IR

TL;DR: CAPTS框架解决多通道检索中触发选择的价值归因偏差和通道间协调问题，通过前瞻性监督和自适应路由提升推荐系统效果


<details>
  <summary>Details</summary>
Motivation: 现有工业推荐系统在多通道检索中面临两个主要挑战：1）价值归因偏差，仅基于触发本身的反馈而非其作为检索种子的下游效用；2）多通道路由不协调，各通道独立选择触发导致跨通道重叠增加

Method: 提出CAPTS框架，包含两个核心模块：1）价值归因模块（VAM），通过前瞻性监督为每个触发分配下游I2I通道检索产生的后续参与度；2）通道自适应触发路由（CATR）模块，协调触发到通道的分配以最大化多通道检索的整体价值

Result: 在快手国际版Kwai平台上的大规模离线实验和在线A/B测试表明，CAPTS持续提升多通道召回率，在线测试中平均设备使用时长提升+0.351%

Conclusion: CAPTS框架将多通道触发选择视为可学习的路由问题，通过统一灵活的方法解决了价值归因偏差和通道协调问题，显著提升了工业推荐系统的性能
...
### [52] [RQ-GMM: Residual Quantized Gaussian Mixture Model for Multimodal Semantic Discretization in CTR Prediction](https://arxiv.org/abs/2602.12593)
*Ziye Tong,Jiahao Liu,Weimin Zhang,Hongji Ruan,Derick Tang,Zhanpeng Zeng,Qinsong Zeng,Peng Zhang,Tun Lu,Ning Gu*

Main category: cs.IR

TL;DR: RQ-GMM提出了一种用于CTR预测的多模态嵌入离散化方法，通过残差量化高斯混合模型解决现有方法在码本利用率、重建精度和语义区分性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 多模态内容对CTR预测至关重要，但直接将预训练模型的连续嵌入输入CTR模型效果不佳，因为优化目标和收敛速度不一致。现有离散化方法存在码本利用率低、重建精度差和语义区分性不足的问题。

Method: 提出RQ-GMM（残差量化高斯混合模型），结合高斯混合模型和残差量化来更好地捕捉多模态嵌入空间的统计结构，提高码本利用率和重建精度。

Result: 在公开数据集和大型短视频平台的在线A/B测试中，RQ-GMM相比强基线在广告主价值上获得1.502%的提升。该方法已全面部署，为数亿用户提供每日推荐服务。

Conclusion: RQ-GMM通过概率建模有效解决了多模态嵌入离散化问题，显著提升了CTR预测性能，已在工业级推荐系统中成功应用。
...
### [53] [Self-EvolveRec: Self-Evolving Recommender Systems with LLM-based Directional Feedback](https://arxiv.org/abs/2602.12612)
*Sein Kim,Sangwu Park,Hongseok Kang,Wonjoong Kim,Jimin Seo,Yeonjun In,Kanghoon Yoon,Chanyoung Park*

Main category: cs.IR

TL;DR: Self-EvolveRec是一个通过用户模拟器和模型诊断工具建立定向反馈循环的推荐系统自动化设计框架，解决了传统方法依赖固定搜索空间和标量指标的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统自动化设计方法（如NAS）受限于人类先验定义的固定搜索空间，限制了创新性。而最近的LLM驱动代码进化框架虽然转向开放程序空间，但主要依赖NDCG、Hit Ratio等标量指标，无法提供模型失败的定性洞察或改进的方向性指导。

Method: 提出Self-EvolveRec框架：1）集成用户模拟器进行定性批判；2）模型诊断工具进行定量内部验证，建立定向反馈循环；3）引入诊断工具-模型协同进化策略，确保评估标准随推荐架构演化而动态适应。

Result: 大量实验表明，Self-EvolveRec在推荐性能和用户满意度方面显著优于最先进的NAS和LLM驱动代码进化基线方法。

Conclusion: Self-EvolveRec通过建立定向反馈循环和动态适应的评估标准，有效解决了传统推荐系统自动化设计方法的局限性，实现了更好的推荐性能和用户满意度。
...
### [54] [Training Dense Retrievers with Multiple Positive Passages](https://arxiv.org/abs/2602.12727)
...
<details>
  <summary>Details</summary>
Motivation: 现代知识密集型系统（如RAG）依赖检索器性能，但传统训练受限于稀疏的单正例标注，导致假阴性噪声和次优监督。虽然LLM可以大规模收集密集的多正例相关性标签，但如何将这些密集信号有效融入训练仍不明确。

Method: 提出统一框架分析多正例优化目标：联合似然（JointLH）、求和边际似然（SumMargLH）和对数求和指数配对（LSEPair）损失。理论分析梯度行为，实证评估在Natural Questions、MS MARCO和BEIR基准上的表现，涵盖同质LLM标注数据和异质混合标注两种现实场景。

Result: LSEPair在所有设置中均表现出优越的鲁棒性和性能，而JointLH和SumMargLH对正例质量高度敏感。随机采样（Rand1LH）作为可靠基线。理论洞察与实证结果一致，为利用密集LLM增强监督提升检索器效果提供实用设计原则。

Conclusion: 通过系统研究多正例优化目标，揭示了不同损失函数在密集监督下的行为差异，为检索器训练提供了基于LLM增强标注的有效策略，特别是推荐使用LSEPair损失以获得最佳鲁棒性和性能。
...
### [55] [SQuTR: A Robustness Benchmark for Spoken Query to Text Retrieval under Acoustic Noise](https://arxiv.org/abs/2602.12783)
...
Method: 构建SQuTR基准，聚合来自6个常用英文和中文文本检索数据集的37,317个独特查询，使用200个真实说话者的语音配置文件合成语音，并在可控信噪比下混合17类真实环境噪声

Result: 实验结果显示检索性能随噪声增加而下降，不同系统性能下降程度差异显著，即使大规模检索模型在极端噪声下也表现不佳，表明鲁棒性仍是关键瓶颈

Conclusion: SQuTR为口语查询到文本检索的鲁棒性提供了可复现的测试平台和诊断分析工具，有助于推动该领域未来的鲁棒性研究
...
### [56] [WISE: A Multimodal Search Engine for Visual Scenes, Audio, Objects, Faces, Speech, and Metadata](https://arxiv.org/abs/2602.12819)
*Prasanna Sridhar,Horace Lee,David M. S. Pinto,Andrew Zisserman,Abhishek Dutta*

Main category: cs.IR

TL;DR: WISE是一个开源的多模态音视频搜索引擎，集成了多种检索功能，支持自然语言查询、反向图像搜索、人脸识别、音频检索等，采用向量搜索技术，可扩展至百万级图像和数千小时视频。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个集成多种多模态检索能力的实用工具，特别是对于没有机器学习专业知识的用户。需要开发一个能够支持图像、视频、音频、文本等多种模态查询的统一搜索引擎，并能够处理大规模数据集。

Method: 采用向量搜索技术实现高效检索，支持场景级和对象级的自然语言和反向图像查询、人脸识别、音频检索（文本或音频文件）、自动转录语音搜索以及用户提供的元数据过滤。采用模块化架构便于集成新模型。

Result: WISE能够扩展到支持数百万张图像或数千小时视频的高效检索，已在多个实际用例中应用。系统可本地部署处理私有或敏感数据，代码已开源。

Conclusion: WISE是一个功能全面、可扩展的开源多模态搜索引擎，为非专业用户提供了强大的音视频检索能力，其模块化设计和本地部署选项使其适用于各种实际应用场景。
...
### [57] [JARVIS: An Evidence-Grounded Retrieval System for Interpretable Deceptive Reviews Adjudication](https://arxiv.org/abs/2602.12941)
...
Method: 提出JARVIS框架：从待评估评论出发，通过混合稠密-稀疏多模态检索获取语义相似证据，通过共享实体扩展关系信号，构建异质证据图，利用大语言模型进行基于证据的裁决

Result: 离线实验中，在构建的评论数据集上，精确率从0.953提升至0.988，召回率从0.830提升至0.901；生产环境中，召回量增加27%，人工检查时间减少75%，模型生成分析采纳率达96.4%

Conclusion: JARVIS框架有效解决了虚假评论检测中的泛化能力和可解释性问题，在离线实验和生产环境中均表现出显著性能提升
...
### [58] [RGAlign-Rec: Ranking-Guided Alignment for Latent Query Reasoning in Recommendation Systems](https://arxiv.org/abs/2602.12968)
*Junhua Liu,Yang Jihao,Cheng Chang,Kunrong LI,Bin Fu,Kwan Hui Lim*

Main category: cs.IR

TL;DR: RGAlign-Rec：一种用于电商聊天机器人主动意图预测的闭环对齐框架，通过LLM语义推理器与查询增强排序模型集成，利用排序信号反馈优化LLM推理，显著提升预测准确率和服务质量。


<details>
  <summary>Details</summary>
Motivation: 现有工业系统面临两大挑战：1）离散用户特征与聊天机器人知识库中语义意图之间的语义鸿沟；2）通用LLM输出与任务特定排序效用之间的目标错位。需要解决这些对齐问题以提升主动推荐效果。

Method: 提出RGAlign-Rec框架：集成LLM语义推理器和查询增强排序模型。引入排序引导对齐训练范式，利用下游排序信号作为反馈来优化LLM的潜在推理过程，实现语义推理与排序目标的对齐。
...
Conclusion: 排序感知对齐能有效同步语义推理与排序目标，显著提升现实世界主动推荐系统的预测准确率和服务质量。RGAlign-Rec框架为解决工业推荐系统中的语义鸿沟和目标错位问题提供了有效方案。
...
### [59] [Asynchronous Verified Semantic Caching for Tiered LLM Architectures](https://arxiv.org/abs/2602.13165)
*Asmit Kumar Singh,Haozhe Wang,Laxmi Naga Santosh Attaluri,Tak Chiam,Weihua Zhu*

Main category: cs.IR

TL;DR: Krites是一种异步LLM判定的语义缓存策略，通过LLM法官异步验证接近静态阈值的查询，将验证通过的静态响应提升到动态缓存中，扩展静态缓存的覆盖范围而不影响关键路径延迟。


<details>
  <summary>Details</summary>
Motivation: 当前生产部署中的分层静态-动态缓存设计通常使用单一的嵌入相似度阈值，导致保守阈值会错过安全重用机会，而激进阈值则可能提供语义错误的响应。需要一种方法能够在不改变服务决策的情况下扩展静态缓存的覆盖范围。

Method: Krites采用异步LLM判定的缓存策略：在关键路径上，它表现得像标准的静态阈值策略；当查询的最近静态邻居略低于静态阈值时，异步调用LLM法官验证静态响应是否适用于新查询；验证通过的匹配项被提升到动态缓存中，允许未来的重复和转述查询重用经过审查的静态答案。

Result: 在对话和搜索工作负载的跟踪驱动模拟中，Krites将使用经过审查的静态答案服务的请求比例（直接静态命中加验证提升）提高了最多3.9倍，同时关键路径延迟保持不变。

Conclusion: Krites通过异步LLM验证机制有效扩展了静态缓存的覆盖范围，在保持关键路径延迟不变的情况下显著提高了缓存命中率，解决了传统单一阈值策略中保守与激进之间的权衡问题。
