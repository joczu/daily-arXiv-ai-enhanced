...
TL;DR: Aletheia：基于RAG和LLM的浏览器扩展，通过证据解释、用户讨论和事实核查功能检测假新闻


<details>
  <summary>Details</summary>
Motivation: 数字时代假新闻泛滥威胁公众信任和民主制度，现有浏览器扩展存在模型行为不透明、解释支持有限、用户参与度不足等问题

Method: 开发Aletheia浏览器扩展，结合检索增强生成（RAG）和大语言模型（LLM）检测假新闻并提供基于证据的解释；包含讨论中心和"保持知情"两个交互组件

Result: Aletheia在检测性能上优于最先进的基线方法；用户研究（250名参与者）证实了系统的可用性和感知有效性

Conclusion: Aletheia作为一种透明工具，在打击在线假新闻方面具有潜力，通过技术性能和用户体验的双重验证
...
TL;DR: 论文研究语法约束解码(GCD)，证明语言等价语法产生相同token掩码但不同状态空间和歧义成本，建立解码成本等价类和最小歧义成本语法，推导硬掩码解码的KL散度和总变差失真界限。


<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码中语法等价性与解码效率的关系，揭示虽然语言等价语法产生相同的token掩码，但编译后的状态空间和在线歧义成本存在显著差异，需要建立理论框架来理解和优化解码成本。

Method: 将GCD建模为自回归下一个token分布与下推系统可达性预言机的耦合，提出结构歧义成本(SAC)度量每个token的增量打包解析森林增长，建立预言机不变性定理，分析语法重写对状态空间膨胀的影响。

Result: 证明语言等价语法产生相同可接受下一个token集但不同状态空间；给出a^n b^n语言在冗余非终结符委托下的精确状态膨胀计数；建立任何在线掩码引擎在特定CFG族上必须承受Ω(t²)每token工作的下界；定义解码成本等价类并证明存在最小SAC代表。

Conclusion: 语法约束解码的效率不仅取决于语言等价性，更受语法结构影响；通过最小化结构歧义成本可以显著提升解码性能；理论结果为自动化语法优化和高效GCD引擎设计提供了理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法缺乏可解释性，用户难以理解检测决策背后的依据。需要开发一个既能准确检测又能提供透明解释的系统，帮助用户理解AI生成文本的识别过程。

Method: 扩展Fast-DetectGPT框架，整合曲率信号、神经特征和文体特征共17个可解释特征，使用XGBoost梯度提升树作为元分类器，应用SHAP进行特征归因，并通过LLM解释层生成结构化自然语言解释。

Result: 开发了NOTAI.AI系统，部署为交互式Web应用，支持实时分析、可视化特征检查和结构化证据呈现。系统公开了源代码和演示视频以确保可复现性。

Conclusion: NOTAI.AI提供了一个可解释的机器生成文本检测框架，通过多特征整合和透明解释机制，增强了检测系统的可信度和用户理解，为AI生成文本检测提供了实用的可解释解决方案。
...
TL;DR: CoT提示提升LLM推理但增加PII泄露风险，研究提出模型无关框架量化泄露，发现泄露与模型家族和推理预算相关，并评估多种轻量级门控方法。


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought提示虽然能提升大语言模型的推理能力，但会增加隐私风险，可能将提示中的个人身份信息泄露到推理轨迹和输出中，即使模型被指示不要重复PII。需要系统研究这种推理时的直接PII泄露问题。

Method: 提出模型无关框架：1) 将泄露定义为跨11种PII类型的风险加权、令牌级事件；2) 追踪泄露曲线作为允许CoT预算的函数；3) 在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。评估四种轻量级推理时门控方法：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: CoT持续增加泄露风险，特别是高风险类别；泄露强烈依赖于模型家族和推理预算；增加推理预算可能放大或减弱泄露，取决于基础模型。没有单一门控方法在所有模型或预算上表现最优，需要混合、风格自适应的门控策略。

Conclusion: CoT提示确实增加PII泄露风险，泄露模式因模型和推理预算而异。需要开发混合、自适应的门控策略来平衡效用和风险，并建立可复现的通用协议。
...
TL;DR: 研究通过扰动框架测试LLM道德判断的稳定性与可操纵性，发现视角转换比表面编辑更易导致判断翻转，协议选择是影响判断一致性的主导因素。


<details>
  <summary>Details</summary>
Motivation: 随着人们越来越多地使用LLM获取道德和人际关系指导，但这些系统无法审视缺失的上下文，且对呈现方式的依赖性可能导致判断不稳定和可操纵，存在公平性和可复现性担忧。

Method: 使用2,939个来自r/AmItheAsshole的道德困境案例，构建三类内容扰动：表面编辑（词汇/结构噪声）、视角转换（声音和立场中立化）、说服线索（自我定位、社会证明、模式承认、受害者框架）。同时改变评估协议（输出排序、指令放置、非结构化提示）。使用四个模型（GPT-4.1、Claude 3.7 Sonnet、DeepSeek V3、Qwen2.5-72B）进行评估，共129,156个判断。

Result: 表面扰动产生低翻转率（7.5%），在自一致性噪声范围内（4-13%）；视角转换导致显著更高的不稳定性（24.3%）。37.9%的困境对表面噪声稳健但在视角变化下翻转。不稳定性集中在道德模糊案例中，无责任方的情景最易受影响。说服扰动产生系统性方向性偏移。协议选择是主导因素：结构化协议间一致性仅67.6%（kappa=0.55），仅35.7%的模型-情景单元在所有三种协议中匹配。

Conclusion: LLM道德判断由叙事形式和任务框架共同塑造，当结果依赖于呈现技巧而非道德实质时，会引发可复现性和公平性问题。这表明当前LLM的道德判断系统存在结构性问题。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个降低技术门槛的多语言文本分析工具，特别是支持越南语这种在NLP中代表性不足但广泛使用的语言，促进越南语语言资源的开发。

Method: 设计统一的双语NLP流水线，集成混合VnCoreNLP和BPE分词策略、微调的TabularisAI情感分类器和微调的Qwen2.5模型进行抽象摘要。系统结合语料库分析功能（如索引、关键词分析、词关系探索）和基于Transformer的NLP组件。

Result: 通过分词、情感分析和摘要三个部分的评估，显示该方法在越南语和英语上都取得了与广泛使用的基线模型相当或更优的性能。

Conclusion: FreeTxt-Vi通过降低多语言文本分析的技术障碍，支持可重复性研究，促进越南语语言资源开发，适用于教育、数字人文、文化遗产和社会科学等领域的大规模文本处理。
...
TL;DR: 使用RGB基准评估RAG系统在噪声、信息整合、否定拒绝和反事实场景下的鲁棒性，比较GraphRAG与RGB基线，测试三种GraphRAG定制化方案以提升鲁棒性


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能增强LLM能力并减少事实幻觉，但检索到的不一致信息可能对LLM响应产生负面影响，需要评估RAG系统在此类条件下的鲁棒性

Method: 使用RGB基准在四种场景下评估LLMs：噪声鲁棒性、信息整合、否定拒绝和反事实鲁棒性；比较RGB RAG基线与基于知识图谱的GraphRAG系统；测试三种GraphRAG定制化方案

Result: GraphRAG定制化方案相比RGB基线有改进，为设计更可靠的现实世界RAG系统提供了见解

Conclusion: 通过RGB基准评估和GraphRAG定制化测试，展示了提升RAG系统鲁棒性的方法，为构建更可靠的现实应用RAG系统提供了指导
...
TL;DR: 通过全球调查评估不同群体对文化在生成式AI中应如何体现的理解与信念，提出文化敏感性框架建议


<details>
  <summary>Details</summary>
Motivation: 缺乏关于全球对生成式AI应如何体现文化的态度的实证证据，需要了解不同群体对文化的定义及其在AI中的表现期望

Method: 进行大规模全球调查，收集欧洲、北美、南美、亚洲和非洲地区数据，从社区直接提炼文化的工作定义，分析文化概念复杂性及其与生成式AI表现的关系

Result: 建立了对文化概念复杂性的理解及其与生成式AI表现的关系，形成了具体的文化定义和表现期望

Conclusion: 提出文化敏感性框架建议：采用参与式方法，优先考虑超越地理的特定文化维度（如宗教和传统），建立处理文化"红线"的敏感性框架
...
TL;DR: 提出基于L-积的张量Transformer架构，通过将嵌入空间分解为p个独立的光谱子变换器，在保持标准Transformer语义的同时实现约1/p的编码器参数压缩。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在取得最先进性能的同时，伴随着参数增长和嵌入维度冗余的问题。需要一种既能压缩参数又能保持性能的结构化方法。

Method: 基于三阶张量的L-积进行结构化光谱分解，将令牌表示重塑为光谱张量切片，在变换域执行注意力机制和前馈操作。使用实值离散余弦变换(DCT)实现可微分的光谱分解。

Result: 在IMDB和AG News数据集上，模型可显著减少编码器参数（p=4时达75%），同时保持竞争力精度。在BERT-base宽度(d=768)下，性能可恢复至基线水平。

Conclusion: 提出的L-Transformer通过光谱张量分解实现了参数高效压缩，同时引入了嵌入频率的归纳偏置，支持切片依赖的频率缩放以改进泛化能力，为Transformer压缩提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前人工智能和人机交互系统通常被呈现为通用解决方案，但其设计仍然以文本为中心，未能充分服务以口语为主的土著语言和社区。作者以巴拉圭广泛使用的官方语言瓜拉尼语为例，指出除非AI语言支持与真实的口语实践相一致，否则这种支持仍然是不充分的。

Method: 提出一种替代标准"文本转语音"流程的口语优先多智能体架构。该架构将瓜拉尼语自然语言理解与专门的对话状态智能体和社区主导的治理智能体解耦，创建了一个尊重土著数据主权和双语现象的技术框架。研究重点从单纯的语言识别转向关注话轮转换、修复和共享上下文作为交互的主要场所。

Result: 展示了一个技术框架，该框架尊重土著数据主权和双语现象，将口语对话作为一等设计需求。通过解耦语言理解与对话状态管理，创建了一个能够适应口语实践而非强制文本中心化的系统架构。

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适应文本中心系统转变为将口语对话作为一等设计需求。数字生态系统应该赋能而非忽视多样化的语言实践，确保技术设计尊重土著社区的语言主权和文化实践。
...
TL;DR: NERdME：首个包含200个手动标注README文件的数据集，包含10,000+标注跨度和10种实体类型，用于填补学术信息提取在代码仓库实现层面细节的空白。
...
Method: 构建了NERdME数据集：包含200个手动标注的README文件，超过10,000个标注跨度和10种实体类型。使用大型语言模型和微调的transformer模型进行基线实验，并进行了下游实体链接实验。

Result: 基线结果显示论文层面和实现层面实体之间存在明显差异，表明将SIE基准扩展到README文件中可用的实体类型具有价值。实体链接实验证明从README提取的实体可以支持工件发现和元数据集成。

Conclusion: NERdME填补了学术信息提取在实现层面细节的空白，README文件中的实体类型扩展了SIE基准，能够支持代码仓库工件的发现和元数据集成。
...
TL;DR: PVminerLLM：针对患者生成文本中患者声音结构化提取的监督微调大语言模型，在多个数据集上显著优于提示基准方法，即使较小模型也能实现可靠的患者声音提取。


<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含患者生活经历、社会环境和护理参与等关键信息，这些因素强烈影响依从性、护理协调和健康公平。然而这些患者声音信号很少以结构化形式存在，限制了其在以患者为中心的结果研究和临床质量改进中的应用。可靠提取此类信息对于大规模理解和解决健康结果的非临床驱动因素至关重要。

Method: 引入PVminer基准用于患者声音的结构化提取，并提出PVminerLLM——专门为此任务设计的监督微调大语言模型。通过多个数据集和不同模型规模进行验证。

Result: PVminerLLM在多个数据集和模型规模上显著优于提示基准方法，达到83.82% F1（代码预测）、80.74% F1（子代码预测）和87.03% F1（证据跨度提取）。即使使用较小模型也能实现强性能，表明无需极大模型规模即可实现可靠的患者声音提取。

Conclusion: PVminerLLM实现了患者生成文本中嵌入的社会和体验信号的大规模分析，为患者声音的结构化提取提供了有效解决方案，有助于改善以患者为中心的结果研究和临床质量改进。
...
TL;DR: 开发了一个用于大规模分析辅导对话的辅导行为分类法，通过混合演绎-归纳过程构建，包含四个主要类别，特别关注学习支持行为的学生参与度谱系


<details>
  <summary>Details</summary>
Motivation: 需要系统分析辅导过程中教师教学行为的方法，以理解辅导的有效性，支持国家辅导观察站的大规模辅导对话分析

Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统的研究构建初步框架，然后通过专家注释者对真实辅导转录本的迭代编码进行细化

Result: 开发了一个包含四个类别的辅导行为分类法：辅导支持、学习支持、社会情感与动机支持、后勤支持。学习支持行为进一步按学生参与度谱系组织，区分激发学生推理的行为与提供直接解释或答案的行为
...
TL;DR: RouteGoT：一种预算可控的节点自适应路由框架，用于图结构推理，通过智能分配不同规模的模型到推理图的不同节点来平衡性能与成本


<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确性，但引入了显著的令牌消耗和延迟开销，且收益在不同任务分布中不稳定。作者观察到推理管道中存在阶段和节点的异质性：高质量规划和最终综合需要强大模型，而许多中间子任务可由轻量级模型准确解决。

Method: 提出RouteGoT框架，在图内进行路由，优先为规划和综合分配强大模型，同时根据预测的难度动态分配轻量级模型和成本效益策略给叶子子任务。框架集成了明确的预算约束到全局推理调度器中，在用户指定的令牌预算下控制图扩展，实现可预测的性能-成本权衡。

Result: 在推理、检索和多跳问答基准测试中，RouteGoT在匹配或提高准确性的同时显著减少令牌使用；相比AGoT平均实现8.1个百分点的准确性提升和79.1%的输出令牌减少。在变化的预算目标和任务下，RouteGoT保持更优的成本-准确性权衡，表现出改进的鲁棒性。

Conclusion: RouteGoT通过节点自适应路由和预算控制，有效解决了图结构推理中的效率问题，实现了更好的性能-成本平衡，为实际部署提供了可预测的权衡方案。
...
TL;DR: HART是一个细粒度幻觉归因与证据检索框架，通过结构化建模解决LLM幻觉追踪问题，包含跨度定位、机制归因、证据检索和因果追踪四个阶段。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉归因方法主要基于外部知识检索或内部模型机制，侧重于语义相似度匹配或表示级判别，难以在跨度层面建立幻觉类型、错误生成机制和外部事实证据之间的结构化对应关系，限制了幻觉片段的可解释性和支持/反对证据的可追溯性。

Method: HART将幻觉追踪形式化为包含四个阶段的结构化建模任务：1)跨度定位，2)机制归因，3)证据检索，4)因果追踪。基于此框架开发了首个针对幻觉追踪的结构化数据集，联合标注幻觉类型、错误机制和反事实证据集。

Result: 在提出的数据集上的实验结果表明，HART显著优于BM25和DPR等强检索基线，验证了所提追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力。

Conclusion: HART通过结构化建模方法实现了细粒度的幻觉归因和证据检索，为LLM幻觉分析提供了因果级可解释性评估框架，在幻觉追踪任务上表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在代码生成时采用"系统1"的单次前向生成方式，面对复杂算法任务时性能受限。现有的迭代优化策略在推理时主要依赖外部反馈、执行引擎或计算昂贵的提示-响应循环，缺乏完全自主的内在反思能力。

Method: 提出ReflexiCoder强化学习框架，采用RL-zero训练范式，使用细粒度奖励函数优化整个反思-修正轨迹。该方法将结构化推理轨迹（初始生成、错误和优化感知的反思、自我修正）内部化到模型权重中，使模型在推理时具备完全自主的自我反思和修正能力。

Result: 在7个基准测试中，ReflexiCoder-8B在1.5B-14B规模的开源模型中达到最先进水平：HumanEval (Plus) 94.51% (87.20%)、MBPP (Plus) 81.80% (78.57%)、BigCodeBench 35.00%、LiveCodeBench 52.21%、CodeForces 37.34%。在单次尝试设置下，性能媲美或超越GPT-5.1等专有模型。推理时计算开销减少约40%。

Conclusion: ReflexiCoder通过将结构化推理轨迹内部化到模型权重中，实现了从外部依赖优化到内在自主反思-修正的范式转变，显著提升了代码生成性能，同时提高了推理效率，为代码生成模型提供了新的发展方向。
...
<details>
  <summary>Details</summary>
Motivation: SparseGPT等基于二阶梯度（Hessian）的LLM单次剪枝方法采用预定义的从左到右剪枝顺序，当权重呈现列模式时会导致次优性能。需要研究剪枝顺序对剪枝效果的影响。

Method: ROSE方法：1）预剪枝识别候选权重；2）估计列和块的剪枝损失；3）两级重排序：块内按列损失降序重排列，块间按块损失重排；4）引入块损失相对范围指标识别列层，实现自适应重排序。

Result: 在主流LLMs（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的大量实验表明，ROSE超越了原始SparseGPT和其他对比剪枝方法。

Conclusion: 剪枝顺序对基于Hessian的LLM剪枝效果有显著影响，ROSE通过自适应重排序策略有效提升了剪枝性能，特别适用于权重具有列模式的情况。
...
TL;DR: CoCA提出了一种置信度优先的LLM不确定性估计方法，通过GRPO强化学习框架联合优化置信度校准和答案准确性，使用分段信用分配避免奖励攻击，在数学、代码和事实QA基准上实现了更好的校准和不确定性区分能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法主要是答案优先的，即在生成答案后才产生置信度，这种方法只能衡量特定回答的正确性，限制了实际可用性。需要研究置信度优先的范式，让模型在回答前输出置信度，将其解释为模型在当前策略下正确回答问题的概率。

Method: 提出CoCA（联合优化置信度和答案）框架，采用GRPO强化学习方法，通过分段信用分配联合优化置信度校准和答案准确性。该方法为置信度段和答案段分配独立的奖励和组相对优势，实现稳定的联合优化并避免奖励攻击。

Result: 在数学、代码和事实QA基准测试中，CoCA显示出改进的校准性能和不确定性区分能力，同时保持了答案质量。这使得更广泛的下游应用成为可能。

Conclusion: 置信度优先范式结合GRPO强化学习框架能够有效提升LLM的不确定性估计可靠性，通过联合优化置信度校准和答案准确性，为LLM的可靠部署提供了更好的不确定性估计方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的分词方法BPE及其变体本质上是脚本无关的，主要针对英语等屈折语优化，无法有效处理黏着语的特点。黏着语中单个词可能编码词根、时态、体、人称、数、性一致、格和后置词等多种语法信息，需要专门的分词方法。

Method: 论文未在摘要中明确描述具体方法，但暗示需要针对黏着语特点设计新的分词方法，可能涉及基于语素边界的分词策略，而不是简单的字节对统计。

Result: 统计分词器（如BPE）会将黏着语单词分割成破坏语素边界的字节对块，导致token数量膨胀，影响语言模型处理效率和质量。

Conclusion: 需要为黏着语设计专门的分词方法，以保留语素边界并优化token化过程，提高语言模型对这类语言的处理能力。
...
TL;DR: 提出了ConStory-Bench基准测试，用于评估长篇故事生成中的叙事一致性，包含2000个提示和5类19种子类型的错误分类，并开发了自动检测管道ConStory-Checker。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇叙事时经常出现前后不一致的问题，包括事实、角色特征和世界规则的矛盾。现有的故事生成基准主要关注情节质量和流畅性，忽略了一致性错误。

Method: 创建ConStory-Bench基准，包含2000个提示覆盖四种任务场景，定义5类19种子类型的错误分类。开发ConStory-Checker自动管道，检测矛盾并提供文本证据支持判断。

Result: 评估多个LLM发现：一致性错误在事实和时间维度最常见；倾向于出现在叙事中间部分；出现在token级熵值较高的文本段；某些错误类型倾向于共现。

Conclusion: ConStory-Bench填补了长篇故事生成中一致性评估的空白，研究结果可为改进叙事一致性提供指导。基准测试网站已公开。
...
TL;DR: 本文提出了一种基于LLM的集成系统方法，用于联合国安理会决议的语义标注，通过引入CPR和TWF指标避免幻觉问题，GPT-4.1表现最佳，GPT-4.1-mini在成本效益上最优。


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能变异性构建集成系统，完成联合国安理会决议的数据清洗和语义标注任务，同时避免模型幻觉和不必要的文本修改。

Method: 提出基于LLM的集成系统方法，引入两个评估指标：内容保持率（CPR）和标签格式良好性（TWF），通过多个GPT模型的多轮运行选择最佳输出。

Result: GPT-4.1在两个任务中均获得最高指标（清洗：CPR 84.9%；语义标注：CPR 99.99%和TWF 99.92%）。GPT-4.1-mini以20%的成本达到可比性能，集成系统能为所有模型选择最优输出。

Conclusion: 通过集成设计和评估指标，构建了可靠的LLM系统，能够在挑战性文本上执行语义标注任务，同时平衡性能与成本效益。
...
TL;DR: 该研究探讨了在大型语言模型中解决生态谬误问题，通过建模作者的语言上下文（HuLM任务）来提升8B参数Llama模型的性能，结果显示在微调和预训练阶段考虑作者上下文都能带来性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型训练和推理忽略了一个基本语言学事实：同一作者生成的多个文本序列之间存在依赖关系。先前研究表明，解决这种生态谬误形式可以显著提升较小规模GPT模型的性能。本研究旨在探索在更大规模的8B Llama模型中，通过建模作者语言上下文（HuLM任务）是否也能带来类似益处。

Method: 研究探索了多种变体，这些变体按时间顺序处理作者的其他文本作为上下文。研究了两种主要方法：1）使用HuLM目标进行预训练，2）在微调阶段使用作者上下文（HuFT：人类感知微调）。具体采用QLoRA技术进行参数高效微调，并比较了标准微调与考虑作者上下文的微调效果。

Result: 实证比较显示：1）仅通过QLoRA在微调阶段解决生态谬误问题，就能使8B模型性能优于标准微调；2）基于QLoRA的持续HuLM预训练产生了一个人类感知模型，该模型仅通过线性任务分类器训练就能在八个下游任务上获得改进性能。

Conclusion: 结果表明，在语言原始生成者（作者）的上下文中建模语言具有实用性和重要性。解决生态谬误问题不仅能提升较小模型性能，对大规模模型同样有效，这为语言模型开发提供了新的方向。
...
<details>
  <summary>Details</summary>
Motivation: 真正的主动AI系统需要预测用户下一步行为，这需要比稀疏提示更丰富的上下文信息。现有方法缺乏对用户多模态交互（屏幕截图、点击、传感器数据）的长期推理能力，需要新的数据和建模方法。

Method: 1) 数据标注：使用视觉语言模型标注自然计算机使用数据，开发开源标注流程，标注20名用户一个月手机使用的360K个动作；2) LongNAP模型：结合参数化和上下文学习，通过策略梯度训练生成用户特定推理轨迹，从历史轨迹库检索相关轨迹，在上下文中应用这些轨迹预测未来动作。

Result: 在LLM-as-judge评估中（0-1相似度），LongNAP显著优于监督微调（提升79%）和提示基线（提升39%）。尽管用户可能动作空间无限，但17.1%的预测轨迹与用户实际行为良好对齐（得分≥0.5），高置信度预测时达到26%。模型能泛化到未见用户。

Conclusion: 通过学习用户行为的完整上下文来预测用户需求已成为可行任务，具有重大机会。LongNAP展示了在长交互历史中推理的有效性，为真正主动的AI系统奠定了基础。
...
TL;DR: 开发可解释模型预测幸福感并识别适应性/非适应性自我状态，整合人格特质与情境特征，理论驱动特征在保持可解释性的同时达到竞争性性能


<details>
  <summary>Details</summary>
Motivation: 心理健康不是固定特质而是动态过程，受个体倾向与情境背景交互影响。需要开发能够捕捉这种动态性并在真实世界数据（如社交媒体）中识别适应性/非适应性自我状态的可解释模型

Method: 整合个人层面心理特质（韧性、认知扭曲、内隐动机）与基于情境8钻石框架的语言推断情境特征。比较这些理论驱动特征与心理测量学信息语言模型的嵌入，后者捕捉时间和个体特定模式

Result: 理论驱动的原则性特征在保持可解释性的同时提供竞争性性能。定性分析进一步突显了对幸福感最具预测性特征的心理一致性

Conclusion: 将计算建模与心理学理论整合对于以情境敏感和人类可理解的方式评估动态心理状态具有重要价值，理论驱动特征在性能和可解释性之间取得良好平衡
...
TL;DR: MASFactory是一个图中心框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可编辑工作流规范，提供可重用组件和可视化工具。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的多智能体系统框架在实现复杂图工作流时需要大量手动工作，复用性有限，难以集成异构外部上下文源。

Method: 提出MASFactory框架，引入Vibe Graphing方法将自然语言意图编译为可编辑工作流规范并转换为可执行图，提供可重用组件、可插拔上下文集成和可视化工具。
...
TL;DR: 提出结构化风格重写框架，通过显式解耦风格为三个可解释维度（词汇特征、句法模式、语用风格），并结合隐式风格条件策略，使小语言模型能够生成高保真风格化内容，在动漫角色领域超越更大规模基线模型。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在角色扮演中面临数据稀缺和风格解耦复杂性的挑战，标准监督微调往往只能捕捉表层语义而无法复现角色的复杂句法和语用细微差别，导致"角色偏离"生成。
...
Conclusion: 该方法为在消费级硬件上进行推理和部署提供了数据高效的范式，实现了小语言模型的高保真风格化生成，解决了角色扮演中的风格解耦挑战。
...
TL;DR: 该研究评估了四种开源大语言模型在奥地利德语A级考试作文自动评分中的应用，发现模型在评分细则维度上最高仅达到40.6%的人类评分一致性，最终成绩匹配率仅32.8%，表明当前模型尚不适合实际评分环境。


<details>
  <summary>Details</summary>
Motivation: 自动化作文评分系统旨在减轻教师评分工作量并减少主观偏见。随着大语言模型的发展，研究者希望探索开源模型在德语作文评分中的应用潜力，特别是基于评分细则的评估方法。

Method: 研究使用了101份奥地利德语A级考试的匿名学生试卷，涵盖三种文本类型。评估了四种开源大语言模型：DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b，采用不同上下文和提示策略进行评分细则维度的评估。

Result: 模型在评分细则维度上最高达到40.6%的人类评分一致性，最终成绩匹配率仅32.8%。结果表明，虽然较小模型能够使用标准化评分细则进行德语作文评分，但准确性不足以应用于实际评分环境。

Conclusion: 尽管开源大语言模型在德语作文评分中展现出一定潜力，但当前模型在评分准确性和一致性方面仍存在显著不足，不适合在实际评分环境中部署使用。
...
TL;DR: Track-SQL框架通过双提取模块增强生成式语言模型，解决多轮Text-to-SQL中上下文和模式链接的复杂性，在SparC和CoSQL数据集上取得SOTA性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM开发过度关注统一性能基准，偏向特定行为倾向（如自信），忽视了人类问题解决中多样化的风格和人格特质。研究旨在探索不同经验如何塑造机器个性并影响问题解决能力。

Method: 采用持续预训练方法，以无监督方式让模型接触领域特定文本，模拟经验积累过程。使用基于大五人格框架的机器人格量表（MPI）量化模型变体的人格特质，并分析其与语言风格和推理行为的关系。

Result: 模型能力呈双峰分布，在"表达型通才"和"抑制型专才"处达到峰值；发现"抑制优势"现象——社交特质降低能提升复杂推理性能；建立了训练数据语言学特征（如祈使句频率、词汇多样性）与模型个性之间的因果关系。

Conclusion: 研究为"个性工程"提供了路线图，表明通过精心设计训练数据的语言学特征可以系统性地塑造LLM的个性特质，从而优化其在特定任务上的表现，特别是复杂推理任务。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提或主张是隐含的）。现有NLP方法只能识别省略三段论但不能解码其底层逻辑，而基于逻辑的方法假设存在足够公式的知识库通过溯因推理解码。缺乏将省略三段论文本组件转换为逻辑论证并生成解码所需逻辑公式的系统方法。
...
Conclusion: 该管道成功整合了LLM的自然语言处理能力和符号推理的逻辑严谨性，为处理文本中的省略三段论提供了一种系统方法，能够将自然语言论证转换为逻辑形式并确定逻辑蕴涵关系。
...
TL;DR: ViewFusion：两阶段框架，通过显式分离跨视角空间预对齐与问答，提升多视角空间推理能力


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多视角空间推理方面存在不足，即使有多个视角可用，模型也往往未能充分利用跨视角关系，而是依赖单图像捷径，导致在视角变换和遮挡敏感案例中表现脆弱

Method: 提出ViewFusion两阶段框架：第一阶段进行空间预思考，推断视角关系和空间变换，形成中间工作空间；第二阶段基于该工作空间进行问题驱动的推理。使用合成推理监督训练，随后通过GRPO进行强化学习

Result: 在MMSI-Bench上，ViewFusion比Qwen3-VL-4B-Instruct准确率提升5.3%，在需要真正跨视角对齐的示例上提升最大

Conclusion: 通过显式分离跨视角空间预对齐与问答，ViewFusion能更有效地处理多视角空间推理任务，特别是需要跨视角对齐的复杂场景
...
TL;DR: 提出基于图的框架，通过实体交互图表示对立叙事，结合节点级因果估计，构建最小因果子图，在分类任务中超越现有方法


<details>
  <summary>Details</summary>
Motivation: 当前文本分析方法依赖预定义本体标注的数据，将人类偏见嵌入黑盒模型，仅利用非结构化线性模式识别，未能建模话语中自然出现的实体间结构化交互

Method: 提出基于图的框架：1) 将叙事表示为实体交互图；2) 在节点层面结合因果估计，将构建的句子图提炼为最小因果子图；3) 基于此表示构建分类流程

Result: 该方法在对立思维分类任务中超越了现有方法

Conclusion: 通过实体交互图和因果子图表示，能够更好地检测、分析和分类对立叙事及其底层实体，克服了传统方法的结构化建模不足问题
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标未能充分纳入完整临床上下文（如患者年龄、指征和指南决策规则），且容易让正常或不显著发现对总体评分产生不成比例的影响，缺乏对临床显著错误的优先级区分。

Method: CRIMSON将错误分类为全面分类法，涵盖假阳性发现、遗漏发现和八种属性级错误；每个发现分配临床显著性级别（紧急、可行动非紧急、不可行动、预期/良性），基于与心胸放射科医生合作开发的指南，实现严重性感知加权；通过三个基准验证：ReXVal（与放射科医生标注的临床显著错误计数对齐）、RadJudge（临床挑战性通过-失败场景）和RadPref（放射科医生偏好基准）。

Result: CRIMSON在ReXVal中与六名认证放射科医生标注的临床显著错误计数高度一致（Kendall's tau = 0.61-0.71；Pearson's r = 0.71-0.84）；在RadJudge中与专家判断一致；在RadPref中与放射科医生偏好对齐最强；发布了度量标准、评估基准和微调的MedGemma模型。

Conclusion: CRIMSON提供了一个临床基础、全面且可重复的胸部X光报告生成评估框架，通过纳入完整临床上下文、严重性感知加权和全面错误分类，显著优于现有指标，能够优先考虑临床重要错误并促进报告生成系统的可重复评估。
...
Method: 提出FlashPrefill框架，采用快速块搜索技术同时定位动态垂直、斜线和块稀疏注意力模式，引入动态阈值机制绕过排序或累积注意力分数的计算开销，有效消除长尾分布以增强稀疏性。

Result: 在256K序列上实现27.78倍加速，在4K上下文长度上仍保持1.71倍加速，表明在不同序列规模下均具有鲁棒性和实用性。

Conclusion: FlashPrefill通过创新的模式发现和阈值化技术，显著提升了长上下文预填充效率，解决了现有稀疏注意力方法的局限性，为大规模序列处理提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明大语言模型与人类在逻辑推理中存在相似偏见，但批评这些偏见可能不公平，因为人类推理不仅包含形式演绎，还涉及从有限信息中得出试探性结论的溯因推理。需要探究大语言模型在溯因推理中的表现，以更全面地评估其推理能力。

Method: 将三段论数据集转换为适合溯因推理的形式，用于评估最先进的大语言模型在溯因推理任务中的准确性。通过这种转换，研究模型是否在溯因推理中存在偏见，并识别潜在的改进领域。

Result: 论文未在摘要中提供具体实验结果，但研究框架旨在揭示大语言模型在溯因推理中的表现特征，包括可能的偏见模式以及与人类推理的差异。

Conclusion: 该研究强调超越形式演绎的语境化推理的重要性，对于推进大语言模型在复杂推理任务中的理解和应用至关重要，为弥合机器与人类认知之间的差距提供了见解。
...
TL;DR: PONTE是一个个性化可解释AI框架，通过闭环验证和适应过程生成可靠的自然语言解释，解决传统XAI方法忽视用户差异和LLM幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI方法大多采用"一刀切"范式，忽视了用户在专业知识、目标和认知需求上的差异。虽然大语言模型可以将技术解释转化为自然语言，但存在忠实性和幻觉问题，需要更可靠、个性化的解释生成框架。

Method: PONTE采用人在回路框架，将个性化建模为闭环验证和适应过程而非提示工程。包含三个核心组件：(1)低维偏好模型捕捉风格需求；(2)基于结构化XAI工件的偏好条件生成器；(3)强制执行数值忠实性、信息完整性和风格对齐的验证模块，可选支持检索增强论证。用户反馈迭代更新偏好状态，实现快速个性化。

Result: 在医疗和金融领域的自动和人工评估表明，验证-精炼循环显著提高了完整性和风格对齐性。人工研究进一步证实：预期偏好向量与感知风格之间具有强一致性、对生成随机性的鲁棒性，以及持续积极的质量评估。

Conclusion: PONTE通过将个性化建模为闭环验证过程，而非简单的提示工程，成功解决了XAI中的个性化解释生成问题，提高了自然语言解释的可靠性和适应性，为不同用户提供更有效的AI系统透明度。
...
Method: 提出AI-CROWD协议，利用大语言模型集合的集体输出近似真实标注，通过多数投票聚合输出，并使用诊断指标分析模型间的一致/分歧模式。

Result: 协议能够生成基于共识的近似标注，识别高置信度分类，同时标记潜在的模糊性或模型特定偏差。

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过模型集合的集体智慧近似真实标注，克服了人工标注的局限性。
...
TL;DR: LIT-RAGBench是一个新的RAG生成器基准测试，涵盖集成、推理、逻辑、表格和弃权五个类别，通过虚构实体和场景评估LLM在统一条件下的多能力表现。


<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖有限，无法在统一条件下同时评估多种能力（如长上下文整合、多步推理、表格解释、证据缺失时的弃权）。需要弥合现有评估与实际应用之间的差距。

Method: 定义五个评估类别（集成、推理、逻辑、表格、弃权），每个类别进一步细分为实际评估方面。使用虚构实体和场景构建114个人工构造的日语问题，通过机器翻译和人工校对生成英文版本。采用LLM-as-a-Judge进行评分，报告类别和整体准确率。

Result: 在API和开源模型测试中，没有模型超过90%的整体准确率。基准测试能够测量每个类别内的优势和弱点，为实际RAG部署中的模型选择和RAG专用模型构建提供有价值的度量标准。

Conclusion: LIT-RAGBench填补了现有RAG评估与实际需求之间的空白，通过系统覆盖多类别组合模式，成为实际RAG部署中模型选择和RAG专用模型构建的重要基准。数据集和评估代码已开源发布。
...
TL;DR: SPOT框架通过将显式思维链压缩为紧凑的潜在暂停令牌，使用跨段语义对齐和冻结头部解码约束，在减少推理成本的同时提升推理性能并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 显式思维链虽然能提升大语言模型的推理性能，但会产生高推理成本（冗长的令牌级追踪）。现有方法主要通过简洁提示或步骤剪枝来减少开销，但这些方法大多只是截断模型输出而非内化模型思考过程。潜在推理提供了有前景的替代方案，但现有方法面临两个关键挑战：1) 依赖刚性的点对点对齐，迫使潜在令牌近似推理步骤的最终表示，不足以捕捉整个推理段的密集、可变长度语义；2) 缺乏可解释性，潜在状态通常通过无约束优化或嵌入混合产生，难以在预训练语言头下解码或审计。

Method: SPOT框架包含两个核心组件：1) 跨段语义对齐：使用Sinkhorn最优传输目标，将每个暂停令牌软匹配到整个推理段的语义，克服步骤末端对齐的刚性；2) 冻结头部解码约束：保持潜在状态在冻结的预训练LM头下可直接解码为令牌分布，实现对潜在思考的可读关键词解释。该方法不强制固定的响应模板，灵活地将显式CoT压缩为紧凑的潜在暂停令牌。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成令牌37.5%，并为潜在推理过程提供忠实的语义解释。

Conclusion: SPOT通过跨段语义对齐和冻结头部解码约束，成功解决了潜在推理中的语义对齐刚性和可解释性不足问题，在减少推理成本的同时提升性能并保持可解释性，为高效的潜在推理提供了有效框架。
...
TL;DR: LLMs在多元文化环境中存在宗教文化对齐问题，特别是对少数群体的宗教观点存在偏差和刻板印象，需要系统性区域审计以确保公平部署。


<details>
  <summary>Details</summary>
Motivation: LLMs主要依赖英语中心训练数据，在多元文化多语言部署中可能与不同社会的文化价值观存在错位，特别是在宗教等敏感领域。需要评估LLMs在不同文化背景下的对齐程度。

Method: 对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代LLMs进行多语言审计，聚焦印度、东亚和东南亚地区。使用log-probs/logits分析模型内部表征，将模型观点分布与真实公众态度对比。采用人口统计提示和母语提示等轻量级干预措施，并在CrowS-Pairs、IndiBias、ThaiCLI、KoBBQ等偏见基准上进行下游评估。

Result: 主流模型在广泛社会问题上与公众意见基本一致，但在宗教观点上持续失败，特别是对少数群体的宗教观点，经常放大负面刻板印象。轻量级干预措施部分缓解但未能消除文化差距。下游偏见基准评估显示在敏感情境中存在持续危害和代表性不足问题。

Conclusion: LLMs在宗教文化对齐方面存在系统性偏差，特别是对少数群体。需要基于区域的系统性审计来确保LLMs在全球部署中的公平性，当前方法不足以解决深层次的文化错位问题。
...
TL;DR: 研究探索ChatGPT模仿艺术家原创作品生成图像的能力，通过12位艺术家评估发现AI模仿在色彩纹理相似度与构图概念感知相似度存在显著差距，建议使用多指标"风格转换仪表板"而非单一指标评估相似性。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉创造力方面的能力，特别是ChatGPT能否有意识地模仿原创艺术作品（绘画、雕塑、装置等）生成新的图像，并评估这种模仿与原创作品之间的相似性和差异。

Method: 邀请来自罗马尼亚、保加利亚、法国、奥地利和英国的12位艺术家，每人提供3件原创作品；使用ChatGPT生成这些作品的模仿版本；结合人类评估（艺术家评分和评论）与计算方法检测视觉和风格相似性或差异。

Result: 研究发现基于色彩和纹理的相似性与基于构图、概念和感知的相似性之间存在显著差距；艺术家认为ChatGPT生成的模仿作品缺乏维度感、上下文和意图感，更像是近似引用而非能引发情感的有价值艺术品。

Conclusion: 建议使用包含互补指标的"风格转换仪表板"来评估模仿作品与原创作品之间的相似性，而非依赖单一风格指标；AI在模仿当代艺术作品时存在局限性，无法完全捕捉艺术作品的深度和意图。
...
TL;DR: 该研究将数学问题理解建模为数学实体关系抽取任务，使用Transformer模型自动提取数学文本中的操作数和运算符关系，BERT模型达到99.39%准确率，并结合SHAP可解释性分析增强模型透明度。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为存在专业实体和复杂关系。需要自动提取数学问题中的实体关系以支持自动化问题解决、知识图谱构建和智能教育系统。

Method: 将数学问题解释建模为数学实体关系抽取任务，将操作数视为实体，运算符视为关系。应用基于Transformer的模型自动提取这些关系，使用BERT模型，并整合SHAP可解释性人工智能技术分析特征重要性。

Result: BERT模型在数学实体关系抽取任务中表现最佳，达到99.39%的准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的洞察。

Conclusion: 通过结合基于Transformer的学习、任务特定数据集和可解释建模，该工作为数学实体关系抽取提供了有效且可解释的框架，支持未来在自动化问题解决、知识图谱构建和智能教育系统中的应用。
...
TL;DR: 该研究通过引入包含道义模态的Wason选择任务数据集，系统比较了LLM在道义规则和描述性条件下的推理表现，发现LLM与人类类似，在道义规则上推理更好且表现出匹配偏差错误模式。


<details>
  <summary>Details</summary>
Motivation: 随着LLM语言能力提升，其推理能力受到关注。人类推理在特定领域（尤其是规范性而非纯形式化情境）表现良好，但LLM推理的领域特异性尚未充分探索。研究旨在系统比较LLM在不同规则类型下的条件推理表现。

Method: 引入新的Wason选择任务数据集，明确编码道义模态以区分道义条件句和描述性条件句。使用该数据集检验LLM在道义规则下的条件推理，并分析错误模式是否更符合确认偏差（寻求支持规则的证据）或匹配偏差（忽略否定并选择与规则词汇匹配的项目）。

Result: 结果显示，与人类类似，LLM在道义规则上推理表现更好，并表现出类似匹配偏差的错误模式。LLM性能随规则类型系统变化，其错误模式与人类在该范式中的已知偏差相似。

Conclusion: LLM的推理性能在不同规则类型间存在系统性差异，其错误模式能够反映人类推理中的已知认知偏差，表明LLM的推理机制与人类认知过程存在相似性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立地考察各种基于LLM的自动作文评分技术，缺乏对它们在英语作为第二语言写作中相对优势的系统比较。本研究旨在填补这一空白，为LLM在英语L2写作自动评分中的应用提供统一的实证基准。

Method: 在雅思写作任务2的统一基准上，评估了四种主要LLM-based AES方法：(1)基于编码器的分类微调，(2)零样本和少样本提示，(3)指令调优和检索增强生成(RAG)，(4)监督微调结合直接偏好优化(DPO)和RAG。通过系统比较这些方法的性能表现。

Result: 研究结果揭示了不同方法在准确性、成本和鲁棒性之间的明确权衡。最佳配置（结合k-SFT和RAG）取得了最强的整体结果，F1分数达到93%。这为英语L2写作自动评分提供了有效的技术方案。

Conclusion: 该研究首次对现代基于LLM的英语二语写作自动评分策略进行了统一实证比较，展示了不同范式在自动评分任务中的潜力，为后续研究和应用提供了重要参考。代码已公开。
...
<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语设置和短孤立话语，虽然上下文感知ASR有进展，但仍存在两个关键挑战：有限的多语言支持和缺乏语音与上下文表示之间的原则性对齐。
...
Conclusion: 上下文建模和跨模态对齐在多语言ASR中都很重要，提出的框架在保持预训练模型模块性的同时，支持多语言和口音，并通过对比学习改善语音与上下文交互。
...
TL;DR: KCLarity团队参与SemEval 2026的CLARITY任务，研究政治话语中歧义和回避技术的分类，比较了直接预测清晰度标签和通过任务层级推导清晰度两种建模方法。


<details>
  <summary>Details</summary>
Motivation: 参与SemEval 2026的CLARITY共享任务，该任务旨在对政治话语中的歧义和回避技术进行分类，以促进对政治沟通中模糊性和回避策略的理解和检测。

Method: 研究两种建模方法：(1) 直接预测清晰度标签；(2) 先预测回避标签，然后通过任务分类层级推导清晰度。还探索了多种辅助训练变体，并在回避优先的框架下评估了零样本设置的仅解码器模型。

Result: 两种建模方法在性能上表现相当。在基于编码器的模型中，RoBERTa-large在公共测试集上取得了最佳结果，而零样本GPT-5.2在隐藏评估集上表现出更好的泛化能力。

Conclusion: 两种建模方法在政治话语清晰度分类任务中表现相似，但不同模型在不同数据集上各有优势：RoBERTa-large在可见数据上表现更好，而GPT-5.2在未见数据上泛化能力更强。
...
TL;DR: RoboLayout扩展LayoutVLM，通过集成可达性约束和局部优化，生成适合不同智能体交互的室内场景布局


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理和3D场景布局生成方面取得进展，但生成的布局往往缺乏对具身智能体交互可行性的考虑，特别是在物理约束的室内环境中。需要一种能够生成既语义连贯又适合智能体交互的布局方法。

Method: RoboLayout在LayoutVLM基础上扩展，主要方法包括：1）将显式可达性约束集成到可微分布局优化过程中；2）采用通用的智能体抽象，可表示不同物理能力的实体（服务机器人、仓库机器人、不同年龄段人类、动物等）；3）提出局部优化阶段，选择性重新优化问题对象放置，同时保持场景其余部分固定，提高收敛效率而不增加全局优化迭代次数。

Result: 实验结果表明，RoboLayout在保持LayoutVLM强语义对齐和物理合理性的同时，增强了面向智能体中心的室内场景生成适用性。该方法能够生成可导航和可操作的布局，适应不同场景配置。

Conclusion: RoboLayout通过集成智能体感知推理和改进优化稳定性，成功扩展了LayoutVLM框架，实现了适合具身智能体交互的室内场景布局生成。该方法具有平台无关性，可适应不同物理能力的智能体，并通过局部优化提高了效率。
...
TL;DR: 论文评估了大型语言模型对思维链（CoT）的可控性，发现当前模型在控制思维链内容方面能力有限，这对CoT监控的可靠性提供了谨慎乐观的依据。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估大型语言模型控制其思维链内容的能力，因为如果模型能够随意控制思维链中的内容，就会破坏CoT监控的有效性，从而影响对模型行为的检测和理解。

Method: 提出了CoT-Control评估套件，包含要求模型在解决特定问题时遵守思维链指令的任务（如在遗传学问题中不使用"染色体"一词）。通过测试不同模型在多种条件下的表现，包括模型大小、RL训练程度、推理时间、问题难度等因素的影响。

Result: 研究发现：1）推理模型的CoT可控性显著低于最终输出的可控性；2）更大模型具有更高的CoT可控性；3）更多RL训练、更多推理时间和更高问题难度会降低可控性；4）即使给予激励，模型也难以有效控制CoT；5）对抗性提示优化对提升可控性效果有限。

Conclusion: 目前CoT可控性不太可能成为CoT监控的失效模式，但低可控性的机制尚不清楚。建议前沿实验室在未来模型中跟踪CoT可控性，以确保CoT监控的有效性。
...
### [49] [The World Won't Stay Still: Programmable Evolution for Agent Benchmarks](https://arxiv.org/abs/2603.05910)
...
TL;DR: ProEvolve：一个基于图的框架，用于可编程地演化LLM智能体环境，以评估智能体对动态变化的适应性


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设静态环境，忽略了现实世界的演化特性以及智能体对环境变化的鲁棒性。需要一种可扩展且可控的环境演化方法来更好地评估智能体对现实世界动态的适应性。

Method: 提出ProEvolve框架，使用类型化关系图统一表示环境的数据、工具和模式。通过图变换（添加、删除、修改）来编程环境演化动态，并通过子图采样和编程实例化任务沙箱。

Result: 将单个环境演化为200个环境和3000个任务沙箱，并据此对代表性智能体进行基准测试，验证了框架的有效性。

Conclusion: ProEvolve提供了一种可编程、可扩展的环境演化方法，能够更好地评估LLM智能体在动态变化环境中的适应性，弥补了现有静态基准测试的不足。
...
TL;DR: 本研究提出了一种基于大语言模型的多智能体系统，用于自动化产品概念评估，通过虚拟专家团队对技术可行性和市场可行性进行结构化评估，验证了与行业专家评估结果的一致性。


<details>
  <summary>Details</summary>
Motivation: 产品概念评估是企业战略资源分配和项目成功的关键阶段，但传统专家主导方法存在主观偏见、时间和成本高等局限性，需要更客观高效的自动化评估方法。
...
Result: 通过专业显示器概念的案例研究，系统评估排名与行业资深专家的评估结果一致，验证了多智能体评估方法的有效性。

Conclusion: 基于多智能体的自动化评估方法可用于支持产品开发决策，提供与传统专家评估一致的结果，同时克服了传统方法的主观偏见和高成本问题。
...
TL;DR: 本文提出了一种新的聚合语义家族，用于定量双极论证框架，通过将攻击者和支持者分别聚合的三阶段计算，增强了参数化能力和可解释性。


<details>
  <summary>Details</summary>
Motivation: 在定量双极论证框架中，现有方法通常对称处理攻击和支持关系，但在实际应用中攻击者和支持者可能扮演不对称的角色。需要一种更灵活、可解释的语义来更好地建模这种不对称性。

Method: 提出聚合语义家族，采用三阶段计算：1) 单独计算攻击者的全局权重；2) 单独计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。讨论了三个聚合函数在不同情境下应满足的性质及其与经典渐进语义原则的关系。

Result: 通过简单示例和最终测试500种聚合语义的示例，展示了聚合语义的广泛行为范围。三阶段分解使计算更具参数化，保持了双极性，并产生了更易理解的渐进语义。

Conclusion: 聚合语义通过将攻击者和支持者分别聚合的三阶段计算，提供了比现有方法更灵活、更可解释的渐进语义，能够更好地处理攻击者和支持者不对称的情况。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习方法在计算材料发现中主要使用生成模型，但由于最大似然训练的限制，无法有效探索材料空间中有吸引力的区域，需要一种能直接优化目标属性的替代方法

Method: 提出CliqueFlowmer模型，将基于团的离线模型优化（MBO）融入Transformer和流生成框架，通过直接优化目标材料属性来指导生成过程

Result: CliqueFlowmer在优化能力验证中表现出色，生成的材料性能显著优于生成基线方法，证明了其在材料优化问题中的有效性

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的离线模型优化方法，能够直接优化目标属性并生成高性能材料，代码已开源以支持跨学科研究
...
TL;DR: 提出基于强化学习的洪水适应规划决策支持框架，结合降雨预测、洪水建模和交通模拟，在哥本哈根案例中展示优于传统优化方法的效果。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和洪水，导致城市交通系统中断增加。制定有效适应策略面临长期基础设施投资、深度气候不确定性以及洪水-基础设施-交通复杂交互的挑战。

Method: 提出基于强化学习的决策支持框架，作为综合评估模型，整合降雨预测、洪水建模、交通模拟，量化基础设施和交通的直接间接影响。RL方法学习平衡投资维护成本与避免影响的适应策略。
...
<details>
  <summary>Details</summary>
Motivation: 当前个人AI面临用户数据分散在孤立数据孤岛中的问题，传统的检索增强生成方法依赖非结构化向量相似性，无法捕捉潜在语义拓扑和时间依赖关系，难以实现整体意义建构。

Method: 引入EpisTwin神经符号框架：1）利用多模态语言模型将异构跨应用数据提升为语义三元组，构建用户中心的可验证个人知识图谱；2）在推理时通过智能协调器结合图检索增强生成和在线深度视觉细化，在原始视觉上下文中动态重新锚定符号实体。

Result: 开发了PersonalQA-71-100合成基准测试模拟真实用户数字足迹，EpisTwin在一系列最先进的评判模型上展现出稳健的性能，为可信赖的个人AI提供了有前景的方向。

Conclusion: EpisTwin通过将生成推理建立在可验证的个人知识图谱基础上，解决了个人AI中的数据碎片化问题，其神经符号方法结合图检索增强生成和视觉细化，为实现可信赖的个人AI提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 递归自我改进正从理论走向实践，但迭代自我修改存在微妙的对齐漂移风险，需要可部署的监控和控制框架来确保改进过程中的对齐保持。

Method: 提出SAHOO框架，包含三个核心组件：(1)目标漂移指数(GDI)，结合语义、词汇、结构和分布度量的多信号检测器；(2)约束保持检查，强制执行安全关键不变性；(3)回归风险量化，标记可能撤销先前增益的改进周期。

Result: 在189个任务的代码生成、数学推理和真实性测试中，SAHOO实现显著质量提升：代码任务改进18.3%，推理任务改进16.8%，同时在两个领域保持约束，在真实性任务中保持低违规率。阈值在18个任务的验证集上校准。

Conclusion: SAHOO使递归自我改进期间的对齐保持变得可测量、可部署且可系统验证，揭示了能力-对齐前沿，显示早期改进周期效率高但后期对齐成本上升，以及领域特定张力（如流畅性与事实性）。
...
TL;DR: 论文提出schema-gated编排作为解决科学工作流中LLM灵活性与确定性执行冲突的原则，通过多模型LLM评分评估20个系统，发现现有系统无法同时实现高灵活性和高确定性，并提出了分离对话与执行权限的架构原则。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以将研究人员的自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决定运行内容难以保证这些要求。工业研发专家的访谈揭示了两个竞争需求：确定性约束执行与无工作流刚性的对话灵活性。

Method: 提出schema-gated编排作为解决原则：模式成为组合工作流级别的强制执行边界，确保任何操作（包括跨步骤依赖）必须通过机器可检查的规范验证才能运行。将需求操作化为执行确定性（ED）和对话灵活性（CF），使用多模型协议（3个LLM家族的15个独立会话）对20个系统进行评分，评估其在验证范围谱系上的表现。

Result: 多模型LLM评分显示高度一致性（ED的Krippendorff α=0.80，CF的α=0.98），表明多模型LLM评分可作为人类专家小组的可重用替代方案。现有系统呈现经验帕累托前沿，没有系统能同时实现高灵活性和高确定性，但在生成式和以工作流为中心的两极之间出现了收敛区域。

Conclusion: schema-gated架构通过分离对话权限和执行权限，能够解耦灵活性与确定性的权衡。提出了三个操作原则：执行前澄清、约束的计划-行动编排、工具到工作流级别的门控，以指导采用这种架构。
...
TL;DR: AutoThinkRAG：通过查询复杂度路由和功能解耦架构，解决文档问答中长上下文和信息过载问题，降低推理成本并提升性能


<details>
  <summary>Details</summary>
Motivation: 文档问答面临长上下文和信息过载的约束，现有多模态GraphRAG框架存在双重挑战：需要大规模模型处理不同复杂度查询，以及端到端视觉语言模型存在固有推理瓶颈

Method: 提出AutoThinkRAG框架，包含查询复杂度路由器根据查询难度分配推理路径，采用功能解耦架构：小规模VLM作为高保真视觉解释器将视觉线索转为文本表示，再由LLM进行逻辑推理和综合

Result: 在DocBench和MMLongBench上的实验表明，AutoThinkRAG显著降低推理成本的同时实现了新的最先进性能，消融研究验证了方法的有效性

Conclusion: 通过协同利用多个模型能力，AutoThinkRAG有效解决了文档问答中的复杂文档理解问题，为多模态文档理解提供了高效解决方案
...
TL;DR: CBR-to-SQL框架利用案例推理方法，通过两阶段检索过程将自然语言医疗问题转换为SQL查询，解决了传统RAG在医疗领域中的术语变异和噪声问题。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录数据库的SQL查询需要专业知识，这阻碍了医疗决策和研究。虽然可以使用LLM通过RAG将自然语言问题转换为SQL，但标准RAG在医疗领域面临挑战：医疗术语的变异性和噪声导致单步检索效果不佳，扩大示例池会引入噪声和可扩展性问题。

Method: 提出CBR-to-SQL框架，受案例推理启发，将问题-SQL对表示为可重用的抽象案例模板。采用两阶段检索过程：第一阶段捕获逻辑结构，第二阶段解析相关实体。在MIMICSQL数据集上评估。

Result: 在MIMICSQL上实现了最先进的逻辑形式准确率和有竞争力的执行准确率。更重要的是，相比标准RAG方法，展示了更高的样本效率和鲁棒性，特别是在数据稀缺和检索扰动条件下。

Conclusion: CBR-to-SQL框架通过案例推理方法有效解决了医疗领域自然语言到SQL转换的挑战，提供了比标准RAG更好的样本效率和鲁棒性，有助于降低医疗数据查询的门槛。
...
TL;DR: 该研究应用"个性化多样性助推框架"来扩展用户在新闻地域性（国内与世界新闻）方面的阅读覆盖，通过主题-地域双重校准算法助推和基于LLM的新闻个性化呈现助推，在5周真实用户实验中验证了算法助推能有效增加曝光和消费多样性，LLM助推效果不一，用户主题兴趣是点击的强预测因子，突出新闻与先前阅读的相关性优于通用主题和无个性化。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决新闻推荐系统中用户阅读覆盖范围狭窄的问题，特别是在新闻地域性（国内与世界新闻）方面的不平衡。希望通过个性化多样性助推框架来扩展用户的阅读多样性，促进更平衡的新闻消费习惯。

Method: 研究方法包括：1）设计主题-地域双重校准算法助推；2）开发基于大语言模型的新闻个性化呈现助推；3）在POPROX新闻推荐实验平台上进行为期5周的真实用户研究，涉及120名美国新闻读者；4）收集用户交互日志和调查响应进行分析。

Result: 研究结果显示：1）算法助推能成功增加曝光和消费多样性；2）基于LLM的呈现助推效果存在差异；3）用户层面的主题兴趣是用户点击的强预测因子；4）突出新闻文章与先前阅读文章的相关性优于通用主题和无个性化方法；5）长期接触校准新闻可能改变读者的阅读习惯，使其更重视国内和世界文章的平衡新闻摘要。

Conclusion: 研究结论表明个性化多样性助推框架在扩展新闻阅读覆盖方面具有潜力，特别是主题-地域双重校准算法能有效促进多样性消费。研究结果为未来新闻推荐系统中多样性消费助推工作提供了方向，强调了算法校准和个性化呈现策略的重要性。
...
TL;DR: MLLMRec-R1：针对多模态序列推荐的高效稳定GRPO推理框架，通过文本化视觉信号、构建高质量多模态CoT监督和混合粒度数据增强，解决现有方法成本过高和奖励膨胀问题。


<details>
  <summary>Details</summary>
Motivation: 将GRPO推理范式扩展到基于MLLM的多模态序列推荐面临两大挑战：1）视觉token主导输入导致基于组的rollout成本随历史长度和候选集规模急剧增加，训练成本过高；2）现有CoT监督在推荐场景中存在奖励膨胀问题，高训练奖励不能可靠转化为排序性能提升，可能导致捷径学习。

Method: 提出MLLMRec-R1框架：1）离线文本化视觉信号以消除昂贵视觉token同时保留多模态语义；2）通过细化和置信度感知评估构建高质量多模态CoT监督；3）采用混合粒度数据增强策略，选择性注入可靠CoT样本同时保留标准训练数据，缓解奖励膨胀并提高泛化稳定性。

Result: 在三个基准数据集上的广泛实验表明，MLLMRec-R1持续优于最先进方法，为多模态序列推荐建立了实用有效的GRPO推理流程。

Conclusion: MLLMRec-R1成功解决了GRPO在多模态序列推荐中的效率和稳定性问题，通过文本化视觉信号、高质量CoT监督构建和混合粒度数据增强，实现了高效稳定的推荐性能提升。
...
TL;DR: R4T是一种三步法检索框架，使用RL作为目标转换器，先训练带集合级奖励的fan-out LLM，再合成目标一致训练对，最后训练轻量扩散检索器，在保持检索质量的同时将查询时fan-out延迟降低一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现代检索问题多为集合值检索，需要优化高阶属性（如多样性、覆盖率、互补性、连贯性），但现有监督数据集只关注top-1检索。RL可以优化集合级目标但推理成本高，扩散检索能高效单次fan-out但需要目标对齐的训练数据。

Method: 提出R4T三步法：1) 使用复合集合级奖励训练fan-out LLM；2) 合成目标一致训练对；3) 训练轻量扩散检索器建模集合值输出的条件分布。该方法将RL作为一次性目标转换器，避免推理时的高成本。

Result: 在时尚和音乐大规模基准测试中，R4T相对于强基线提高了检索质量，同时将查询时fan-out延迟降低了一个数量级。

Conclusion: R4T通过将RL用作目标转换器而非推理组件，有效解决了集合值检索中目标对齐与效率的权衡问题，实现了高质量且高效的fan-out检索。
